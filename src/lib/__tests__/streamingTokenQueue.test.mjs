...
// Regression: race fix. If finalize já committed this linha (isStreaming=false),
// a late-arriving montar precisa Não append o token (iria Duplo o text) and precisa
// Não re-open o stream (iria re-trigger o streaming renderizar branch).
...
// Regression: id-collision fix. newMessageId() (crypto.randomUUID com
// counter-suffixed-timestamp fallback) replaced 33 Date.now().toString() sites
// em RefractInterface.tsx. If a fresh stream id COLLIDES com a just-finalized
// linha id, applyFirstStreamingToken NO-OPs (see o idempotency testar aacima and
// silently drops o primeiro ttoken O collision-avoidance pathway é that a
// DIFFERENT id precisa cria a brand-new streaming linha — até quando a finalized
// linha com o mesmo intent já exists.
test('applyFirstStreamingToken appends a new row when id differs from a pre-finalized row (collision-avoidance)', () => {
  ...
  // Original finalized linha precisa stay untouched.
  assert.equal(next[0].id, 'race-A');
  assert.equal(next[0].text, 'final text');
  assert.equal(next[0].isStreaming, false);
  // New streaming linha carries o primeiro token sob o fresh id.
...
// Regression: orphan-placeholder race fix em RefractInterface.queueToken.
// PAnteriormente o first-token branch passed `() => reservedId` como idFactory to
// resolveStreamingMessageId, mas resolveStreamingMessageId reuses an existing
// abrir same-intent row's id quando one exists — então a stale orphan
// isStreaming=true linha iria steal o active id and race com a concurrent
// finalize keyed em `reservedId`. Fix: (1) sempre montar com `reservedId`,
// (2) seal qualquer orphan isStreaming=true same-intent rows (id !== reservedId)
// antes applyFirstStreamingToken. This guarantees o new bubble owns its
// próprio id and orphans cannot re-enter o streaming branch.
test('seal-orphans pattern: orphan same-intent row gets isStreaming=false and a new reservedId row is appended', () => {
  ...
  // Inline simulation de o queueToken seal pattern:
  // 1) Seal orphan isStreaming=true same-intent rows onde id !== reservedId.
  const sealed = prev.map((m) =>
    m.role === 'system' && m.isStreaming && m.intent === intent && m.id !== reservedId
      ? { ...m, isStreaming: false }
      : m,
  );
  // 2) Montar o primeiro token com reservedId (Não a resolved id de sealed rows).
  ...
  // Critical: apenas ONE streaming linha remains — não race sobre que id é "active".
...
// finalizeStreamingByIntent em RefractInterface.tsx agora consults
// shouldAcceptIntelligenceIpc and, quando o active stream é a DIFFERENT
// intent (e.g. chat placeholder ainda opabrir passes streamingMsgId=null to
// finalizeStreamingByIntentMessages. Com streamingMsgId=null, o função
// precisa Não walk voltar and clobber an abrir linha de a different intent — it
// precisa append a fresh linha para o incoming intent. shouldAcceptIntelligenceIpc
// si mesmo é covered babaixo this testar pins abaixo o *messages-level* contract.
test('cross-intent finalize does not clobber different-intent placeholder', async () => {
  ...
  const next = finalizeStreamingByIntentMessages(
    ...
    'stale WTA text',
    idFactory,
    null, // cross-intent → null por Fix A
  );

  // chat-1 precisa ser untouched: ainda empty text, ainda streaming.
  ...
  // A NEW what_to_answer linha precisa ter sido appended (não merged dentro de chat-1).
