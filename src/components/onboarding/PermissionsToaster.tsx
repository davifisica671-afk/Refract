...
//
// Cartão de onboarding de permissões em visualização dividida.
// Mostra uma vez na primeira inicialização, após a interface do launcher ficar visível.
// macOS: solicita microfone via diálogo do sistema, abre Preferências do Sistema para gravação de tela.
// Windows: exibe notificação simples (o SO gerencia permissões no primeiro uso)
//
...
// ─── Spring configs para Apple-like feel ───────────────────────
const SPRING = {
...
export const PermissionsToaster: React.FC<Props> = ({ isOpen, onDismiss }) => {
  ...
  // Dynamic estilo tokens based em light/dark modo
  const colors = {
    cardBg: isLight 
    ...
    step1RefractShadow: isLight ? '0 4px 10px rgba(0,0,0,0.12)' : '0 4px 12px rgba(0,0,0,0.4)',
    ...
    // Step 2: macOS System Settings Alternar Mockup
        ...
        >
          ...
          >
            ...
            <div style={{ display: 'flex', minHeight: '460px' }}>
              ...
              {/* ── LEFT: Permissão controla ── */}
              <div style={{ flex: 1, padding: '32px 32px 28px', display: 'flex', flexDirection: 'column' }}>

                {/* Cabeçalho linha */}
                <div style={{ display: 'flex', alignItems: 'center', marginBottom: '24px' }}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
                    <img src={refractIcon} alt="Refract" style={{ width: '18px', height: '18px', borderRadius: '4px', flexShrink: 0 }} />
                ...
                >
                  ...
                  <p style={{ fontSize: '13px', lineHeight: 1.65, color: t3, margin: 0 }}>
                    Refract needs a few permissions to capture meetings and transcribe speech.
                  </p>
                </motion.div>
              ...
              >
                {/* Fechar button em o top-right corner de graphics section */}
                  ...
                  >
                    ...
                    >
                      <div style={{ display: 'flex', gap: '10px', alignItems: 'flex-start' }}>
                        {/* Refract Icon com subtle breath/float loop */}
                        <motion.div 
                          animate={{ y: [0, -2, 0] }}
                          transition={{ repeat: Infinity, duration: 2.8, ease: "easeInOut" }}
                          style={{ width: '32px', height: '32px', flexShrink: 0 }}
                        >
                          <img src={refractIcon} alt="Refract" style={{ width: '32px', height: '32px', borderRadius: '7px', boxShadow: colors.step1RefractShadow }} />
                        </motion.div>
                        
                        {/* Prompt Text */}
                        <div style={{ display: 'flex', flexDirection: 'column', gap: '2px' }}>
                          <div style={{ fontSize: '10px', fontWeight: 600, color: colors.step1TextPrimary, lineHeight: 1.3, letterSpacing: '-0.01em' }}>
                            "Refract" wants to record the screen.
                          </div>
                    ...
                    >
                      ...
                      <div style={{ width: '22px', height: '22px', borderRadius: '5px', background: colors.step2IconBg, display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0, border: colors.step2IconBorder }}>
                        <img src={refractIcon} alt="Refract" style={{ width: '14px', height: '14px', borderRadius: '3px' }} />
                      </div>
                      {/* Label */}
                      <span style={{ fontSize: '10px', fontWeight: 550, color: colors.step2Text, flex: 1, letterSpacing: '-0.01em' }}>
                        Refract
                      </span>
                      {/* Active Alternar Trocar com premium gradient, looping animation, and manual tap sobrescrever */}
                      <motion.div 
                        animate={{
...
// ─── Single permissão item com alternar ───────────────────────
function PermItem({
  ...
  // macOS lê o Screen Recording conceder at processo launch: re-enabling it em
  // System Settings faz Não aplica para o executando app, então a previously-denied
  // user precisa relaunch. O mic conceder Faz take efeito live, então não hint tlá
