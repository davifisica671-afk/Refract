...
const UpdateModal: React.FC<UpdateModalProps> = ({
    ...
    // Função auxiliar para formatar string de versão
    ...
    // Alternar automaticamente para visão de progresso se o status mudar para downloading E foi iniciado pelo usuário
    const handleUpdateClick = () => {
        onInstall();
    };

    const handleCopyCommand = () => {
        navigator.clipboard.writeText('xattr -cr /Applications/Refract.app');
        setCopied(true);
        setTimeout(() => setCopied(false), 2000);
    };
    ...
    React.useEffect(() => {
        // Apenas executar se não estiver baixando/com erro e tivermos notas
        ...
        const scroll = () => {
            ...
            // Velocidade de rolagem suave
            el.scrollTop += 0.5;

            // Verificar se chegou ao fundo (com pequeno buffer)
        ...
        // Iniciar rolagem após pequeno atraso para permitir que a renderização finalize
            ...
            {isOpen && (
                <div className="fixed inset-0 z-[9999] flex items-center justify-center font-sans antialiased">
                    ...
                    <motion.div
                        ...
                        className="update-modal dialog-surface relative w-[530px] h-[400px] overflow-hidden flex flex-col"
                    >
                        {/* Content Container */}
                        {status === 'error' ? (
                                    ...
                                    {isMac ? (
                                        <>
                                            <div className="space-y-1 w-full">
                                                ...
                                                <CopyBlock command={`xattr -cr ~/Downloads/Refract-${displayVersion.replace('v', '')}-${instructionsArch || 'arm64'}.dmg`} />
                                            </div>
                                            <div className="space-y-1 mt-1 pl-0.5">
                                                <p className="text-[12px] font-medium text-white/80">2. Open the file and install Refract.</p>
                                            </div>
                                            <div className="space-y-1 mt-3 w-full">
                                                ...
                                                <CopyBlock command="xattr -cr /Applications/Refract.app" />
                                            </div>
                                        </>
                                    ) : (
                                        <div className="space-y-1 w-full">
                                            <p className="text-[12px] font-medium text-white/80">Run the downloaded installer (.exe) and follow the prompts. Refract will restart when finished.</p>
                                        </div>
                                    )}
                                </div>
                            ...
                            <div className="p-8 flex flex-col items-center justify-center h-full text-center relative">
                                ...
                                {/* 2. Premium Troubleshooting Card — macOS-only.
                                    ...
                                    equivalent), and the /Applications/Refract.app
                            ...
                            <div className="p-7 pb-4 flex flex-col gap-2 h-full min-h-0">
                                            ...
                                            {parsedNotes?.sections?.map((section, idx) => {
                                                ...
                                                return (
                                                    <div key={idx} className="space-y-2.5">
                                                        {/* Section HCabeçalho Refined, Medium Weight */}
                                ...
                                <div className="flex items-center justify-between flex-shrink-0">
                                    {/* Secondary Ação - Left Aligned, Plain Text */}
                                    <button
                                        onClick={onDismiss}
                                    ...
                                    {/* Primário Ação - Direito Aligned, System Blue */}
                                    {status === 'ready' ? (
                                        <button
                                            onClick={() => window.electronAPI.restartAndInstall()}
                                            className="h-9 px-5 bg-accent-primary hover:brightness-110 text-white text-[12px] font-semibold rounded-lg shadow-sm transition-all active:scale-[0.98]"
                                        >
                                            Restart & Install
                                        </button>
                                    ) : (
