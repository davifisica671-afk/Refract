 ...
 *
 * Overlay de chat global para busca em todas as reuniões salvas.
 * Utiliza RAG (Retrieval-Augmented Generation) para buscar contexto
 * em reuniões anteriores e gerar respostas relevantes.
 * Suporta streaming de respostas e fallback para chat padrão.
 */
...
const GlobalChatOverlay: React.FC<GlobalChatOverlayProps> = ({
    ...
    // Submit initial consulta quando overlay abre
    useEffect(() => {
    ...
    // Ouvir para novo queries de parent
    useEffect(() => {
        if (isOpen && initialQuery && messages.length > 0) {
            // This é a follow-up consulta
            submitQuestion(initialQuery);
        }
    }, [initialQuery]);

    // ESC chave manipulador
    useEffect(() => {
    ...
    const submitQuestion = useCallback(async (question: string) => {
        ...
        try {
            // Adiciona typing indicator atrasar (200ms) - makes o AI feel "thoughtful"
            await new Promise(resolve => setTimeout(resolve, 200));

            // Cria assistant mensagem placeholder
            ...
            // Conjunto para cima RAG streaming listeners (RAF-batched)
            streamBuffer.reset();
            ...
            if (result?.fallback) {
                ...
                // Configura alternativa listeners (Standard Gemini)
                streamBuffer.reset();
