...
//
// Toast de promoção de trial inspirado no estilo Apple.
// Exibe 5 segundos após o launcher ficar visível em iniciios não-primários,
// quando nenhuma chave de API Refract está armazenada e nenhum trial está ativo.
// Acento violeta/roxo — consistente com a marca de trial em todo o app.
...
// Chave de localStorage para rastrear se as permissões já foram exibidas
const PERMS_KEY        = 'refract_perms_shown_v1';
// Atraso antes de mostrar o toast (10 segundos)
const STARTUP_DELAY_MS = 10_000;

// Tokens de design: fonte e paleta de cores violeta
const T = {
...
// Variantes de animação: escalonamento dos itens internos do toast
const STAGGER = { hidden: {}, show: { transition: { staggerChildren: 0.07, delayChildren: 0.12 } } };
// Variantes de animação: entrada individual de cada item com desfoque
const ITEM    = {
  hidden: { opacity: 0, y: 14, filter: 'blur(4px)' },
  show:   { opacity: 1, y: 0,  filter: 'blur(0px)', transition: { duration: 0.5, ease: [0.22, 1, 0.36, 1] as any } },
};

// Props do toast de promoção de trial
interface Props {
  isOpen:         boolean;          // Se o toast deve estar disponível
  hasRefractKey: boolean;          // Se o usuário já possui uma chave Refract
  hasTrialToken:  boolean;          // Se o usuário já tem um trial ativo
  onDismiss:      () => void;       // Chamado ao dispensar o toast
  onStartTrial:   () => Promise<void>; // Chamado ao iniciar o trial
  onManualSetup:  () => void;       // Dispensa e abre as configurações
}

// Componente principal do toast de promoção de trial
export const TrialPromoToaster: React.FC<Props> = ({
  isOpen, hasRefractKey, hasTrialToken, onDismiss, onStartTrial, onManualSetup,
}) => {
  // Estado: visibilidade, iniciar trial e erros
  const [visible,  setVisible]  = useState(false);
  const [starting, setStarting] = useState(false);
  const [error,    setError]    = useState<string | null>(null);
  // Preferência de movimento reduzido do sistema
  const reduced = useReducedMotion() ?? false;
  // Tema atual (claro/escuro)
  ...
  // Efeito para controlar quando o toast deve aparecer
  useEffect(() => {
    if (!isOpen) { setVisible(false); return; }

    // Não exibe se o usuário já tem chave ou trial ativo
    if (hasRefractKey || hasTrialToken) return;

    // Não exibe no primeiro launch (o toast de permissões é exibido nesse caso)
    const permsShown = localStorage.getItem(PERMS_KEY);
    if (!permsShown) return;

    // Verifica o controle central de exibição do toast
    if (!isToasterAllowed('trial_promo')) return;

    // Exibe após o atraso configurado
    const t = setTimeout(() => {
      setVisible(true);
      markToasterAsShown('trial_promo');
    }, STARTUP_DELAY_MS);
    return () => clearTimeout(t);
  }, [isOpen, hasRefractKey, hasTrialToken]);

  // Dispensa o toast e registra que foi exibido
  const handleDismiss = () => {
    markToasterAsShown('trial_promo');
    setVisible(false);
    onDismiss();
  ...
  // Inicia o trial e oculta o toast em caso de sucesso
  ...
  // Configuração manual: dispensa e abre as configurações
  const handleManual = () => {
    markToasterAsShown('trial_promo');
    setVisible(false);
    onManualSetup();
  ...
  // Não renderiza nada se não for para ser visível
  ...
  return (
    <AnimatePresence>
      ...
      >
        {/* Wrapper externo principal — container Bento Grid sem borda */}
        <motion.div
          key="trial-card"
        ...
        >
          ...
          <div style={{
            ...
            {/* Textura orgânica fina (noise/grain) */}
            ...
            <div style={{ padding: '28px 24px 26px', position: 'relative', zIndex: 6 }}>
              {/* Cabeçalho do toast com título e botão de fechar */}
              ...
              <motion.div variants={STAGGER} initial="hidden" animate="show" style={{ display: 'flex', flexDirection: 'column', gap: '22px' }}>
                ...
                {/* Seção hero: grande número "10" representando minutos gratuitos */}
                <motion.div variants={ITEM} style={{ textAlign: 'center' }}>
                  <div style={{ display: 'flex', alignItems: 'stretch', justifyContent: 'center', gap: '5px', marginBottom: '16px' }}>
                    ...
                    {/* Stacked label coluna — tight pilha at bottom */}
                  ...
                  <p style={{ fontSize: '13px', lineHeight: 1.66, color: t3, margin: '0 auto', maxWidth: '330px' }}>
                    Full Refract API access — AI chat, meeting transcription, and company research — free for 10 minutes. Bound to this device. No sign-in.
                  </p>
                </motion.div>

                {/* Chips de funcionalidades: AI Chat, Transcrição, Pesquisa */}
...
// Botão violeta de chamada para ação (CTA) com efeito de vidro e brilho
const VioletCTA: React.FC<{ label: string; onClick: () => void; disabled: boolean; reduced: boolean }> = ({ label, onClick, disabled, reduced }) => {
  ...
  return (
    ...
    >
      {/* Camada de destaque brilhante 3D estilo gelatina */}
      {!disabled && (
        <span style={{ 
      ...
      {/* Efeito de brilho shimmer animado */}
      {!reduced && !disabled && (
        <motion.div aria-hidden
          style={{
      ...
      {/* Ícone de seta no canto direito (padrão botão-dentro-do-botão) */}
      {!disabled && (
        <div style={{
