...
//
// Modal de atualização pós-trial — estilo Apple com vidro escuro.
// Os cartões de plano seguem a estética de assinatura do Apple One:
// elevação no hover, brilho de destaque, texto orientado a benefícios,
// CTA dominante único e rodapé de confiança — tudo otimizado para conversão.
...
// Curva de easing Apple para transições suaves
const EASE = 'cubic-bezier(0.4,0,0.2,1)';

// Níveis de opacidade para texto — o contraste é controlado por toque
const C = {
  t1:  '#FFFFFF',
...
// Paletas de destaque — cores para ícone, botão, borda e brilho no hover
const ACC = {
  violet: {
    iconColor:   '#A78BFA',
...
// Props do modal de trial
interface TrialModalProps {
  usage:      { ai: number; stt_seconds: number; search: number }; // Uso atual do trial
  onByok:     () => Promise<void>;   // Usar chave própria (BYOK)
  onStandard?: () => Promise<void>;  // Atualizar para plano Standard
  onDone?:    () => void;            // Chamado ao fechar o modal
}

// Etapas do modal: escolha, limpando dados e concluído
type Step = 'choose' | 'wiping' | 'done';

// Componente principal do modal de trial gratuito
export const FreeTrialModal: React.FC<TrialModalProps> = ({ usage, onByok, onStandard, onDone }) => {
  // Estado: etapa atual, mensagem de erro e preferência de movimento reduzido
  const [step,  setStep]  = useState<Step>('choose');
  const [error, setError] = useState<string | null>(null);
  const reduced = useReducedMotion() ?? false;

  // Abre URL externa via API do Electron
  const openUrl = (url: string) => (window.electronAPI as any)?.openExternal?.(url);

  // Lida com a opção BYOK: limpa dados do trial e transiciona para "concluído"
    ...
    <>
      <div style={{
        ...
        <motion.div
          ...
          style={{padding:'1px',borderRadius:'19px',background:'rgba(255,255,255,.12)',boxShadow:'0 44px 110px -24px rgba(0,0,0,.9)'}}
        >
          <div style={{
            ...
            <div style={{padding:'22px 22px 24px',position:'relative',zIndex:6}}>
              {/* Renderiza o estado correspondente à etapa atual */}
...
// ─── Estado de Escolha ──────────────────────────────────────────
// Exibe os planos disponíveis e opções de atualização
...
function ChooseState({ usage, error, reduced, onPro, onMax, onUltra, onStandard, onByok }: {
  ...
  // Converte segundos de STT para minutos
    ...
    <div style={{display:'flex',flexDirection:'column',gap:'10px'}}>
      ...
      <div style={{display:'flex',alignItems:'center',gap:'10px',paddingBottom:'12px',borderBottom:`1px solid ${C.div}`}}>
        <div style={{width:'34px',height:'34px',borderRadius:'10px',background:'rgba(139,92,246,.13)',border:'1px solid rgba(139,92,246,.22)',display:'flex',alignItems:'center',justifyContent:'center',flexShrink:0}}>
          <RefractLogoMark size={16} className="text-blue-400" />
        </div>
        <div>
      ...
      <div style={{display:'flex',flexDirection:'column',gap:'6px'}}>
        ...
        <HeroCard
          title="Refract Pro" price="$15" period="/mo" icon={Zap}
      ...
      {/* ── Usar chave própria (BYOK) e informações de confiança ─── */}
...
// ─── Cartão Hero (Plano Pro) ──────────────────────────────────
// Linguagem Tesla: nome + badge inline + âncora de preço + uma linha de especificação + CTA.
// Hover: eleva 2px + borda brilhante + brilho violeta expandido
...
// ─── Cartão de Nível (Max / Ultra) ──────────────────────────────
// Mesma anatomia do HeroCard mas compacto — sem faixa, preço menor.
...
// ─── Cartão Slim (Standard) ────────────────────────────────────
// Linha horizontal simples — elevação + borda no hover.
      ...
      >
        ...
        <div style={{flex:1}}>
          ...
          <div style={{fontSize:'10.5px',color:C.t4,marginTop:'1px'}}>
            Refract API disabled · No Pro features
          </div>
        </div>
...
// Estados de transição: limpando dados e conclusão
...
// Estado de conclusão: exibe mensagem de sucesso após limpeza dos dados do trial
