// src/components/RefractQuotaBanner.tsx
// Banner de cota exibido quando qualquer cota do Refract atinge ≥90% de uso.
// Verifica na inicialização — sem limitação de taxa. Exibe aviso de proximidade
// do limite e botão de upgrade.
...
export const RefractQuotaBanner: React.FC = () => {
    ...
    useEffect(() => {
        ...
        const check = async () => {
            ...
            try {
                const result = await window.electronAPI?.getRefractUsage?.();
                console.log('[RefractQuotaBanner] usage:', JSON.stringify(result));

                if (cancelled || !result?.ok || !result.quota) {
                    console.log('[RefractQuotaBanner] no quota data — skipping');
                    return;
                }
                ...
                console.log('[RefractQuotaBanner] near-limit:', near);
                ...
                console.log('[RefractQuotaBanner] error:', e?.message);
            }
        };
                ...
                <div className="bg-[#1A1A1A] border border-amber-500/25 shadow-2xl rounded-2xl p-4 flex flex-col gap-3">
                    ...
                    <div className="flex items-start justify-between gap-2">
                        <div className="flex items-center gap-2">
                            ...
                            <span className="text-[13px] font-semibold text-[#E0E0E0]">Refract quota almost full</span>
                        </div>
                        <button
