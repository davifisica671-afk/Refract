 ...
 *
 * Painel de configuração de provedores de IA do Refract.
 * Gerencia chaves de API para provedores cloud (Gemini, Groq, OpenAI, Claude, DeepSeek, OpenCode Zen),
 * proxy LiteLLM, provedores locais (Ollama, Codex CLI), provedores personalizados (cURL),
 * modelo padrão, modo rápido e roteamento de visão.
 */
...
// Predefinições de tokens máximos de saída do LiteLLM — orçamentos padrão por modelo
// ('' = Auto: resolve o orçamento real de cada modelo do /model/info do proxy, alternativa 8192).
...
export const AIProvidersSettings: React.FC = () => {
    ...
    const canUseFastMode = !!(hasStoredKey.groq || hasStoredKey.refract || codexCliConfig.enabled);
            ...
            try {
                ...
                if (creds) {
                    setHasStoredKey({
                        ...
                        opencode_zen: creds.hasOpencodeZenKey || false,
                        litellm: creds.hasLitellmBaseURL || false,
                        refract: creds.hasRefractKey || false
                    });
                    ...
                    if (creds.opencodeZenPreferredModel) pm.opencode_zen = creds.opencodeZenPreferredModel;
    ...
    // Effect to enforce fast mode disabled if neither Groq key nor Refract API is configured.
    ...
    useEffect(() => {
        ...
        if (!canUseFastMode && fastResponseMode) {
            ...
            localStorage.setItem('refract_groq_fast_text', 'false');
            // @ts-ignore
    ...
    const handleSaveKey = async (provider: string, key: string, setter: (val: string) => void) => {
        ...
        try {
            ...
            // @ts-ignore
            if (provider === 'opencode_zen') result = await window.electronAPI.setOpencodeZenApiKey(key);
    ...
    const handleRemoveKey = async (provider: string, setter: (val: string) => void) => {
        ...
        try {
            ...
            // @ts-ignore
            if (provider === 'opencode_zen') result = await window.electronAPI.setOpencodeZenApiKey('');
        ...
        <div className="space-y-5 animated fadeIn pb-10">
            ...
            <div className="space-y-5">
                ...
                <div className="bg-bg-item-surface rounded-xl p-5 border border-border-subtle flex items-center justify-between">
                    ...
                    <ModelSelect
                        ...
                        options={(() => {
                            ...
                            if (hasStoredKey.refract) {
                                opts.push({ id: 'refract', name: 'Refract API' });
                            }
                ...
                <div
                    ...
                    title={!canUseFastMode ? "Requires Groq, Refract API, or Codex CLI to be configured" : ""}
                >
                    <div className="flex-1">
                        ...
                        <p className="text-[10px] text-text-secondary mt-0.5">Super fast responses using the Codex CLI fast model, Groq, or Refract. Turn this off to use the selected normal model.</p>
                        {!canUseFastMode && (
                            <p className="text-[10px] text-orange-500 mt-0.5 font-medium">Requires Groq, Refract API, or Codex CLI to be configured.</p>
                        )}
                    </div>
                    <div
                        onClick={async () => {
                            if (!canUseFastMode) {
                                alert("Please configure Groq, Refract API, or Codex CLI first to enable Fast Response Mode.");
                                return;
                            }
                            const newState = !fastResponseMode;
                            setFastResponseMode(newState);
                                localStorage.setItem('refract_groq_fast_text', String(newState));
                            // @ts-ignore
            ...
            <div className="space-y-5">
                ...
                <div className="space-y-4">
                    ...
                    {/* OpenCode Zen — OpenAI-compatible gateway for curated coding models. */}
                    <ProviderCard
                        providerId="opencode_zen"
                        providerName="OpenCode Zen"
                        apiKey={opencodeZenApiKey}
                        preferredModel={preferredModels.opencode_zen}
                        hasStoredKey={!!hasStoredKey.opencode_zen}
                        onKeyChange={setOpencodeZenApiKey}
                        onSaveKey={async () => { await handleSaveKey('opencode_zen', opencodeZenApiKey, setOpencodeZenApiKey); }}
                        onRemoveKey={() => handleRemoveKey('opencode_zen', setOpencodeZenApiKey)}
                        onTestConnection={() => handleTestConnection('opencode_zen', opencodeZenApiKey)}
                        testStatus={testStatus.opencode_zen || 'idle'}
                        testError={testError.opencode_zen}
                        savingStatus={!!savingStatus.opencode_zen}
                        savedStatus={!!savedStatus.opencode_zen}
                        keyPlaceholder="oc_zen_..."
                        keyUrl="https://opencode.ai/zen"
                        onPreferredModelChange={(model) => setPreferredModels(prev => ({ ...prev, opencode_zen: model }))}
                    />
    ...
    "temperature": 0.7
            ...
            <div className="space-y-5">
                <div>
                    ...
                    <p className="text-xs text-text-secondary mb-2">Pick how Refract reads what is on your screen. All paths use the vision-capable AI provider directly; OCR is no longer used.</p>
                </div>
            ...
            <div className="space-y-5">
                ...
                <div className="bg-bg-item-surface rounded-xl p-4 border border-border-subtle flex flex-col gap-2">
                    ...
                    <div className="flex items-start gap-2 mt-1 pt-3 border-t border-border-subtle">
                        ...
                        <p className="text-[11px] text-text-tertiary leading-relaxed">When a data type is disabled, Refract falls back to the best available local model to keep that data on-device.</p>
                    </div>
                </div>
            </div>
            </div>
