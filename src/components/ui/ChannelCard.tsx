/**
 * ChannelCard — Card de status de canal de áudio (STT)
 *
 * Exibe o status atual de um canal de transcrição de fala para texto (STT),
 * incluindo o nome do canal, status de conexão (conectado, reconectando,
 * falhou, aguardando áudio), provedor utilizado e detalhes de erro quando
 * aplicável. Permite copiar os detalhes do erro para a área de transferência.
 */
import React, { useState } from 'react';
import type { SttErrorCategory } from '../../lib/sttErrorMapper';

interface ChannelCardProps {
    /** Nome do canal exibido no cabeçalho */
    name: string;
    /** Status do canal ('awaiting-audio' = estado neutro pré-verificação de áudio) */
    status: 'connected' | 'reconnecting' | 'failed' | 'awaiting-audio';
    /** Nome do provedor STT (ex: 'google', 'openai', 'deepgram') */
    provider?: string;
    /** String de erro bruta */
    error?: string;
    /** Informações categorizadas do erro (título + corpo) */
    errorCategory?: SttErrorCategory | null;
    /** Ícone SVG para cada status */
    iconConnected: React.ReactNode;
    iconReconnecting: React.ReactNode;
    iconFailed: React.ReactNode;
}

/** Rótulo legível do provedor */
...
const ChannelCard: React.FC<ChannelCardProps> = ({
    ...
    // 'awaiting-audio' usa o mesmo tratamento visual de 'reconnecting'
    // ( âmbar, sem erro) mas com um rótulo distinto para que os usuários saibam
    // que o áudio ainda não foi verificado — diferente de "tínhamos a conexão
    // e a perdemos."
