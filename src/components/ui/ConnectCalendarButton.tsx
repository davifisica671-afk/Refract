/**
 * ConnectCalendarButton — Botão de conexão com Google Calendar
 *
 * Componente que permite ao usuário conectar sua conta do Google Calendar.
 * Possui dois estados visuais: desconectado (botão com estilo dark/roxo)
 * e conectido (indicateur com estilo glass premium e efeito de aurora).
 * O estado é verificado automaticamente ao montar o componente.
 * Inclui efeitos visuais avançados como borda iridescente animada,
 * textura de ruído cristalino e shimmer elegante.
 */
...
const ConnectCalendarButton: React.FC<ConnectCalendarButtonProps> = ({ className = '', variant = 'default', ...props }) => {
    ...
    const handleClick = async (e: React.MouseEvent<HTMLButtonElement>) => {
        ...
        if (connected) return; // Por enquanto não desconecta aqui
        ...
        try {
            ...
            if (res.success) {
                setConnected(true);
                props.onConnect?.();
                // Registra a conexão do calendário no analytics
    ...
    if (connected) {
        return (
            ...
            >
                {/* 1. Borda Aurora Iridescente Animada */}
                <motion.div
                ...
                {/* 2. Textura de Ruído Cristalino (Grão sutil para realismo) */}
                <div
                ...
                <span className="relative z-10 flex items-center gap-3 pl-0.5">
                    ...
                    {/* Texto: Tipografia de Alta Qualidade */}
    ...
    return (
        <button
            ...
            `}
            style={{
                // Preenchimento base: roxo escuro
                backgroundColor: 'rgba(60, 20, 80, 0.4)',
                // Filtro de desfoque de fundo 12-16px
        ...
        >
            ...
            {/* Destaque interno (borda superior) */}
            <div
            ...
            <span className="relative z-10 flex items-center gap-2.5 font-semibold">
                ...
                {!loading && (
                    <ArrowRight
                        ...
                        style={{ color: 'rgba(244, 246, 250, 0.9)' }} // Ligeiramente mais claro/combinando com o texto
                    />
                )}
            </span>
        </button>
