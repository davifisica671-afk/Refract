 ...
 *
 * Componente dropdown que permite ao usuário selecionar um modelo de inteligência
 * artificial entre três categorias: Cloud (provedores externos como Google, OpenAI),
 * Custom (provedores personalizados via cURL) e Local (modelos Ollama).
 *
 * O dropdown carrega dinamicamente os modelos disponíveis a partir das credenciais
 * armazenadas e dos provedores Ollama locais. Exibe ícones e descrições para cada
 * modelo, com indicador visual do modelo selecionado.
 */
...
export const ModelSelector: React.FC<ModelSelectorProps> = ({ currentModel, onSelectModel }) => {
    ...
    // Carrega os dados dos modelos quando o dropdown é aberto
    useEffect(() => {
        if (!isOpen) return;

        const loadData = async () => {
            try {
                // Carrega provedores personalizados
                ...
                // Constrói a lista dinâmica de modelos cloud a partir das credenciais
                // @ts-ignore
                const creds = await window.electronAPI?.getStoredCredentials?.();
                const cModels: { id: string; name: string; desc: string; provider: string }[] = [];

                if (creds?.hasRefractKey) {
                    cModels.push({ id: 'refract', name: 'Refract API', desc: 'Managed AI • Fast execution', provider: 'refract' });
                }
    ...
    const handleSelect = (model: string) => {
        // Para modelos custom/local, podemos precisar passar um ID ou formato específico.
        // A lógica do backend (LLMHelper) precisa saber como manipular essa string, ou
        // precisamos de um objeto mais rico. Por enquanto, para consistência com o app
        // existente, passamos uma string. Confiaremos em uma convenção de prefixo ou
        // apenas no nome se for único o suficiente. Assumimos que onSelectModel
        // gerencia a lógica de alternância.
