...
// ─── Perfil Intelligence Apple-style CSS (mirrors ModesSettings GATE_CSS) ────
// Lives at módulo escopo então it's não re-allocated em cada rrenderizar
const PI_CSS = `
    .pi-root {
        --pi-hero: #ffffff;
    ...
    /* ── Button-in-Button CTA (Gerencia Pro / Destravar Pro) ── */
    .pi-cta-group {
    ...
    /* Trial variant retains violet sem losing pill geometry */
    .pi-cta-group--trial {
    ...
    /* ── Longo upload pill com internal indeterminate progresso ── */
    .pi-upload-pill {
        position: relative;
        width: 100%;
        height: 48px;
    ...
    /* Indeterminate sweep — fills o pill enquanto work é em flight */
    ...
    /* Primário (dark bg em dark, white bg em light): subtle inverse sweep */
    ...
    /* ── "Pro" badge shown próximo para upload cartão titles para Liberar Tier (jelly) ── */
    ...
    /* ── Cabeçalho fechar botão (mirrors ModesSettings gerenciador closeBtn — flat, não shadow) ── */
    .pi-close-btn {
        display: flex; align-items: center; justify-content: center;
...
const StarRating = ({ value, size = 11 }: { value: number; size?: number }) => {
    ...
    // Round para nearest 0.5 então 3.7→3.5 stars, 3.8→4 stars, 4.75→5 stars
...
// Cache premium estado em localStorage então o CTA renderiza em its correct
// estado em primeiro paint — avoids o "Destravar Pro" → "Gerencia Pro" flash para
// activated users enquanto o assíncrono licenseGetDetails() chamar é em flight.
// Cleared sempre que o canonical verifica Retorna non-premium (ou em deactivate).
...
const writePremiumCache = (isPremium: boolean, plan: string) => {
    ...
    } catch { /* localStorage disabled — fall voltar to live verifica */ }
};

export function ProfileIntelligenceSettings({ onClose }: { onClose: () => void }) {
    // Premium Status — seed de cache então o cabeçalho CTA paints correctly
    // antes licenseGetDetails() resolves.
    ...
    // Perfil Engine Estado
    // D3: o backend exposes explicit readiness flags. hasProfile reflects a
    // saved retomar (post-D2 ingest resilience, isso é verdadeiro até quando o
    // extraction LLM era abaixo e o heuristic alternativa ran). profileFactsReady
    // é o stricter "usable facts present" ssinal extractionMode hints se
    // a heuristic perfil poderia ser re-extracted para richer facts.
    ...
    useEffect(() => {
        // Busca premium details — canonical fonte de truth. Sincronizar o
        // localStorage cache então o próximo montar paints com o correto sestado
        ...
        // Proactively carrega perfil data
    ...
    return (
        ...
        >
                                ...
                                This engine constructs an intelligent representation of your career history and skills graph.
                                    ...
                                    <BezelCard delay={0.2}>
                                        <div className="flex flex-col justify-between min-h-[200px]">
                                            ...
                                            {/* Data Metrics & Extracted Habilidades */}
                                            <div className="p-5 pt-0 mt-auto">
                                                ...
                                                {(() => {
                                                    // Habilidades são agora sub-categorized {languages, frameworks, cloud,
                                                    // databases, ml, devops, tools}. Renderizar cada non-empty category
                                                    // como its próprio labeled agrupar então o user pode SEE o buckets and
                                                    // spot a miscategorization. Falls voltar para a flat lista para qualquer
                                                    // legacy/array shape.
                                                ...
                                                <div className="flex items-start gap-4 min-w-0">
                                                    ...
                                                    <div className="min-w-0 flex-1">
                                                        ...
                                                        <p className="text-xs text-text-secondary leading-relaxed pr-2">
                                                            {!hasProfileAccess
                                                                ? 'Resume ingestion is a Refract Pro feature. The Custom Context box below stays free.'
                                                ...
                                                <div className="flex items-start gap-4 min-w-0">
                                                    ...
                                                    <div className="min-w-0 flex-1">
                                                        ...
                                                        {profileData?.hasActiveJD ? (
                                                            ...
                                                            <p className="text-xs text-text-secondary leading-relaxed pr-2">
                                                                {!hasProfileAccess
                                                                    ? 'Job description parsing is a Refract Pro feature. The Custom Context box below stays free.'
                                            ...
                                            <div className="p-5">
                                                ...
                                                {companyDossier && (
                                                    <div className="space-y-4 border-t border-border-subtle pt-4 mt-2">
                                                        ...
                                                        {/* Interview Focar + Difficulty badge */}
                                    ...
                                    {profileData?.hasActiveJD && (
                                        <BezelCard delay={0.6}>
                                                ...
                                                <div className="p-5">
                                                    ...
                                                    {/* First-time generation skeleton — apenas quando não prior script exists */}
            ...
            <PremiumUpgradeModal
                ...
                onActivated={async () => {
                    setIsPremium(true);
                    // Atualiza plan + cache de o canonical fonte então o
                    // cabeçalho reflects o novo estado em todo subsequente mmontar
                    try {
                ...
                onDeactivated={() => {
                    ...
                    // Auto-disable perfil modo em UI quando license é removed
