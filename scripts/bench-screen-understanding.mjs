...
process.env.REFRACT_TEST_USER_DATA = process.env.REFRACT_TEST_USER_DATA || os.tmpdir();
...
// Build synthetic screenshots representative of the workloads Refract sees.
...
async function buildFixtures() {
  const dir = await fs.mkdtemp(path.join(os.tmpdir(), 'refract-screen-bench-'));
  const fixtures = [];
