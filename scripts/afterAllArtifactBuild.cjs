 ...
 *        "The signature of the binary is invalid" @ Refract.app/Contents/MacOS/Refract
 ...
 * (APPLE_KEYCHAIN_PROFILE, e.g. `refract-notary`). No-op if none are present.
 */
...
const VOLNAME = 'Refract';
const BACKGROUND = path.resolve(__dirname, '..', 'assets', 'dmg-background.png');
const VOLICON = path.resolve(__dirname, '..', 'assets', 'refract.icns');
...
function buildStyledDmg({ appPath, outDmg, identity }) {
  ...
  // exactly [Refract.app, Applications-droplink] and nothing stray.
  const stage = fs.mkdtempSync(path.join(os.tmpdir(), 'refract-dmg-'));
