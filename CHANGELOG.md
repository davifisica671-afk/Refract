    ...
    - **Log Forwarding**: Added `open-log-file` and console logging forwarding to `~/Documents/refract_debug.log`.
    ...
    - **Multimodal Resilience:** Added a "Smart Dynamic Fallback" using Groq Llama 4 Scout. If default vision models fail or get rate-limited during screen analysis, Refract instantly reroutes the image to ensure uninterrupted performance.
    - **Multiple Screenshot Support:** The Refract Interface can now handle and process multiple attached screenshots simultaneously instead of just one.
    ...
    2. Open Terminal and run: `xattr -cr /Applications/Refract.app`
