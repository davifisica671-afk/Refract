...
The 34 required migration-spec scenarios. **Status:** `covered` = a pre-existing test already asserts it; `added` = a Phase-11 gap-filler test was written. Suite keys: **core** = `packages/stt-relay-core/tests/`, **relay** = `services/stt-relay/tests/`, **cp** = `refract-api/tests/`, **mig** = `migrations/__tests__/`, **client** = `electron/audio/__tests__/`.
...
The relay suite grew 102 → **106** with the 4 gap-fillers. The electron suite is run after `npm run build:electron` (the tests load the compiled `dist-electron/electron/audio/RefractProSTT.js`). The control-plane integration tier auto-skips cleanly without Supabase env (same skip-guard as `stt-health-system.test.mjs`); the offline tiers always run.
...
- Boots the relay **in-process** on an ephemeral port (reusing the relay's own `tests/_helpers.mjs` harness, so its `@refract/stt-relay-core` imports resolve through the relay's `node_modules`).
...
| Path | What |
|---|---|
| `refract-api/scripts/load-test-stt-relay.mjs` | in-process load generator (NEW) |
| `refract-api/scripts/verify-stt-relay-rollout.mjs` | rollout smoke verifier, self-contained + `--live` (NEW) |
| `refract-api/services/stt-relay/tests/provider-failure-modes.test.mjs` | 4 gap-filler tests: 1011-loop, key cooldown at relay, Google invalid-language ×2 (NEW) |
| `refract-api/services/stt-relay/tests/_helpers.mjs` | the reused `create()` harness (mock providers, token mint, ephemeral port) |
...
1. **Scripts are `.mjs`**, matching `refract-api`'s `"type": "module"`. They import the relay's `tests/_helpers.mjs` by absolute path so the core package resolves through the relay's own `node_modules` symlink (the core is not resolvable from the `refract-api` root, only from `services/stt-relay/`).
