...
Auth: same credential surface as every authenticated GET — `authenticate()` (`x-refract-key` / `Authorization: Bearer <key>` via `getKey()`, or `x-trial-token` via `getTrialToken()`), preceded by `checkDDoS`. Same 401 vocabulary as the session endpoint.

```
GET /v1/stt/relays
Authorization: Bearer refract_sk_…        (or x-refract-key / x-trial-token)

200 {
  "relays": [
    { "region": "us",   "ws_url": "wss://us-relay.refract.software/ws",   "healthy": true,  "latency_ms": 42 },
    { "region": "asia", "ws_url": "wss://asia-relay.refract.software/ws", "healthy": false, "latency_ms": null }
  ],
...
200 {
  ...
  "regions": {
    "us":   { "url": "wss://us-relay.refract.software/ws", "configured": true, "healthy": true,
...
| `STT_RELAY_RAILWAY_FALLBACK_URL` | `wss://api.refract.software/v1/transcribe` | — | always-present emergency path |
