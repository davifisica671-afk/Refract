...
| `electron/audio/RefractProSTT.ts` | **MODIFIED (additive).** New deps/flags/telemetry injection (`RefractProSTTDeps`), relay pre-flight seam in `connect()`, `buildAuthFrame(url)`, fallback-ladder helpers (`maybeAdvanceTarget`/`forceAdvanceTarget`/`installTarget`/`connectUrl`/`kindForUrl`/`isOnRelayTarget`), telemetry, relay-state reset in `start()`/`stop()`. Reconnect machine NOT rewritten. |
...
| `electron/main.ts` | **MODIFIED (minimal, ~line 1508).** Passes `appVersion` + `platform` into the `RefractProSTT` constructor; the class reads flags from `SettingsManager` itself and derives the control-plane base URL. |
...
`controlPlaneBaseUrl` defaults to `https://api.refract.software` (derived from the wss host of `BACKEND_URL`). `content-type: application/json`. Body:

```json
{
  "key": "refract_sk_…",            // OR "trial_token": "refract_trial_…" (never both)
...
- `connectUrl()` returns the hardcoded `BACKEND_URL = wss://api.refract.software/v1/transcribe`.
