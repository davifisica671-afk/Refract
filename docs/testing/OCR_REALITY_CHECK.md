...
|---------|----------------|-------------|
| `ocr_simple_text.png` | `Hello`, `Refract`, `Screen`, `understanding`, `works` | ~255 ms |
...
Partial. `RefractInterface.tsx` has loading states for "Capturing screen /
...
4. **No real Electron Playwright test for the screen-understanding flow.**
   ...
   (`refract-api/tests/screen-understanding-live.e2e.mjs`) that hits the live
   Refract API, not the Electron app. Phase 8 — pending.
