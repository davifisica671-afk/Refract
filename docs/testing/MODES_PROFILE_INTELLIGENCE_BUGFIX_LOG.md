...
- **Commands run:**
  ```
  node --test electron/services/__tests__/Mode*.test.mjs electron/services/__tests__/ProfileIntelligenceGate.test.mjs electron/services/__tests__/RefractApiE2E.test.mjs
  npm test
  ```
...
### FINDING-011 — Real Refract API smoke test hits an unresolved domain with the wrong auth header

- **Mode affected:** none at runtime (test-only). Production code uses the correct endpoint.
- **Scenario / user affected:** Anyone running `RUN_REFRACT_API_E2E=1 npm run test:modes` expecting the real API smoke test to validate connectivity.
- **Before behavior:** `valid auth — health endpoint responds` failed with `Expected 2xx or 404 for /v1/health; got 0 (fetch failed)`. The test hit `https://api.refract.app/v1/health`, which is `NXDOMAIN`. It also sent `Authorization: Bearer <key>`. The earlier live-response eval succeeded because it goes through `LLMHelper` which uses the correct endpoint and header.
- **Expected behavior:** The test must hit the same base URL `LLMHelper.ts:1709` uses (`https://api.refract.software`) and send the same auth header style (`x-refract-key: <key>`). A 404 from `/v1/health` is an accepted outcome because the route is intentionally absent — the goal is connectivity + auth, not health.
...
- **Root cause:** Drift between the test fixture defaults (`api.refract.app`, `Authorization: Bearer`) and the production base URL/headers (`api.refract.software`, `x-refract-key`) defined in `electron/LLMHelper.ts:1709-1720`.
- **Files changed:** `electron/services/__tests__/RefractApiE2E.test.mjs` — corrected `API_BASE` default and `authHeader()` to use `x-refract-key` for `REFRACT_API_KEY`.
- **Fix applied:**
  ```diff
  - const API_BASE = process.env.REFRACT_API_BASE ?? 'https://api.refract.app';
...
- **Commands run:**
  ```
  REFRACT_API_KEY=<redacted> RUN_REFRACT_API_E2E=1 node --test electron/services/__tests__/RefractApiE2E.test.mjs
  ```
- **After behavior:** 3/3 tests pass; `/v1/health` returns 404 (intentional) within ~1.7s and invalid-auth probe does not return 200.
- **Remaining risk:** If the base URL or auth header changes again, this test will drift again. Consider importing the constants from `LLMHelper` instead of duplicating them in the test, or adding a lint rule that flags `api.refract.app` in the repo.
...
### FINDING-012 — Refract API LLM emits buggy two-sum (`complement = target, num` tuple) intermittently

- **Mode affected:** technical-interview (and any custom mode that surfaces a two-sum-style problem through the Refract API).
...
- **Commands run:**
  ```
  REFRACT_API_KEY=<redacted> REFRACT_LIVE_LLM_TESTS=1 REFRACT_EVAL_SUITE=baseline npx tsx electron/test/modes-live-response-eval.ts
  ```
