...
```
                           ...
                           │  us-relay.refract.software   A → <US VPS IP>  │
                           │  asia-relay.refract.software A → <ASIA VPS IP>│
                           └───────────────┬───────────────┬──────────────┘
...
| `PUBLIC_RELAY_URL` | `''` | advisory; e.g. `wss://us-relay.refract.software/v1/transcribe`. |
...
`wss://us-relay.refract.software/v1/transcribe`.
...
- `REGION=asia`, `RELAY_ID=asia-1`, `PUBLIC_RELAY_URL=wss://asia-relay.refract.software/v1/transcribe` (the template writes these for you).
- `CADDY_SITE=asia-relay.refract.software` (drives the cert + vhost).
...
Create two records in the `refract.software` zone:
...
sudo certbot --nginx -d us-relay.refract.software --redirect -m ops@refract.software --agree-tos
...
RELAY_URL=https://us-relay.refract.software bash deploy/healthcheck.sh
...
| Names | `us-relay.refract.software`, `asia-relay.refract.software` | match `CADDY_SITE` / certbot `-d` / the control plane's `STT_RELAY_US_URL`/`STT_RELAY_ASIA_URL`. |

After creating records, confirm they resolve to the VPS (not a CF proxy IP):

```bash
dig +short A us-relay.refract.software      # → the VPS IPv4, NOT 104.x/172.x CF ranges
dig +short A asia-relay.refract.software
```
...
1. **Instant kill switch (control plane, seconds, no deploy)** — the primary
   ...
   ```bash
   curl -X POST https://api.refract.software/admin/stt-relays/control \
...
```bash
curl -fsS https://us-relay.refract.software/metrics | python3 -m json.tool
...
TOKEN_JSON=$(curl -fsS -X POST https://api.refract.software/v1/stt/session \
