# Refract Desktop ↔ API Contract Audit (Phase 0)

**Date:** 2026-06-01
**Scope:** How the desktop app talks to `api.refract.software`, with focus on the
...
|---|---|---|---|
| Paid API key | `x-refract-key` | `validateKey()` | 30s cache; checks active/suspended; atomic quota (migration 001) |
...
- `REFRACT_EMBED_PRIMARY` / `REFRACT_EMBED_FALLBACK` env vars pin model names without a redeploy.
...
- `REFRACT_GEMINI_EMBED_MODEL` / `REFRACT_GEMINI_EMBED_DIMS` env vars pin the model/dims
