...
4. **Client latency probes** — `electron/audio/relaySession.ts` + `RefractProSTT.ts` wiring.
...
- `getRelayLatencyProbes()` is called synchronously in the `resolveRelaySession` options literal
  (`RefractProSTT.ts:843`). It **cannot throw synchronously**: it reads `_probeCache`, optionally kicks a
...
- Hardcoded hosts `us-relay.refract.software` / `asia-relay.refract.software` + `/healthz` **match
  `docs/10-deploy-regional-relays.md`** (line 174: `curl https://us-relay.refract.software/healthz`).
...
`relaySession.ts` `RELAY_HEALTH_URLS` hardcodes `us-/asia-relay.refract.software`. If a deployment
