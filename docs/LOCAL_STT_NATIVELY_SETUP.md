# Installing This Local-STT Refract Build

This guide is for running the Refract source tree that includes the new `Local STT` provider. Your already-installed Refract app will not have this feature until you build and run this modified project or package a new installer from it.
...
## 4. Run Refract in Development Mode
...
## 5. Configure Local STT in Refract
...
When `Test Connection` says connected, start a meeting or audio capture flow. Refract opens separate local STT streams for system audio and microphone, so WhisperLive must allow at least two clients.
...
- Refract does not start, install, or supervise WhisperLive. WhisperLive must be running before you use Local STT.
