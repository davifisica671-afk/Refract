# macOS Production Readiness Audit — Refract

**Status:** LIVING DOCUMENT — updated after every phase.
**Started:** 2026-05-29
**Owner of last edit:** Claude Code (autopilot)
**App version:** 2.6.0
**Goal:** Make Refract behave like a real, Developer ID-signed + notarized production macOS desktop app — not a hacked ad-hoc local build. Cover signing/notarization, permissions/TCC, overlay/visibility toggle reliability, audio capture stability, premium/Refract-API gating, startup performance, and UX reliability.
...
### P0 — Premium & Refract API (money path)
| File | Role | Why P0 |
|------|------|--------|
...
| `src/components/settings/RefractApiSettings.tsx` | Refract API key UI | Key entry/validation/redaction. |
| `src/components/settings/RefractProSettings.tsx` | Pro plan UI | Plan separation, upgrade copy. |
| `electron/audio/RefractProSTT.ts` | Pro-tier hosted STT | Gated capability; failure messaging. |
...
| `refract-api/server.js` + `refract-api/lib/*` | Backend license/activation | Source of truth for plan→feature mapping (read-only reference). |
...
| `src/components/ui/TopPill.tsx`, `src/components/RefractInterface.tsx` | Renderer toggle UI | Renderer state vs main desync. |
...
| `electron/audio/{Deepgram,OpenAI,Soniox,Google,Rest,RefractPro}*STT.ts` | STT providers start/stop |
...
| S6 | **`--deep` signing** | `codesign --force --deep` | `--deep` is deprecated by Apple and notoriously breaks nested signatures; correct approach signs inside-out (electron-builder does this refract when given an identity). | ⚠️ |
...
### 2.2 Premium / Refract API — STRUCTURE MAPPED (audit pending Phase 2)
...
- `refract-api/` is the authoritative backend (license check, activation, webhooks). Treat as read-only reference.
...
| 2 | Premium + Refract API gating | ✅ F4 fixed + tested (money path otherwise sound; F5/F7 UX follow-ups) | 2026-05-29 |
