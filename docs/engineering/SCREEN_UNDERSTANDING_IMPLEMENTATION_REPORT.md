...
Refract now has a production-ready screen understanding pipeline with:
...
### Issue
The original `validateImagePath` blocked `/Users/` paths before checking userData allowlist, making it reject valid macOS screenshot paths like `/Users/evin/Library/Application Support/Refract/screenshots/abc.png`.
...
- Screen context status chip (lines 3101-3104 in RefractInterface.tsx)
...
3. **No UI consumer** for `provenance` / `ocrRan` / `visionRequested`. The
   service exposes everything; no `RefractInterface` chip or answer pill
