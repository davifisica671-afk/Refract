...
This document outlines the threat model, mitigation strategies, and multi-layer defense architecture implemented to safely capture, process, and ingest active browser tab DOM context in Refract.
...
### Setter API Protocol
The Refract companion browser extension asynchronously reads DOM structures from the active browser tab and populates the typed buffer:
