...
> - Notarization uses an **App Store Connect API key** (`APPLE_API_KEY`/`_KEY_ID`/`_ISSUER`), because the `refract-notary` keychain profile auto-locks mid-build.
...
- **Production build** (`npm run app:build:signed` / `npm run dist:signed`): runs `electron-builder --config electron-builder.signed.cjs`. That config spreads the package.json `build` block and overrides only mac signing keys: `hardenedRuntime: true`, `entitlements: assets/entitlements.mac.plist`, `entitlementsInherit: assets/entitlements.mac.inherit.plist` (minimal helper entitlements), `gatekeeperAssess: false`, `notarize: false`, `identity` from `REFRACT_SIGN_IDENTITY`/`CSC_NAME` (or auto-discovered from `CSC_LINK`/keychain). It sets `REFRACT_PRODUCTION_SIGN=1` so `ad-hoc-sign.js` stands down (never clobbers the Developer ID signature) and wires `afterSign: scripts/notarize.js` to notarize + staple.
...
```bash
APP="release/mac-arm64/Refract.app"   # adjust arch/path
...
codesign --verify --verbose=2 "release/Refract-2.6.0-arm64.dmg"
xcrun stapler validate "release/Refract-2.6.0-arm64.dmg"
