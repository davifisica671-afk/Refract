...
| `electron/LLMHelper.ts:1198-2526` | Per-provider multimodal dispatch (Gemini, OpenAI, Claude, Groq, Ollama, Codex CLI, Refract, custom cURL) |
...
src/hooks/useShortcuts.ts:65 ⌘+H
  → src/components/RefractInterface.tsx:2575 isShortcutPressed('takeScreenshot')
    → handlers.takeScreenshot()  (RefractInterface.tsx:2493)
      → window.electronAPI.takeScreenshot()       (preload.ts:397)
        → ipcMain.invoke "take-screenshot"        (ipcHandlers.ts:266)
          ...
          ← { path, preview }
        ← { path, preview }
      → setAttachedContext([...prev, {path, preview}])  (RefractInterface.tsx:1001)
```
...
src/hooks/useShortcuts.ts                          ⌘+Shift+H
  → handlers.takeSelectiveScreenshot()             (RefractInterface.tsx:2505)
...
global shortcut 'general:capture-and-process'      (main.ts:428)
  ...
  → RefractInterface handler [implicit via attachment + manual submit flow]
    ... still uses the same Path E to actually run OCR+LLM.
```

The handler logic in `RefractInterface.tsx` lines 2003 and 2037 sends the
...
```
RefractInterface.tsx:1499  handleWhatToSay()
...
```
RefractInterface.tsx:1644  handleCodeHint()
...
```
streamWithRefract       → multipart with base64-PNG (Sharp resized to ≤768px)
  ...
  → user accepts the chip
    → window.electronAPI.acceptDynamicAction(...)
      → 'dynamic-action-accepted' IPC
        → handleWhatToSay(promptInstruction)       (RefractInterface.tsx:1499)
