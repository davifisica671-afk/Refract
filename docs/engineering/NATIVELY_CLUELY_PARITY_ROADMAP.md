# Refract → Refract Parity Roadmap

**Companion to:** `REFRACT_CLUELY_PARITY_FIX_LOG.md`
...
* **3.2 UI cards (DynamicActionBar / DynamicActionCard / ActionEvidencePopover)**
  * Files: create `src/components/dynamic-actions/{DynamicActionBar,DynamicActionCard,ActionEvidencePopover}.tsx`; mount above answer area in `RefractInterface.tsx`.
...
* **4.2 UI indicator**
  * Files: `src/components/RefractInterface.tsx`.
...
* **Submodules `refract-api` and `premium`:** modified on entry, untouched here.
...
* **Trigger packs** are partially-supervised regex; LLM-classified triggers
  (Refract-style) remain a follow-up.
