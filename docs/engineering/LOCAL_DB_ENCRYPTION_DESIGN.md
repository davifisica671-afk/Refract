...
This is the design document for at-rest encryption of Refract's local SQLite
...
2. Store it in keytar under service `refract`, account `db-master-key`.
...
1. On first launch with an existing plaintext DB:
   1. Export with `VACUUM INTO 'refract-encrypted.sqlite'` after attaching with
      the key.
