...
6. **Vision providers exist.** Gemini, OpenAI, Claude, Groq Llama-4-Scout,
   Refract, Codex CLI, Ollama vision families, and custom cURL all have
...
8. **Permission UX is partial but exists.** Screen Recording denied surfaces
   ...
   (`RefractInterface.tsx:3111-3143`).
...
9. **No image size cap for cloud providers other than Refract.** Still pending —
   Sharp resize needs to be applied for all providers.

---

## What is missing for Refract-level screen analysis
...
1. **A reliable "Use current screen" button.** Single top-level action that
   ...
   live scene. Refract's most distinctive interaction.
...
A sequenced plan to reach Refract parity. Each item lists the files to touch,
...
Files to edit:
- `src/components/RefractInterface.tsx:3184-3187` — when the chip type is
...
Files to add / edit:
- `src/components/RefractInterface.tsx` — new pill in the action row that
...
- `src/components/RefractInterface.tsx` — render the
...
- `electron/llm/__tests__/ProviderImagePayload.test.mjs` — for each of
  Gemini / OpenAI / Claude / Groq / Ollama / Refract / custom cURL, build
...
1. **Is Refract currently doing OCR?**
...
2. **Is Refract currently using vision LLMs for OCR?**
...
3. **Is Refract sending screenshots directly to LLMs?**
   **Yes.** When the multimodal path runs, the raw PNG bytes go to Gemini /
   OpenAI / Claude / Groq Scout / Ollama / cURL (resized for Refract
   only). The OCR text rides alongside in the prompt, not as a substitute.

4. **Is Refract screen analysis Refract-level?**
...
5. **What is the fastest path to Refract-level?**
   ...
   becomes a Refract-equivalent surface. Everything past Step 3 deepens the
...
const userData = '/Users/evin/Library/Application Support/Refract';
console.log(validateImagePath('/Users/evin/Library/Application Support/Refract/screenshots/abc.png', userData));
"
