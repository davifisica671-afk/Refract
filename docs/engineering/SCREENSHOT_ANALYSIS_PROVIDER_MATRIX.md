...
|---|---|---|---|---|---|---|---|
| **Refract API** | yes | multipart/form-data; image first resized with Sharp to ≤768 px on long edge, base64 encoded | no — only forwards | yes (Refract backend treats as multimodal) | server-side fallback | medium (cloud) | `generateWithRefract` `LLMHelper.ts:1698`; `streamWithRefract` |
...
```
refract → codex → openai → gemini_flash → claude → gemini_pro → groq
```
(`ProviderRouter.ts:149-151`).

Provider order (text-only request):
```
refract → groq → codex → gemini_flash → gemini_pro → openai → claude
```
...
- Scope policy: `assertOutboundScopes('refract', text, imagePaths)` runs at
...
- **No data-scope assertion** runs for this path
  (compare to `LLMHelper.ts:1699` for Refract). A screenshot can leave the
...
the default in the multimodal stream list (after Refract/Codex/OpenAI),
...
layer still applies because the Refract path is short-circuited for
