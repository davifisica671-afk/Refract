...
- `refract-api/services/stt-relay/src/telemetry.js` (Axiom + Sentry-lite + PostHog shippers)
- `refract-api/server.js` `tgAlert` (control-plane Telegram alerting) + Phase-4 relay health tracker
...
Owned by `electron/audio/RefractProSTT.ts` (docs/01 §5.5, §12), distinct id = hashed identity, no transcript text / no raw keys / Sentry breadcrumbs on ladder transitions:
