...
sequenceDiagram
    autonumber
    participant App as Electron App<br/>(RefractProSTT ×2: system, mic)
    participant CP as Railway Control Plane<br/>(server.js, unchanged routes + new /v1/stt/*)
    participant R1 as us-relay.refract.software<br/>(VPS, Caddy + relay)
    participant R2 as asia-relay.refract.software<br/>(VPS, Caddy + relay)
...
flowchart LR
    subgraph Client["Electron Desktop"]
        STT1["RefractProSTT('system')"]
        STT2["RefractProSTT('mic')"]
    end
    ...
    subgraph US["us-relay.refract.software (Hetzner US VPS)"]
        C1[Caddy TLS] --> RA["stt-relay process<br/>(packages/stt-relay-core)"]
    end
    subgraph ASIA["asia-relay.refract.software (SG/IN VPS)"]
...
```
┌──────────────────────────── Electron Desktop ────────────────────────────┐
│  RefractProSTT('system') ─┐         feature flags:                      │
│  RefractProSTT('mic')   ──┤         regional_stt_relay_enabled/percent  │
└────────────────────────────┼──────────────────────────────────────────────┘
        ...
        └───────┬───────────────────────────────────────────────────────────────┘
                ...
                │                 relay_url: wss://us-relay.refract.software/ws,
                │                 fallback_chain: [asia-relay, railway] }
                ▼
   ┌─ us-relay.refract.software ─┐      ┌─ asia-relay.refract.software ─┐
...
1. **Same relay retry** — client reconnect/backoff against the relay it was assigned (existing `RefractProSTT` backoff: base 1500ms, cap 30s, jitter ±20%). Token may have expired → step 2.
...
```json
{
  "key": "refract_sk_…",            // OR "trial_token": "refract_trial_…"
...
```json
{
  ...
  "fallback_chain": [
    { "mode": "relay", "relay_url": "wss://asia-relay.refract.software/ws", "region": "asia" },
    { "mode": "railway", "relay_url": "wss://api.refract.software/v1/transcribe" }
  ],
...
## 5. Client changes — `electron/audio/RefractProSTT.ts`

Minimal, flag-gated; the legacy direct path is preserved untouched as both the flag-off path and ladder step 3.

1. **Session create**: when `regional_stt_relay_enabled`, before opening WS, `POST https://api.refract.software/v1/stt/session` with key/trial token + channel + optional latency probes. Response cached per channel until `expires_at`.
...
## 6. Supabase schema (new migration `refract-api/migrations/003_stt_relay_sessions.sql`)
...
| DNS | Cloudflare, **grey-cloud (DNS-only)** A records `us-relay.refract.software`, `asia-relay.refract.software` | **Justification:** orange-cloud proxying would route every PCM byte through Cloudflare — re-introducing exactly the middleman hop we are removing, adding latency, putting WS audio under CF's proxy limits, and (on paid LB/Argo paths) re-creating per-GB cost. Health-based DNS LB can come later; for now the control plane's health cache + the client ladder ARE the load balancer, with finer control than DNS TTLs allow |
| Process mgmt | systemd unit per relay | `Restart=on-failure`, `TimeoutStopSec=30` (graceful drain §3.5), `EnvironmentFile=/etc/refract/stt-relay.env` (0600) |
