...
- [ ] **5) Set the Railway CP env** (Railway dashboard) — keep **percent 0**:
      ...
      `STT_RELAY_ASIA_URL=wss://asia-relay.refract.software/v1/transcribe`, **`STT_RELAY_ENABLE_PERCENT=0`**,
      ...
      `wss://api.refract.software/v1/transcribe`). Redeploy the CP.
- [ ] **6) Verify both relays show healthy** — `curl -fsS https://api.refract.software/admin/stt-relays -H "x-admin-secret: $ADMIN_SECRET" | python3 -m json.tool` → both regions `healthy:true`,
...
- [ ] **TLS via Caddy** — Caddy auto-issues Let's Encrypt certs on first request once DNS points at the box
      and 80/443 are open. (Nginx alternative: `certbot --nginx -d us-relay.refract.software`.)
- [ ] **Verify resolution + TLS**:
      `dig +short A us-relay.refract.software` (→ the VPS IP, **not** a 104.x/172.x CF range) and
      `curl -fsS https://us-relay.refract.software/healthz` (and asia). Confirm 8080 is NOT public:
...
| `STT_RELAY_US_URL` | `wss://us-relay.refract.software/v1/transcribe` | US relay endpoint |
| `STT_RELAY_ASIA_URL` | `wss://asia-relay.refract.software/v1/transcribe` | Asia relay endpoint |
| `STT_RELAY_RAILWAY_FALLBACK_URL` | `wss://api.refract.software/v1/transcribe` (default) | always-present emergency path |
