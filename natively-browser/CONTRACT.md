...
| `409 {"error":"no_active_session"}` | Refract is running and paired, but there is no active session/overlay to receive the context. The DOM is delivered to the overlay window (the only one that mounts `RefractInterface`); when no overlay exists the route returns 409 instead of silently dropping. The extension surfaces "Start a Refract session, then capture again". |
...
- **Origin must EXACTLY equal a pinned extension ID** (not the structural `[a-p]{32}`
  ...
  Plus an optional `REFRACT_DOM_EXTENSION_ID` override. A web page cannot forge a
...
`/dom`. This is how a **Refract global hotkey** triggers capture from any focused app
...
- **Phone Mirror must be RUNNING.** `/dom` is a path on PhoneMirrorService's HTTP
  ...
  The extension surfaces this as "Open Refract and enable Phone Mirror".
- **An active Refract session (overlay) must exist.** The DOM is delivered to the
