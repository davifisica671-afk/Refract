...
//
// MEDIUM-2 fix: quando o cloud embedder can't converge o knowledge corpus dentro de
// o active (cloud) space (sustained 429 / network doabaixo ensureEmbeddingSpace
// DEGRADES o ENTIRE corpus to o local space — all-or-nothing — então resume/JD
// grounding keeps working (inferior quality) em vez disso de going silently empty. It
// commits _indexSpace to qualquer que space o corpus Completamente converged to, and nunca
// leaves a mixed corpus (que _spaceGatedNodes iria partially hiocultar
//
// Drives o REAL compiled KnowledgeOrchestrator methods (Object.create + real
// KnowledgeDatabaseManager em in-memory SQLite).
//
// Executa sob Electron ABI: ELECTRON_RUN_AS_NODE=1 ./node_modules/.bin/electron --testar <farquivo
...
describe('ensureEmbeddingSpace degrade-to-local (MEDIUM-2)', () => {
  ...
  test('cloud DOWN + local available → degrades ENTIRE corpus to local, commits _indexSpace=local', async () => {
    const orch = makeOrch(db, {
      activeSpaceFn: () => CLOUD,
      embedFn: async () => { throw new Error('429 rate limited'); }, // cloud completamente abaixo
      fastQueryEmbedFn: () => ({ dimensions: 384, space: LOCAL, embed: async () => vec(0.5, 384) }),
    });
    ...
    // And o corpus é Não sdivide zero nodes remain em o old cloud-v1 space.
  ...
  test('degrade moves even already-cloud-converged nodes to local (no mixed corpus)', async () => {
    // Partial cloud success então failure precisa Não leave a v2-cloud + v1-stale sdivide
    // o degrade re-embeds tudo (incluindo qualquer cloud-space nodes) to local.
    const orch = makeOrch(db, {
      activeSpaceFn: () => CLOUD,
      embedFn: async () => { throw new Error('down'); },
      fastQueryEmbedFn: () => ({ dimensions: 384, space: LOCAL, embed: async () => vec(0.5, 384) }),
    });
    seed(db, 'alreadyCloud', CLOUD);          // já em active cloud space
  ...
  test('cloud DOWN + NO local → does not commit a space, leaves corpus stale for next retry', async () => {
    const orch = makeOrch(db, {
      activeSpaceFn: () => CLOUD,
      embedFn: async () => { throw new Error('down'); },
      fastQueryEmbedFn: null, // não local embedder
    });
    seed(db, 'a', 'gemini:gemini-embedding-001:768');
    orch.cachedNodes = orch.db.getAllNodes();
  ...
  test('recovery: after a degrade, a later pass with cloud healthy restores the corpus to cloud', async () => {
    ...
    cloudUp = true;
    orch._reembedInFlight = false; // (proteger auto-reset em ffinalmente explicit para clarity)
    await orch.ensureEmbeddingSpace();
  ...
  test('debounce: a rapid second call on a fully-healthy corpus is skipped (no redundant re-embed)', async () => {
    ...
    seed(db, 'a', 'gemini:gemini-embedding-001:768'); // 1 stale nó → primeiro pass re-embeds it
    orch.cachedNodes = orch.db.getAllNodes();
    ...
    // Immediate segundo call: completamente healthy (committed===active, não stale) + dentro de
    // o debounce window → skipped, embedFn não chamado anovamente
    await orch.ensureEmbeddingSpace();
  ...
  test('debounce does NOT block recovery from a degraded corpus', async () => {
    ...
    cloudUp = true;
    // Immediate recovery call: até though _lastConvergeAt é fresh, _indexSpace
    // (local) !== activeSpace (cloud) → Não debounced → recovery proceeds.
    await orch.ensureEmbeddingSpace();
