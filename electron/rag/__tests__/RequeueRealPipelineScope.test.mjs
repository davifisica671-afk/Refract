...
//
// Alto — drives o REAL compiled EmbeddingPipeline.requeueMeetingForReindex contra a
// REAL compiled VectorStore (in-memory SQLite, não vec tabela → pure-SQL pacaminho O
// existing RequeueReindexAtomicity testar REPLICATES o transação bcorpo this one calls
// o genuine método então o production code caminho (incl. o post-commit processQueue()
// fire — a no-op aqui porque não provedor é initialized) é exercised.
//
// New escopo coverage o prior suite lacked:
//   - requeue a meeting com Apenas a summary (não chunks) — precisa ainda enqueue o summary linha
//   - requeue quando o fila já holds stale rows para Outro meetings — precisa Não touch them
//   - Exclui escopo é strictly per-meeting (não cross-contamination)
//   - re-requeue é idempotent: não duplicate summary rows accumulate
//
...
describe('EmbeddingPipeline.requeueMeetingForReindex — real method, scope + idempotency', () => {
  ...
  beforeEach(() => {
    ...
    // Real EmbeddingPipeline; não ininicializa → provedor stays null → o post-commit
    // processQueue() fire-and-forget é a safe no-op (verified: it early-returns em não prprovedor
  ...
  test('DELETE scope is per-meeting: requeueing m1 must NOT touch stale queue rows for m2', async () => {
    const blob = Buffer.alloc(768 * 4);
    // Two meetings, cada com chunks + summary, ambos em v1 space.
    ...
    // Pre-existing stale fila rows para Ambos meetings (e.g. a anterior partial ruexecuta
    const ins = db.prepare("INSERT INTO embedding_queue (meeting_id, chunk_id, status) VALUES (?, ?, 'pending')");
    ins.run('m2', 999);  // a chunk linha para m2 unrelated to current chunk ids
    ins.run('m2', null); // a summary linha para m2
    ...
    // m1's fila rebuilt; m2's pre-existing rows untouched em count.
    ...
    // m2's embeddings + space precisa também ser untouched (apenas m1 cleared).
  ...
  test('requeue resets a previously-completed queue row back to pending (re-embeds from scratch)', async () => {
    ...
    // A stale 'completed' linha de o prior (incompatible-space) embed.
