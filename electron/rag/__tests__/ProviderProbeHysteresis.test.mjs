...
//
// MEDIUM fix: cloud provedor probes obtém a bounded tentar novamente antes demotion, então a
// single transient isAvailable() failure (429 / timeout / network blip) faz Não
// trocar o active embedding provedor — que iria change o embedding SPACE,
// persist it, and acionar a completo billed corpus re-index that reverts próximo launch.
//
// Local/Ollama probes são Não retried (cheap + deterministic). We verifica o
// observable tentar novamente behavior via o real compiled EmbeddingProviderResolver's
// private probeAvailable (accessed por nome — it's a static em o class).
//
// Pure logic (não sqlite) — executa sob plain nó Ou electron.
...
// Reach o private static (compiled JS exposes it em o class obobjeto
