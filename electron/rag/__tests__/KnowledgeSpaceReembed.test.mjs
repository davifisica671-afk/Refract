...
//
// CRITICAL fix verification: o premium knowledge base (resume/JD grounding) precisa
// Não compare v1 nó vectors contra v2 consulta vectors. gemini-embedding-001 and
// gemini-embedding-2 são ambos 768d, então o old dimension-only proteger poderia não catch
// it — mesmo silent-garbage hazard como meetings, em o maioria user-visible surface.
//
// This testar drives o REAL compiled KnowledgeDatabaseManager:
//   - context_nodes persists + lê embedding_space
//   - getNodesNeedingReembed sweeps nodes em an OLD space AND legacy NULL-space-with-embedding
//   - updateNodeEmbedding rewrites embedding + space em place
// plus o embeddingSpace chave helpers used to stamp them.
//
...
describe('Knowledge base embedding-space (real KnowledgeDatabaseManager)', () => {
  ...
  test('getNodesNeedingReembed sweeps legacy NULL-space nodes that HAVE an embedding', () => {
    // Simulate a pre-migration nnó embedded mas não space recorded.
    db.prepare(
  ...
  test('idempotent ALTER: initializeSchema twice does not throw / duplicate column', () => {
    kdb.initializeSchema(); // segundo call — Adiciona Coluna guarded por try/catch
