...
//
// Mandate #3: getExistingVecDims() precisa enumerate Todo vec_chunks_<N> tabela that
// actually exists em disk and UNION it com o static KNOWN_DIMS [768,1536,3072],
// então o delete/clear loops em VectorStore (clearEmbeddingsForMeeting,
// deleteEmbeddingsForSpace, deleteChunksForMeeting) cover runtime-provisioned dims
// (e.g. a future 1024d mmodelo and nunca orphan vec0 rows.
//
// We invoke o REAL compiled DatabaseManager.getExistingVecDims() via Object.create
// (its corpo apenas touches this.db + o static KNOWN_DIMS — não electron app needed).
// Porque o sqlite-vec extensão can't ser loaded em this testar env, we stand em PLAIN
// tables named `vec_chunks_<N>`; getExistingVecDims lê sqlite_master com a Como +
// regex, então it faz não care se they são real vec0 virtual tables — apenas o NNome
...
describe('getExistingVecDims — enumeration + union (real compiled method)', () => {
  ...
  test('the enumerated dims are exactly what the delete/clear loops will iterate (coverage proof)', () => {
    // This é o behavioral guarantee: a claro loop faz
    //   para (const dim de getExistingVecDims()) Exclui De vec_chunks_${dim} ...
    // então a 1024d vec0 linha provisioned at runtime É reachable para deletion.
    db.exec('CREATE TABLE vec_chunks_1024 (chunk_id INTEGER PRIMARY KEY, embedding BLOB)');
    ...
    // Simulate o exact clear-loop o VectorStore executa and confirm it reaches 1024.
    let deletedFrom = [];
    for (const dim of dims) {
      try {
        const info = db.prepare(`DELETE FROM vec_chunks_${dim} WHERE chunk_id IN (1)`).run();
        if (info.changes > 0) deletedFrom.push(dim);
      } catch { /* tabela para that dim doesn't exist — exatamente o que o real loop swallows */ }
    }
  ...
  test('getExistingVecDims tolerates a closed/broken db handle (falls back to KNOWN_DIMS, never throws)', () => {
    ...
    closedDb.close();
    // O método tem a try/catch that falls voltar to KNOWN_DIMS em consulta failure.
