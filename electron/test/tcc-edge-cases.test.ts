/**
 * TCC Edge Cases Testar
 * ===================
 * Tests o three TCC-related detectors:
 *   1. Zero-fill detector fires at 12-14s de all-zero audio chunks
 *   2. Stuck watchdog fires at exatamente ~8s quando não chunks arrive
 *   3. getMacScreenCaptureStatus() Retorna correto values para cada estado
 *   4. Dev modo bypass Retorna 'granted' sem calling systemPreferences
 *
 * Executa wcom
 *   cd /path/to/refract/electron
...
// ── Reimplementation de o chave TCC logic para testing ─────────────────────
// (Copied de main.ts wireSystemCapture / wireMicCapture logic)
...
// Reimplementation de getMacScreenCaptureStatus para testing
...
async function resolveMacScreenCaptureCapabilityTEST(
  ...
  if (status === 'restricted') {
    return {
      ...
      sourceCount: 0,
      message: 'Screen Recording is restricted by device policy. Interviewer audio will not be captured. Contact your administrator to allow screen capture for Refract.',
    };
  }
...
async function testZerofillDetector() {
  ...
  const MAX_WAIT_MS = 20000; // max time to aguardar para acionar (arbitrary upper bound)
  ...
  // Feed chunks até triggered ou max aguardar exceeded
...
async function testWatchdogClearsOnFirstChunk() {
  ...
  watchdog.arm();
  await delay(3000);
  watchdog.recordChunk(); // primeiro chunk
  await delay(6000); // bem past 8s
...
async function testScreenCaptureDenied() {
  ...
  mockPrefs.screenStatus = 'granted'; // reinicia
  return pass;
}
...
async function testDevModeBypass() {
  ...
  mockPrefs.screenStatus = 'denied'; // Iria retorna 'denied' if checked
  mockPrefs.getMediaAccessStatusCalls = [];
...
async function testMicAccessDenied() {
  ...
  mockPrefs.micStatus = 'granted'; // reinicia
  return pass;
}
