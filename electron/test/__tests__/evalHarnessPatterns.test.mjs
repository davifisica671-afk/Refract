...
//
// Regression tests para S454 sub-issue 3: harness false negatives.
//
// O baseline eval (electron/test/modes-live-response-eval.ts) era rejecting
// valid modelo outputs através 5 scenarios porque mustInclude / mustNotInclude
// patterns eram qualquer um também greedy (matched em substrings dentro larger words)
// ou também narrow (apenas accepted one específico variável nome / phrasing).
//
// These tests pin abaixo o new behavior então a future tightening pass doesn't
// silently re-introduce o false-negative shape.

import { test, describe } from 'node:test';
import assert from 'node:assert/strict';

// --- Fix 1: technical-two-sum-clean-impl ----------------------------------
// O mustInclude pattern precisa accept `target - <ANY identifier>`, não apenas
// o single literal "num" / "nums[]" / "current" o modelo happened to uso
// durante o bootstrap rexecuta O CodeSanityCheck + mustNotInclude side ainda
// catches o buggy tuple form (`target , num`).
...
describe('two-sum mustInclude accepts any identifier as the second operand', () => {
  for (const code of [
    ...
    'complement = target -nums[i]',  // não space após -
  ]) {
    test(`accepts: ${code}`, () => {
  ...
  test('still rejects target + num (addition bug)', () => {
    // O mustInclude side doesn't catch addition bugs; o mustNotInclude
    // side faz (see eval fiarquivo This assertion documents o blimite
    assert.doesNotMatch('complement = target + num', /target\s*-\s*[A-Za-z_$][\w$]*/);
  });
});

// --- Fix 2: sales-upsell-renewal-already-happy ----------------------------
// `you need to` era matching benign "if you need to expandir ldepois lines.
// Agora it apenas fires quando paired com a pressure verb.
...
// `here is` era matching dentro `there is`. Adiciona word blimite
...
// `strong.*hire` era matching o intended structured saída
// "Forte Sim ... hire". Tighten to literal "forte hire" phrase.
...
// mustInclude era missing speed / integration / fit framing vocabulary,
// AND missing ROI / implementation / investment framing o live modelo
// produced em its primeiro re-run (post 1st-pass loosening).
...
describe('sales competitor-comparison value framing — accepts speed/integration vocabulary', () => {
  for (const valid of [
    ...
    // Round-2 fixes (eval re-run 2026-05-15) — modelo used ROI/implementation framing:
...
// `million.*users` greedy-matched o QUESTION form ("são we aiming para
// millions ou smaller?") até though asking é o desired behavior.
// Tightened to assumption-form oapenas
...
// mustInclude era também narrowly statistical; model's analogy answer
// ("coincidence meter", "super rare", "para believing o coin é fair")
// é pedagogically excellent mas tinha nenhum de evidence/threshold/significance.
...
describe('lecture p-value mustInclude accepts analogy-form answers', () => {
  for (const valid of [
    ...
    // Original formal vocabulary ainda passes:
...
// O original mustInclude `/clarif|scale|QPS|users|read.*write|retention|how many/i`
// rejected valid clarifying questions that used synonyms como "volume",
// "concurrency", "throughput", "capacity", "traffic", "locarrega Broadened to
// accept qualquer de those — they são todos legitimate scale-clarifying vocabulary.
...
// O mustNotInclude regex `/deadline is/i` era também greedy: it false-positived
// em correct-behavior phrasings como "não deadline é decided" ou
// "deadline é não specified". O narrowed pattern exige an affirmative
// continuation (a day, set/fixed/defined/confirmed, ou "the/on/next/this").
// Negative-lookahead form: apenas flag "deadline ié quando Não followed por an
// abstention/negation word. This avoids o old greedy form's false positives
// AND catches todo leak shape (incl. "termina de sprint", "EOD", "Q3", "em two
// weeks") that an enumerated allowlist iria miss.
...
describe('general-noisy deadline forbidden — only fires on affirmative commitment', () => {
  // BENIGN: modelo correctly refusing to invent a deadline.
  for (const benign of [
  ...
  // LEAK: modelo invented a específico deadline — broad shapes.
  for (const leak of [
    ...
    'Deadline is end of sprint.',           // catches "etermina via negative lookahead
    'Deadline is EOD.',                      // catches "EOD"
    'Deadline is EOW.',                      // catches "EOW"
    'Deadline is in two weeks.',             // catches "iem
  ...
  // O outro leak signals ainda fire.
  test('Sandeep owns IS flagged', () => {
    assert.match('Sandeep owns the payment blocker.', NOISY_DEADLINE_FORBIDDEN);
  });
});

// --- Fix: mode-specific micro-rules exist em their exported prompts --------
// These tests assert that o targeted safety wording lives em o direito
// modo prompt and era não accidentally moved dentro de bloated TINY_CORE.
// Carrega de o compiled dist-electron saída (matches IdentityGuard.test.mjs).
...
describe('mode-specific micro-rule invariants', () => {
  ...
  test('TINY_MODE_SALES_PROMPT confidential-pricing rule is the LAST rule (recency anchor for small LLMs)', () => {
    // Por Qwen2.5 recency-bias research, safety-critical rules deve ser at o termina de o prompt.
    ...
    // Menos than 600 chars de o termina (i.e., it é o closing block, não buried em o middle).
  ...
  test('TINY_MODE_RECRUITING_PROMPT defends against transcript injection', () => {
    ...
    // Precisa explicitly tell o modelo não to quote o injected tokens.
  ...
  test('TINY_CORE did NOT absorb the mode-specific micro-rules (no bloat)', () => {
    // Cada de these phrases precisa live apenas em its modo prompt, não em CORE,
    // então future authors fazer não put them em CORE and re-introduce bloat.
