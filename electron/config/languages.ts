 ...
 * 
 * DESCRIÇÃO:
 * Define TODOS os idiomas suportados para:
 * 1. Reconhecimento de fala (STT — Speech-to-Text):
 *    - O app transcreve áudio em tempo real
 *    - Cada provedor STT suporta diferentes idiomas
 *    - Formatos: BCP47 (ex: pt-BR), ISO 639 (ex: pt)
 * 
 * 2. Geração de respostas de IA:
 *    - O LLM pode gerar respostas em diferentes idiomas
 *    - "auto" detecta o idioma da pergunta automaticamente
 * 
 * IDIOMAS SUPORTADOS (30+):
 * - Inês (5 variantes: US, UK, India, Australia, Canada)
 * - Português, Espanhol, Francês, Alemão, Italiano
 * - Russo, Ucraniano, Polonês, Tcheco, Húngaro
 * - Japonês, Coreano, Chinês (Simplificado)
 * - Árabe, Hebraico, Hindi
 * - Turco, Romeno, Holandês, Grego
 * - Búlgaro, Malaio, Finlandês, Sueco
 * - Norueguês, Dinamarquês, Tailandês, Vietnamita
 * 
 * DETECÇÃO AUTOMÁTICA:
 * Quando "auto" é selecionado, o STT tenta detectar o idioma
 * automaticamente a partir das primeiras palavras faladas.
 * A lista AUTO_DETECT_ALTERNATES define quais idiomas tentar.
 * =============================================================================
 */

export type LanguageOption = {
    label: string;
    code: string; // Chave interna (ex: 'russian')
    bcp47: string; // Para Google, Azure (ex: 'ru-RU')
    iso639: string; // Para OpenAI, Groq (ex: 'ru')
    group: string; // Para agrupamento na interface
    alternates?: string[];
};
