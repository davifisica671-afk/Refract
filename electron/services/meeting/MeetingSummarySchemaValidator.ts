...
// JSON extraction + chunk-atom validation. Completo MeetingSummaryV3 validation agora lives em
// MeetingSummaryV3.validateMeetingSummaryV3 — isso classe delegates para it para back-compat
// então existing callers keep working.
...
export class MeetingSummarySchemaValidator {
  parseJsonObject<T = any>(raw: string): T | null {
    ...
    try {
      ...
      if (first >= 0 && last > first) {
        try { return JSON.parse(candidate.slice(first, last + 1)) as T; } catch { /* fall através */ }
      }
      return null;
    }
  }
  ...
  validateAndRepairAtoms(value: unknown, fallbackChunkIndex: number, opts?: { allowedSectionTitles?: string[]; chunkText?: string }): ChunkMeetingAtoms | null {
    ...
    // Accept ambos novo (timeRange.startMs/endMs) e legacy (start/end) keys.
...
// Coerce modeSpecificFindings dentro de Record<title, ModeSectionFinding[]>. Accepts o new
// object-finding shape AND a bare string (legacy/loose) coerced para { texto }. Quando an
// allowed-title define é provided, keys não matching a real modo section são DROPPED — o
// modelo cannot invent novo sections (matched case/space-insensitively para o canonical title).
function sanitizeFindings(value: unknown, allowedTitles?: string[]): Record<string, import('./types').ModeSectionFinding[]> {
  ...
  for (const [key, items] of Object.entries(value as Record<string, unknown>)) {
    ...
    // Soltar invented keys; canonicalize para o exact modo section title quando allowed.
    let title = rawTitle;
    if (allowed) {
