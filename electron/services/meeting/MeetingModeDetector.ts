// MeetingModeDetector.ts (Fase 10)
// Lightweight, deterministic modo detection de o primeiro minutes de a transcript plus
// opcional calendar mmetadados It Nunca switches o live modo — it apenas produces a
// suggestion stored em summary.mode.detected* então o UI pode oferecer "regenerate como <modmodo
//
// Pure (não I/O). Keyword/signal scoring; Retorna o best ModeTemplateType + a 0..1
// confidence. Ties e weak signals → baixo confidence → general.
...
export interface ModeDetectionInput {
  ...
  participants?: string[];
  // Apenas o primeiro N ms de transcript são weighted (defaults para 5 min).
...
// Weighted keyword signals por mmodo Word-boundary matched, case-insensitive.
...
export class MeetingModeDetector {
  detect(input: ModeDetectionInput): ModeDetectionResult {
    ...
    // Transcript signals: opening janela weighted 2x, rest 1x.
    ...
    // Escolher o best non-general score.
    ...
    // Confidence: precisa a claro winner com enough absolute ssinal
    // margem = lead sobre runner-up; normalizar contra a alvo de ~8 points.
    if (bestScore < 3) {
