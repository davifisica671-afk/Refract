...
test('strategy selector: long_context only when explicitly enabled + allowed + safe', () => {
  ...
  // Também grande para o safe cap → map_reduce até quando enabled.
...
// ── generateStructured ladder (com a fake LLM hauxiliar ───────────────────────
...
// ── Bad JSON de o chunk extractor (validator pcaminho ───────────────────────
...
// ── FollowUp deterministic fallback (não LLM) ─────────────────────────────────
...
test('follow-up generator falls back deterministically and maps mode→type', async () => {
  ...
  // LLM Retorna nada usable → deterministic corpo de decisions/actions.
  const llm = fakeLLM(['']);
...
// ── Long-meeting: não truncation, chunk coverage ──────────────────────────────
...
test('long transcript: chunker covers beginning, middle, end with overlap', () => {
  ...
  // segmentIds preserved por chunk.
  assert.ok(chunks.every(c => Array.isArray(c.segmentIds) && c.segmentIds.length === c.segments.length));
  // Total coverage: todo original segment appears em at menos one chunk.
...
// ── Template-section extraction (Fase 16b: modo template = fonte de truth) ──
...
test('reducer Summary is outcome-first and never boilerplate; empty when no content', () => {
  const reducer = new MeetingSummaryReducer();
  const normalized = new TranscriptNormalizer().normalize([seg('user', 'hello there friend', 0)]);
  // Com a decision, Summary leads com grounded content.
...
test('SectionPromptCompiler returns LLM instruction when it passes guardrails, else fallback', async () => {
  // Good compiled instrução (tem ambos required clauses).
  ...
  // Missing guardrails twice → fall voltar to deterministic.
...
test('chunk extractor prefers compiledPrompt over description in the prompt', () => {
  // White-box: verifica o compiled guidance reaches o section block. We assert via o
  // reducer caminho that a compiled section ainda routes (prompt text é internal), então aqui we
  // apenas confirm MeetingModeSectionInput carries compiledPrompt através o reducer.
  const reducer = new MeetingSummaryReducer();
  const normalized = new TranscriptNormalizer().normalize([seg('user', 'hello there friend', 0)]);
...
test('newSignificantTokens flags fact-shaped tokens not present in the source', () => {
  ...
  // "Mixpanel" (próprio noun não em sfonte and "40%" (número não em sfonte precisa ser flagged.
...
test('newSignificantTokens allows pure rephrasing (no new facts)', () => {
  ...
  // Rephrase using apenas grounded facts (PostHog, Ari, Friday todos present).
...
// ── Provedor diarization (#3): diarized speakerId precedence ─────────────────
...
test('normalizer: provider speakerId wins over channel mapping and sets display name', () => {
  ...
  // Three distinct speakers agora resolvable (diarization divide o remote channel).
