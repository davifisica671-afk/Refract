// MeetingSummaryV3.ts
// Canonical, spec-aligned schema para Refract post-meeting notes (Fase 5).
//
// This é o único fonte de truth para o MeetingSummaryV3 shape e its runtime
// validation/repair. It deliberately uses a hand-written validator (não novo dependency)
// que mirrors o repo's existing defensive-coercion convention. O validator nunca
// throws e sempre Retorna a usable result ou a claro `ok:false` então o caller pode fall
// voltar safely.
//
// Privacy: isso módulo apenas shapes/validates data. It executa não I/O, não telemetry, and
// não network calls. Evidence quotes live apenas dentro o note objeto (stored locally).
...
// Per-meeting mapa de canonical speaker id → user-assigned exibir nnome
// e.g. { "speaker_1": "John de ClCliente "speaker_2": "Sarah, PM" }
// "me" é reserved para o local user e é renameable ttambém
...
// Render/back-compat auxiliar block (additive; não part de o strict spec mas kept então o
// UI pode renderizar typed blocks e copy/export sem re-deriving structure).
...
// ─────────────────────────────────────────────────────────────────────────────
// Campo coercers. Cada accepts loose entrada (LLM JSON, legacy rows) e a mutable
// `track` flag define para verdadeiro sempre que a valor tinha para ser coerced/dropped.
// ─────────────────────────────────────────────────────────────────────────────
...
export function sanitizeEvidenceArray(value: unknown, max: number): EvidenceRef[] {
  return arr(value)
    .map((item: any) => {
      // Accept ambos novo (timestampMs, speakerId/speakerName) e legacy (timestamp, speaker).
...
// ─────────────────────────────────────────────────────────────────────────────
// Public: valida + repair a completo MeetingSummaryV3.
// ─────────────────────────────────────────────────────────────────────────────
...
export function validateMeetingSummaryV3(value: unknown): ValidationResult<MeetingSummaryV3> {
  ...
  // Overview é a whole-meeting paragraph TLDR (para cima para ~400 words ≈ 2800 chars).
...
// ─────────────────────────────────────────────────────────────────────────────
// Back-compat: renderizar a legacy (pre-V3) detailedSummary como a minimal V3 vvisão
// Faz Não mutate ou rewrite o stored linha — purely para rendering/normalization.
// ─────────────────────────────────────────────────────────────────────────────
