...
export class OllamaManager {
  ...
  /**
   * Initialize o manager. Checks se Ollama is running, starts it se not.
   */
  ...
  /**
   * Ping o local Ollama server para see se it responds.
   */
  private async checkIsRunning(): Promise<boolean> {
    try {
      ...
      // ECONNREFUSED ou tempo limite significa it's não running
      return false;
    }
  }

  /**
   * Spawns o 'ollama serve' command invisibly.
   */
  private startOllama(): void {
    try {
      this.isAppManaged = true;

      // Spawn detached e hidden
      this.ollamaProcess = spawn('ollama', ['serve'], {
        detached: false, // Keep attached to app lifecycle
        windowsHide: true, // Ocultar terminal em Windows
        stdio: 'ignore', // Não nos importamos com seus logs
      });
  ...
  /**
   * Polls todo 5 seconds para up para 2 minutes.
   */
  ...
  /**
   * Kills o Ollama processar ONLY se isso app started it.
   * Called quando Electron is quitting.
   */
  public stop(): void {
    ...
    if (this.isAppManaged && this.ollamaProcess && this.ollamaProcess.pid) {
      ...
      try {
        // Uso tree-kill para garante Ollama e todos its nested runner processa die
