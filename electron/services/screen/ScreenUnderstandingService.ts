...
//
// VISION-FIRST tela understanding pipeline para Refract.
//
// Flow:
//   requisição
//   → resolver imagem paths (ou capture se missing)
//   → valida paths (defense-in-depth — IPC manipulador também vvalida
//   → perceptual hash para cache/dedupe
//   → build provedor lista via buildVisionProviders() — ordenar depends em modo
//   → runVisionFallback() — primeiro non-empty sucesso wins
//   → classify result dentro de a ScreenUnderstandingResult
//
// O que isso serviço Não LONGER faz (legacy OCR pivot, 2026-05-17):
//   - Tesseract OCR. Removed de o padrão pcaminho O antigo OcrProviderManager
//     ainda exists para tests mas é não invoked haqui
//   - "OCR-first, vision quando warranted" branching. Vision é sempre o pcaminho
//   - Modes 'auto' / 'ocr_only' / 'private' (com local OCR). Replaced por
//     vision_first / vision_only / private_vision (see SettingsManager).
//
// O que isso serviço intentionally faz Não dfazer
//   - Hold provedor API clients. Those live em LLMHelper. We declare a thin
//     ProviderInvoker contract então tests pode inject fake providers e o
...
export interface ScreenUnderstandingResult {
  ...
  // PromptAssembler compatibility — populated então existing callers que lê
  // `ocrText` ou `imagePath` keep working sem a sweeping rename.
  ocrText?: string;
  imagePath?: string;
  hash?: string;
  timestamp?: number;
  // Marker então PromptAssembler knows isso came de vision, não OCR.
...
export class ScreenUnderstandingService {
  ...
  async understand(request: ScreenUnderstandingRequest): Promise<ScreenUnderstandingResult> {
    ...
    // Honor screenshots-scope gate para cima front — nunca até resolver paths se o user
    // desabilitado screenshots para o ativo pprovedor
    ...
    // Resolve imagem paths (capture se requested + missing).
    ...
    const userDataDir =
      ...
      process.env.REFRACT_TEST_USER_DATA ||
      '';
    ...
    // Cache consulta — mesmo imagem dentro de 5 min → reuse.
    if (imageHash) {
      const cached = this.cacheLookup(imageHash);
      if (cached) return cached;
    }

    // Build o provedor lista por mmodo Production callers inject isso via
    // buildVisionProviders(); tests pode substituir their próprio lista via o
    // opcional `request.providerPolicy.__providersOverride` hook (untyped to
    // keep o public contract clean).
    ...
    if (providers.length === 0) {
      ...
      return this.buildUnavailable({
        ...
        unavailableReason: mode === 'private_vision'
          ...
          : 'No vision-capable provider is configured. Add an API key for OpenAI, Claude, Gemini, Groq, or Refract, or configure a local Ollama vision model.',
      });
    }

    // Escolher optimization pperfil
    const profile = this.pickOptimizationProfile(request);

    // System & user prompts come de o prompts módulo (Fase 6).
  ...
  // Lightweight structured extraction de o model's texto osaída
  // If o modelo returned JSON (structured-extract pacaminho analisa it.
  // Caso contrário treat o saída como `visibleSummary` e executa regex extractors.
  private extractStructured(rawOutput: string): {
    ...
    if (trimmed.startsWith('{') && trimmed.endsWith('}')) {
      try {
        ...
        // fall através para plain-text caminho
      }
    }
    return {
  ...
  private assembleResult(
    ...
    // Technical interview / código hint / depurar → mark como vision_direct (final answer).
    // Caso contrário extraction caminho → vision_extract.
    ...
    return {
      ...
      ocrText: extractedText,                              // legacy kchave populated por vision
