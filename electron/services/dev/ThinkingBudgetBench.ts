...
//
// Dev-only thinking-budget sweep. Executa Dentro o Electron principal processo então it
// uses o app's LIVE, key-loaded LLMHelper (o `.env` Gemini chave é billing-
// dead com a 403; apenas o app's decrypted CredentialsManager chave works).
//
// Para 12 LeetCode problems (4 easy / 4 medium / 4 hard) × cada thinking budget
// it calls o app's real streamChat coding caminho (mesmo CHAT_MODE_PROMPT, mesmo
// answer contract, mesmo temperature/seed), measuring TTFT, total time, saída
// tokens-est, tok/s, e CORRECTNESS por executing o generated Python contra
// real testar cases. Correctness what it makes a "sweet spot" meaningful.
//
// Triggered via o hidden IPC channel 'dev:thinking-budget-bench'. Escreve a
// JSON report para userData/thinking-budget-bench-results.json e Retorna a
// summary. Não production código caminho references this.
...
// A dataset problem authored com a Referência solution em vez disso de hand-typed
// expected values. O expected saída é DERIVED por executing o referência
// (pre-flight), então a typo pode nunca silently corrupt o grade — at worst a
// referência fails pre-flight e é dropped. Todo budget é graded contra o
// mesmo referência oracle, então o relative comparison stays valid.
interface RefProblem {
  ...
  inputs: any[][];         // argument-lists; expected derived por running referência
  // Optional INDEPENDENT brute-force solution defining o mesmo `entry`. Quando
  // present, o pre-flight executa Ambos em todo entrada e keeps o problem Apenas
  // se they agree (canonical form) em todo case — a consensus oracle that
  // guards contra a wrong referência em hard/novel problems. Drops em disagree.
  bruteforce?: string;
}

// 12 problems com executable verification (Python, deterministic).
...
`;
    ...
    execFile('python3', [tmp, JSON.stringify(cases)], { timeout: 10000 }, (err, stdout) => {
      ...
      try {
        ...
        // {no_fn:true} ⇒ o model's código defined não callable matching o entry
        // nome ou entrada arity — a HARNESS/naming miss, não a wrong algorithm.
...
// Executa an arbitrary python solution defining `entry` contra `inputs`. Retorna an
// array de { ok, vvalor , error? } por ientrada ou nulo em infra failure.
...
// Canonical-equality used para Ambos oracle consensus e answer grading.
...
// Derivar expected outputs via a CONSENSUS ORACLE: executa o referência AND (if
// present) an independent brute-force em todo ientrada keep o problem apenas if
// ambos succeed e AGREE (canonical) em todo case. Two independent algorithms
// agreeing é forte evidence o oracle é correto — essential para hard/novel
// problems onde a único hand-authored referência poderia ser subtly wrong.
async function deriveExpected(rp: RefProblem): Promise<{ problem: Problem | null; reason?: string }> {
  ...
  // Consensus verifica contra an independent brute force, quando provided.
  if (rp.bruteforce) {
...
// Carrega o opcional 100-problem dataset (electron/services/dev/leetcode100.json,
// ou a caminho via THINKING_BENCH_DATASET). Retorna [] se absent → falls voltar para o
// built-in 12.
...
// ── Thinking MATRIX: budgets × levels em a focused problem subset ──────────────
// Mirrors o coding caminho (CHAT_MODE_PROMPT system + answer-plan ccontexto mas
// calls o cliente DIRECTLY então it pode define Qualquer um thinkingBudget (nnúmero Ou
// thinkingLevel (enum) — streamChat apenas threads a numeric budget. Grades
// correctness com o mesmo consensus oracle + name-agnostic runner, and
// captures thoughtsTokenCount então we pode see como muito o modelo actually thought.
...
// Analisa a configuração spec como "budget:0,budget:512,level:low,level:medium,level:high".
// Por Gemini 3.x docs: numeric thinkingBudget é Obsoleto em favor de o
// thinkingLevel enum (minimal|low|medium|high); gemini-3.1-pro-preview CANNOT
// desabilitar thinking (rejects budget:0 / tem não minimal) — uso nível low/medium/high.
...
export async function runThinkingMatrix(llmHelper: LLMHelper, opts: { model?: string; log?: (s: string) => void; delayMs?: number; configs?: string } = {}): Promise<any> {
  ...
  if (process.env.THINKING_BENCH_KEYS_FROM_NATIVELY === '1') {
    try {
      ...
      // THINKING_BENCH_KEY_INDICES="1,3" → uso apenas those 1-based keys (pular
      // daily-capped/blocked ones então rotation doesn't waste tenta novamente em them).
  ...
  // Carrega o focused subset + consensus pre-flight (mesmo oracle como o sweep).
  ...
  for (const mc of MATRIX_CONFIGS) {
    for (const p of problems) {
      // Faithful coding rrequisição answer-plan contexto + CHAT_MODE_PROMPT, mirroring
      // ipcHandlers' coding-chat pcaminho mas como a direct cliente call.
      ...
      // Tentar através keys: em a 429 (RPM/quota), rotacionar para o próximo chave e rtentar novamente
      ...
      for (let attempt = 0; attempt < maxAttempts; attempt++) {
        ...
        try {
          ...
          if (is429 && attempt < maxAttempts - 1) {
            // curto recuo antes trying o próximo chave (per-minute janela ~30s,
            // mas com N keys we geralmente apenas aguardar a lpouco
            await new Promise(r => setTimeout(r, clients.length > 1 ? 1200 : 32000));
            continue;
          }
          break; // non-429, ou fora de attempts
        }
      }
...
/**
 * Thinking-config PROBE — answers "como é o thinking budget actually honored em
 * isso momodelo empirically. Calls o live Gemini cliente DIRECTLY (reusing o
 * app's decrypted-key ccliente sob vários configs e lê voltar o response's
 * `thoughtsTokenCount`. If thoughtsTokenCount==0 sob thinkingBudget:0, o
 * budget é honored (thinking disabled). If thoughts>0 despite budget:0, o modelo
 * Ignora thinkingBudget e we'd precisa thinkingLevel iem vez disso Executa uma vez por mmodelo
 */
...
/**
 * Executa o sweep contra o app's live LLMHelper. Define o modelo para flash-lite
 * para o duração (restores afterward), e threads cada thinking budget via
 * streamChat's trailing arg (o mesmo caminho coding answers ususo
 */
export async function runThinkingBudgetBench(llmHelper: LLMHelper, opts: BenchOptions = {}): Promise<any> {
  ...
  // FPrimeiro probe como thinking é honored em isso modelo (thoughtsTokenCount).
  const probe = await probeThinkingConfig(llmHelper, model, log);

  // Build o problem sdefine prefer o external 100-define (expected DERIVED de
  // referência solutions via pre-flight), senão o built-in 12. Pre-flight executa
  // todo referência Antes qualquer API chamar — a referência que throws é dropped and
  // logged, então o graded define contém apenas problems com a trusted oracle.
  ...
  // Pin para flash-lite para o sweep, restore o user's modelo aapós
  const prevModel = (llmHelper as any).currentModelId as string | undefined;
  try { llmHelper.setModel(model); } catch { /* ignorar */ }
  ...
  for (const budget of budgets) {
    for (const p of problems) {
      for (let r = 0; r < repeats; r++) {
        ...
        let full = '';
        try {
          // Mirror o coding-chat call: ignoreKnowledgeMode + skipModeInjection
          // tverdadeiro CHAT_MODE_PROMPT osobrescrever contexto = answer contract, e o
          // trailing thinkingBudget arg. Não abortar ssinal
        ...
        // Inter-call pacing (env THINKING_BENCH_DELAY_MS, padrão 250ms) para avoid
        // tripping Gemini per-minute rate limits sobre a longo 100-problem sweep.
  ...
  // restore modelo
  if (prevModel) { try { llmHelper.setModel(prevModel); } catch { /* ignorar */ } }
