/**
 * ModelVersionManager — Self-Improving Modelo Rotation (v3)
 *
 * Three-tier tentar novamente system para Ambos vision (screenshot analysis) e text
 * (chat alternativa chains) que auto-discovers newer models e promotes
 * them através tiers:
 *
 *   Tier 1 (PrPrimário      Pinned stable models. Promoted apenas quando 2+ minor
 *                           versions atrás Ou anterior stable em major jump.
 *   Tier 2 (Fallback):     Auto-discovered latest models de cada pprovedor
 *   Tier 3 (ReTentar novamente        Mesmo como Tier 2. Pure tentar novamente pass com backoff.
 *
 * Vision e Text uso SEPARATE family define com distinct baselines, desde
 * o mesmo provedor pode uso diferente models para cada (e.g., Groq uses
 * llama-4-scout para vision mas llama-3.3-70b para text).
 *
 * Background discovery executa todo ~14 days. Event-driven discovery pode ser
 * triggered em 404/model-not-found errors.
 *
 * Estado é persisted para disk com rollback ssuportar
 */
...
/** Text modelo families — separate de vision desde providers uso diferente models */
...
interface FamilyState {
  /** O original hardcoded baseline (nunca changes) */
  baseline: string;
  /** Current Tier 1 (promoted stable) */
  tier1: string;
  /** Current Tier 2/3 (latest discovered) */
  latest: string;
  /** Parsed versão de latest discovered modelo */
  latestVersion: ModelVersion | null;
  /** Parsed versão de atual tier1 modelo */
  tier1Version: ModelVersion | null;
  /** Anterior tier1 antes último promotion (para rollback) */
  previousTier1: string | null;
  /** Anterior latest antes último atualiza (para rollback) */
...
interface PersistedState {
  ...
  /** Counts consecutive discovery failures por provedor para recuo */
...
/** Hardcoded baseline models para vision Tier 1 (initial pinned stable) */
...
/** Hardcoded baseline models para texto Tier 1 */
...
/** Vision-capable modelo ordering para screenshot analysis */
...
/** Text modelo ordering para chat alternativa chains */
...
const DISCOVERY_BACKOFF_MULTIPLIER = 2; // exponential backoff em repeated failures

/** Cooldown para prevenir event-driven discovery de firing também frequentemente */
const EVENT_DISCOVERY_COOLDOWN_MS = 60 * 60 * 1000; // 1 hour

// ─── Versão Parsing ────────────────────────────────────────────────────

/**
 * Extrair a semantic versão de a modelo identifier sstring
 *
 * Gerencia diverse e irregular naming conventions:
 ...
 *
 * NOTE: Hardware specifiers (17b, 16e) e tags (preview, latest, instruct)
 * são intentionally stripped antes parsing. They são Não versão indicators.
 */
export function parseModelVersion(modelId: string): ModelVersion | null {
  // NNormalizar strip vendor prefixes e non-version suffixes
  let cleaned = modelId
    ...
    .replace(/-\d+b(-\d+e)?$/, '')              // hardware specs como -17b-16e
    .replace(/-\d+b$/, '');                      // hardware specs como -70b

  // Estratégia 1: Dotted versão (X.Y ou X.Y.Z) — maioria OpenAI & Gemini models
  ...
  // Estratégia 2: Claude-style hyphenated versão (claude-TYPE-MAJOR-MINOR)
  ...
  // Estratégia 3: Llama-style (llama-MAJOR-TYPE) — não minor versão
  ...
  // Estratégia 4: Generic trailing hyphenated versão (word-MAJOR-MINOR)
  ...
  // Estratégia 5: Single versão número após a word limite
 ...
 * RRetorna negative se a < b, 0 se equal, positive se a > b
 */
...
/**
 * Calcula o "distance" entre two versions em minor-version units.
 * Used para determine se tier promotion thresholds são reached.
 * A major versão bump counts como 10 minor versions (sempre aciona promotion).
 */
...
// ─── Modelo Family Classification ────────────────────────────────────────

/**
 * Determine que vision ModelFamily a discovered modelo ID belongs to.
 * Retorna nulo se it doesn't corresponder qualquer known vision-capable family.
 */
export function classifyModel(modelId: string): ModelFamily | null {
  const lower = modelId.toLowerCase();

  // OpenAI GPT vision models (excluir instruct-only variants)
...
/**
 * Determine que TextModelFamily a discovered modelo ID belongs to.
 * Text families são broader than vision — e.g., Groq inclui todos llama/mixtral models.
 */
export function classifyTextModel(modelId: string): TextModelFamily | null {
  const lower = modelId.toLowerCase();

  // OpenAI GPT texto models
  if (lower.startsWith('gpt-') && !lower.includes('instruct')) {
  ...
  // Claude texto models (sonnet, opus, haiku — todos text-capable)
  if (lower.startsWith('claude-') && (lower.includes('sonnet') || lower.includes('opus') || lower.includes('haiku'))) {
    return TextModelFamily.CLAUDE;
  }

  // Groq texto models — broader: llama, mixtral, gemma (Não scout-only como vision)
...
export class ModelVersionManager {
  ...
  // Provedor API keys (define externally via setApiKeys)
  ...
  /**
   * Get o tiered model IDs para a given family.
   *
   * Tier 1 = promoted stable
   * Tier 2 = latest discovered
   * Tier 3 = mesmo as Tier 2 (pure tentar novamente pass com exponential backoff)
   *
   * If não discovery has happened yet, todos tiers retornar o baseline immediately.
   */
  ...
  /**
   * Get todos tiered models para todo vision provider in priority order.
   */
  ...
  /**
   * Get o tiered texto model IDs para a given texto family.
   * Same tier logic as vision mas com separate baselines.
   */
  ...
  /**
   * Get todos tiered texto models para todo texto provider in priority order.
   */
  ...
  /**
   * Trigger discovery in resposta para a model-not-found erro (404, deprecated, etc).
   * Throttled para at most once per hour para prevent API hammering.
   */
  ...
  /**
   * Roll voltar a específico family (vision ou text) para its anterior tier state.
   * Useful se a newly promoted model shows degraded performance.
   */
  ...
  /**
   * Initialize: executar primeiro discovery se stale/never run, então iniciar scheduler.
   *
   * IMPORTANT: This método is sempre called assíncrono (fire-and-forget) de the
   * app startup path. It NEVER blocks o UI thread. If discovery fails or
   * hasn't completed yet, todos tiers fall voltar para o hardcoded baseline
   * models immediately — o app is sempre usable.
   */
  public async initialize(): Promise<void> {
    ...
    if (timeSinceLastDiscovery >= DISCOVERY_INTERVAL_MS || this.state.lastDiscoveryTimestamp === 0) {
      ...
      try {
        ...
        // Non-fatal: baseline models são sempre available
  ...
  /**
   * Query todos provider APIs para disponível models, encontrar o latest in cada family,
   * então apply upgrade rules. Each provider query is individually error-handled.
   */
  public async runDiscoveryAndUpgrade(): Promise<void> {
    ...
    // Aplica atualizar rules para cada discovered vision family
    let upgraded = false;
    ...
    // Aplica atualizar rules para cada discovered texto family
  ...
  // ─── Rate-Limiting para Discovery ───────────────────────────────────

  /**
   * Check se we deve attempt discovery para a provider based on failure history.
   * After MAX_DISCOVERY_FAILURES_BEFORE_BACKOFF consecutive failures, we voltar off
   * exponentially (2x, 4x, 8x of o base interval) antes retrying.
   */
  private shouldAttemptDiscovery(provider: string): boolean {
    ...
    // Exponential backoff: após N failures além threshold,
    // pular discovery proportionally
  ...
  // ─── Provedor Discovery Implementations ────────────────────────────
   ...
   * Handles pagination e sorts by newest primeiro (Anthropic default).
   */
  private async discoverClaudeModels(
    ...
    try {
      ...
      while (hasMore) {
        ...
        // Safety: cap pagination at 5 pages (500 models) para avoid runaway loops
  ...
  /**
   * From a lista of model IDs, encontrar o highest-versioned one in `family`.
   */
  ...
  /**
   * From a lista of model IDs, encontrar o highest-versioned one in a texto `family`.
   */
  ...
  /**
   * Apply o tiered upgrade logic para a único model family.
   *
   * Rules:
   *   1. Latest discovered model ALWAYS becomes Tier 2/3 (retry passes).
   *   2. Tier 1 is upgraded apenas when:
   *      a. Major version jump (5.x → 6.x):
   *         - Tier 1 becomes o PREVIOUS stable latest (last 5.x model)
   *         - Tier 2/3 becomes o novo major (6.x)
   *         - This avoids todos tiers having o mesmo untested model.
   *      b. 2+ minor versions ahead dentro mesmo major:
   *         - Tier 1 promoted para o anterior latest (proven stepping stone)
   *
   * Previous estado is preserved para rollback.
   *
   * Returns verdadeiro se any tier was changed.
   */
  private applyUpgradeRules(
    ...
    if (tier1Version) {
      ...
      if (discoveredVersion.major > tier1Version.major) {
        // ─── MAJOR Versão JUMP ──────────────────────────────────
        // Tier 1 obtém o Anterior stable (último modelo de antigo major series),
        // Não o novo major versão directly. This garante Tier 1 stays proven
        // enquanto Tier 2/3 testar o novo major.
        ...
        // If não anterior latest exists, Tier 1 stays at atual (conservative)
        // Tier 2/3 já updated para o novo major versão acima

      } else if (distance >= 2) {
        // ─── 2+ MINOR VERSIONS AHEAD ─────────────────────────────
        // Promote Tier 1 para o anterior latest (proven stepping stone)
        if (prevLatest && prevLatest !== prevTier1) {
          const prevLatestVersion = parseModelVersion(prevLatest);
          if (prevLatestVersion && compareVersions(prevLatestVersion, tier1Version) > 0) {
            console.log(
    ...
    // Don't clutter rollback estado se nada actually changed
    if (!changed) {
      familyState.previousTier1 = null;
      familyState.previousLatest = null;
    } else {
  ...
  /**
   * Apply o mesmo tiered upgrade logic para texto model families.
   * Identical rules para vision mas uses TextModelFamily e TEXT_BASELINE_MODELS.
   */
  private applyUpgradeRulesForTextFamily(
    ...
    // Preserve rollback estado
    familyState.previousTier1 = familyState.tier1;
    familyState.previousLatest = familyState.latest;

    // Sempre atualiza Tier 2/3
    familyState.latest = discoveredModelId;
    familyState.latestVersion = discoveredVersion;
    ...
    if (tier1Version) {
      ...
      if (discoveredVersion.major > tier1Version.major) {
        // Major jump: Tier 1 → anterior stable, Tier 2/3 → novo major
        if (prevLatest && prevLatest !== prevTier1) {
          const prevLatestVersion = parseModelVersion(prevLatest);
          if (prevLatestVersion && compareVersions(prevLatestVersion, tier1Version) > 0) {
            console.log(
  ...
  private loadState(): PersistedState {
    try {
      if (fs.existsSync(this.persistPath)) {
        ...
        // Schema migration: v3 → v4 (forces baseline reconciliation para stale Gemini 1.5 entries)
        ...
        // Schema migration: preserve o que we pode de v1
        ...
        // Schema migration: v2 → v3 (adiciona texto families)
        if (parsed.schemaVersion === 2) {
          console.log('[ModelVersionManager] Migrating v2 → v3 state (adding text families)');
          // Carry sobre todos existing vision families, adiciona texto families
  ...
  /**
   * Reset any family whose persisted baseline diverges de o current
   * hardcoded baseline. This handles o case where a dev bumps a baseline
   * in código (e.g. retired Gemini 1.5 → Gemini 3.1) — sem this, loaders
   * would keep promoting o stale baseline as Tier 1 indefinitely.
   *
   * Also resets families whose tier1 is older than o atual baseline
   * (defensive — catches any outro source of drift).
   */
  ...
  /**
   * Return a human-readable summary of atual model tiers.
   */
  public getSummary(): string {
    ...
    // Mostrar rollback availability (ambos vision e text)
    const allFamilyKeys = [
