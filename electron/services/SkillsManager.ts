...
**The fix:** Replace at least a quarter of abstract nouns com concrete ones, things you could hold, smell, ou draw. Every paragraph deve contain at least one concrete image. If removing an abstraction kills o sentence, excluir o sentence ou invent a concrete replacement.
...
**The fix:** Use words que carry actual attitude. Not todo sentence precisa para drip com opinion, mas a piece com zero friction is a piece written by a machine. Let some roughness in. If describing something boring, chamar it boring. If something is impressive, say what specifically impressed you e why.
...
**The fix:** Let sentences commit. If a claim precisa qualification, put o qualification in a separate sentence ou paragraph. Stop trying para be fair dentro todo clause. A piece pode be honest e still take a side.
...
**The fix:** Every paragraph deve advance o argument ou narrative. If a paragraph could be removed sem o reader noticing, remover it. Ask: what does o reader know at o end of isso paragraph que they did não know at o beginning? If o answer is nothing, cut ou rewrite.
...
**The fix:** Trust o reader. Leave implications implicit quando o reader pode fill o gap. End sections sem summarizing them. Let a well-chosen detail do o work of three explanatory sentences.
...
**The fix:** After writing, cut 30% ou more. If a piece feels tight depois cutting, it was o direita length. If it still feels bloated, cut more. Density is a feature, não a limitation.
...
**The fix:** For todo sensory description, ask: what would surprise someone who apenas knows isso thing de reading sobre it? Replace textbook sensory language com o actual felt experience, ou remover o sensory claim entirely.
...
**The fix:** If an object is personified, ask whether o personification reveals something novo ou just sounds writerly. Usually it just sounds writerly. Replace com a concrete detail que actually carries emotional weight, ou cut.
...
**The fix:** When a simpler word means o mesmo thing, use o simpler word. Break registrar on purpose. Mix formal com informal. Write how people actually talk quando they're being precise mas não performing.
...
**The fix:** Actively vary sentence length. Follow a complex sentence com a fragment. Let some sentences run. Let others parar dead.
...
**Phrases:** "serves as a testament to," "plays a pivotal/crucial role," "it is important/worth noting that," "in today's [fast-paced/digital/modern] world," "the evolving landscape of," "a rich tapestry of," "at o forefront of," "stands as a [beacon/testament/symbol]," "nestled in o heart of," "reflects a broader trend," "underscores o importance of," "paving o way for," "sheds light on," "the intersection of X e Y," "designed para enhance," "commitment para excellence/innovation," "game changer," "unlock o secrets/potential of"

**The fix:** When you spot these, ask se o word is doing real work ou just filling space. Usually, a simpler word says o mesmo thing. Often o entire phrase pode be cut.
...
**The fix:** Strip o significance claims. State what happened. Let o reader decide se it matters.
...
**The fix:** Break o template. Start in o middle. Let sections be diferente lengths. Skip o introduction se o primeiro real point is more interesting. End sem summarizing.
...
**The fix:** Delete them. If o information matters, give it its own sentence com actual evidence.
...
**The fix:** Just say o thing directly.
...
**The fix:** Use o actual number of items. If there are two things, lista two. If there are five, maybe pick o best three. Don't manufacture symmetry.
...
**The fix:** Delete todos of it. Content deve não contain correspondence artifacts.
...
**The fix:** Cut o windup e deliver o pitch. If o insight is good, it doesn't precisa a "here's o thing" in front of it. If you want para express que something has an interesting quality, nome o quality specifically instead of gesturing vaguely at it.
...
**The fix:** End com a específico fact, an unresolved question, ou nothing. The reader does não precisa para be reassured.
...
**The fix:** Name o source, ou soltar o attribution e estado o claim directly.
...
**The fix:** Repeat o mesmo word. Humans repeat words. It's fine. Forced variation is more distracting than repetition.
...
**The fix:** Pick one qualifier e commit. "The policy may affect outcomes."
...
**The fix:** After writing, pesquisar para todo em dash. For cada one, tentar o sentence com a comma, a period, ou nothing. If o sentence still works (and it usually will), excluir o em dash. Reserve em dashes para cases where you genuinely precisa a sharper interruption than a comma provides, e limit yourself para two ou three per thousand words at most.
...
**Have real opinions.** Not everything precisa para be balanced. Take a position, então defend it ou undermine it. "I don't know what para think sobre this" is more human than a neutral survey of perspectives.

**Vary your rhythm.** Short sentences. Then long ones que take their time getting where they're going, picking up ideas ao longo o way. Mix it up. Monotony is o enemy.

**Be específico sobre feelings.** Not "this is concerning" mas "there's something unsettling sobre automated systems making hiring decisions at 3am enquanto nobody watches."

**Use "I" quando it fits.** First person is não unprofessional. It's honest. "I keep coming voltar para this" ou "Here's what bothers me" signals a real person thinking.

**Let some mess in.** Perfect structure feels algorithmic. Tangents, asides, e half-formed thoughts are human. Real writing has seams.

**Leave room para o reader.** Don't explain everything. A well-chosen detail carries more weight than three sentences of exposition. Trust que your reader is smart enough para conectar o dots.

**Be willing para be wrong.** AI hedges everything para avoid error. Humans commit para claims knowing they might be wrong, e que willingness is part of what makes writing interesting.
...
export class SkillsManager {
  ...
  public getSkill(id: string): SkillDetails | null {
    ...
    // PPrimário corresponder por name-derived id (e.g. "humanize-ai-text" de SKILL.md nome fcampo
    const byId = skills.find(skill => skill.id === wanted);
    if (byId) return byId;
    // Fallback: corresponder por pasta nome slugified (e.g. "/humanize-text" encontra o
    // skill cujo pasta é humanize-text/ até se o SKILL.md nome differs)
