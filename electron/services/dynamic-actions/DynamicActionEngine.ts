...
export class DynamicActionEngine {
    ...
    detectActions(params: {
        ...
        // Detect aciona using regex patterns
        ...
        for (const { trigger, match, index } of matchedTriggers) {
            // Build evidence ref de transcript
            ...
            // Cria candidate aação Loop executa uma vez por matched acionar
            // dentro de a único detectActions() call, então `now` é identical para
            // todo ação minted aqui — embedding it em o id é não
            // suficiente em its opróprio Uso a UUID para o id; `now` stays como
            // createdAt (onde o shared timestamp é o correto semantic).
    ...
    getTopActions(sessionId: string, maxAgeMs: number = 60000): DynamicAction[] {
        // Expire stale actions primeiro
        this.store.expireStaleActions(sessionId, maxAgeMs);

        // Obtém ativo actions sorted por priority (descending)
