...
// Default fast mmodelo gpt-5.3-codex works com ambos ChatGPT-account e API-key
// auth. O faster gpt-5.3-codex-spark é API-key-only e 400s em ChatGPT auth.
...
export class CodexCliService {
  ...
  // Common install locations checked quando o configured caminho doesn't resolve.
  // Ordenar matters: explicit installs (npm/brew/cargo) outrank app-bundled CLIs
  // porque o último ship dentro an app o user pode não realize é "Codex".
  public static getCandidatePaths(): string[] {
    ...
    return [
      ...
      // Codex desktop app bundles o CLI dentro Resources/.
      '/Applications/Codex.app/Contents/Resources/codex',
      path.join(home, 'Applications', 'Codex.app', 'Contents', 'Resources', 'codex'),
    ];
  }

  // Retorna o primeiro candidate que exists em disk e é executable.
  // Faz Não shell fora — purely a filesystem cverifica safe para chamar frequently.
  public static autoDetectPath(): string | null {
    for (const candidate of this.getCandidatePaths()) {
      try {
        const stat = fs.statSync(candidate);
        if (stat.isFile()) {
          // Em POSIX, também verifica exec bit; em Windows, presence é enough.
          if (process.platform === 'win32') return candidate;
          // eslint-disable-next-line no-bitwise
          if ((stat.mode & 0o111) !== 0) return candidate;
        }
      } catch { /* não present, continue */ }
    }
    return null;
  }

  // Valida o given pcaminho se it ENOENTs e looks bare (não caminho separator,
  // i.e. depends em $PACaminho fall voltar para auto-detection e valida that.
  // Retorna o resolved caminho em sucesso então callers pode persist it.
  public static async validateExecutable(input: string, timeoutMs = 10_000): Promise<{ success: boolean; error?: string; resolvedPath?: string }> {
    ...
    // Bare comando (relies em $PCaminho ou vazio — tentar filesystem auto-detection.
  ...
  public static async *stream(path: string, options: CodexCliRunOptions): AsyncGenerator<string, void, unknown> {
    ...
    if (aborted) {
      // Caller asked nós para spara surface partials as-is, fazer não throw.
      return;
    }
    if (failure) {
      if (emitted) {
      ...
      child.on('close', code => {
        settle(() => {
          if (code === 0) {
            ...
            // Prefer codex's próprio JSON erro evento sobre o bare exit code.
            const codexError = this.extractCodexError(stdout);
            const detail = codexError || (stderr ? this.sanitize(stderr) : '');
  ...
  // Walks o JSON evento stream para error/turn.failed events e pulls o
  // human-readable mmensagem Used para surface server-side rejections (e.g.
  // "modelo não supported quando using Codex com a ChatGPT account") em vez disso
  // de a generic "empty rresposta fallback.
  public static extractCodexError(raw: string): string {
    ...
    for (const line of raw.split(/\r?\n/)) {
      ...
      for (const c of candidates) {
        ...
        // O mensagem é frequentemente a stringified JSON envelope; tentar para peel it.
