// Mobile cliente served por PhoneMirrorService.
// Inlined aqui então it travels com o asar bundle sem extra build steps.
// Edit o template babaixo whitespace é preserved como written.
  ...
  <body>
    ...
    <script>
      (function () {
        ...
        let messageIdCounter = 0;   // monotonic counter — Date.now() alone collides quando two acks fire em o mesmo ms (see issue #253)
        ...
        function buildCard(m, opts) {
          // Screenshot-queued notificação cartão (não imagem — stays em desktop)
          ...
          // Label tag para shortcut-triggered responses
        ...
        function handleEvent(ev) {
          ...
          // Non-streaming assistant resposta de shortcut-triggered actions
          ...
          // Ack events de stealth operations (screenshot captured, etetc
          if (ev.type === 'ack') {
            showToast(ev.message || ev.action);
            // Para screenshot acks, também adiciona a pequeno cartão para o feed.
        ...
        // Screenshot botão — aciona a stealth desktop capture queued para AI.
        // O imagem stays em o PC; apenas a confirmation notificação appears em o phone.
        ...
        // Inicia disconnected — buttons desabilitado até connected
