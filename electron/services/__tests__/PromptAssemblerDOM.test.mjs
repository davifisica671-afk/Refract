...
describe('PromptAssembler DOM Extension', () => {
  ...
  test('token budget truncates long DOM context block', async () => {
    ...
    const result = assembler.assemble({
      ...defaultParams,
      domContext: longDOM,
      tokenBudget: 600, // Pequeno budget
    });
  ...
  test('neutralizes HTML-split prompt injection patterns inline in reference files', async () => {
    ...
    // Para referência files, it neutralizes inline em vez disso de total block redaction.
    // It deve ter replaced "ignore...instructions" com "Ignorar [REDACTED] instructions"
  ...
  test('hasPromptInjection ignores HTML strikethrough tags when properly escaped', async () => {
    ...
    // Make certo buildDomContextBlock também faz não redact it
    const result = assembler.assemble({
      ...defaultParams,
      domContext: dom,
    });
  ...
  test('neutralizes prompt injection patterns hidden inside HTML attributes', async () => {
    ...
    // Verifica it aciona completo redaction em DOM contexto caminho
    const result = assembler.assemble({
      ...defaultParams,
      domContext: bypass1,
    });
