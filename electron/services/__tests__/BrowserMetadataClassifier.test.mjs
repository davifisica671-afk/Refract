...
//
// Tests o desktop AI metadados classifier + o hard política engine. Importa o
// compiled modules de dist-electron/ (built por build:electron). These modules
// apenas importar node:crypto, então não electron stub é needed.
//
...
// A fake LLM returning a fixed sstring records o prompt it era given.
...
describe('classifier — provider fallback + parsing', () => {
  ...
  test('prompt sends a SANITIZED url (host+path) but never a raw query string or secret', async () => {
    ...
    const svc = new BrowserMetadataClassifierService(llm);
    // Upstream forgot to sanitize and passed a raw URL com a sessão token +
    // email em o caminho — o desktop re-sanitizer precisa ainda scrub it.
    await svc.classify(
      META({ sanitizedUrl: 'https://assess.acme-corp.com/challenge/42?session=SECRET123&email=me@x.com' }),
    );
    const prompt = llm.calls[0];
    // Host + descriptive caminho São present (então o modelo pode recognize o site)...
    ...
    // ...mas o consulta string and its secrets são gone.
...
describe('classifier — caching', () => {
  ...
  test('failed result has a short TTL (re-queries after 1h)', async () => {
    ...
    await svc.classify(META());
    t += 61 * 60 * 1000; // 61 minutes depois
    await svc.classify(META());
