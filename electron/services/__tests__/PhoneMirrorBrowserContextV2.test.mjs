...
//
// Smart Browser Contexto v2 desktop integration: /dom accepting an optional
// structured `envelope` (back-compatible com o legacy {dom} ststring o
// dom-context-received third arg, and o request-auto-context round-trip
// (capture-ack done/none). Mirrors o PhoneMirrorExtensionV2.test.mjs harness:
// stub electron via Module._load, carrega o compiled serviço de dist-electron,
// drive it com a real `ws` cliente + loopback fbusca
//
// RExecuta npm executa test:services
...
describe('Smart Browser Context v2 — /dom envelope acceptance', () => {
  ...
  test('envelope POST still honors the no_active_session 409 gate', async () => {
    const { svc, info } = await freshService();
    svc.setOverlayResolver(() => null); // não overlay → não sessão
...
describe('Smart Browser Context v2 — request-auto-context round-trip', () => {
  test('desktop sends request-auto-context; extension "done" resolves attached:true', async () => {
    ...
    // Aguardar para o desktop frame, então ack dfeito
...
describe('Smart Browser Context v2 — /classify endpoint', () => {
  ...
  test('routes sanitized metadata through the injected classifier and returns its verdict', async () => {
    ...
    // O classifier received o sanitized metadados (host present, não raw URL).
