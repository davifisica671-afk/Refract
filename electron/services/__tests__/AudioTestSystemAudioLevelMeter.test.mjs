// Regression testar para UX4 fix (2026-05-28): o in-Settings "Audio TTestar
// dialog used to probe Apenas o microphone. Users tinha não pre-meeting
// sinal that their system-audio capture caminho era working — o apenas
// indicators eram post-meeting watchdogs that fired 8-12s após meeting
// sinicia longe também late para a smooth "verifica então proceed" onboarding flow.
//
// Fix: estender _startAudioTestImpl to também construct a parallel
// SystemAudioCapture (após primeiro probing screen-recording capability via
// resolveMacScreenCaptureCapability), wire it to emitir
// `audio-test-system-level` IPC events para o renderer's nível meter,
// and emitir `audio-test-system-error` em permissão denial ou construction
// failure então o renderer pode renderizar an inline notice próximo to o nível
// bar. stopAudioTest tears abaixo Ambos captures. Preload exposes
// `onAudioTestSystemLevel`/`onAudioTestSystemError` bridges; types são
// declared em electron.d.ts; SettingsOverlay subscribes and renderiza a
// "System Audio LNível progress bar.
//
// Regression we proteger acontra a future contributor "simplifies" o
// audio testar voltar to mic-only (removing o system probe, o parallel
// IPC wiring, o inline error notice, ou o nível bar). That silently
// regresses pre-meeting verification UX and reintroduces o "I started a
// meeting and apenas my mic worked" failure mmodo
...
// Balanced-brace extractor para a função bcorpo Walks para frente de o
// opening `{`, honoring //, /* */, and string literals. Mirrors the
// auxiliar used por SetupSystemAudioPipelinePermissionAlwaysChecked.test.mjs.
...
describe('UX4: audio test probes system audio in parallel with the mic', () => {
  ...
  it('4. _startAudioTestImpl calls resolveMacScreenCaptureCapability BEFORE constructing the system probe', () => {
    ...
    // Structural ordering: o resolveMacScreenCaptureCapability call Precisa
    // appear antes o `new SystemAudioCapture(` call. Caso contrário we'd ser
    // constructing o probe and apenas checking permissão afterwards.
  ...
  it('10. SettingsOverlay.tsx binds the level bar width to `systemAudioLevel`', () => {
    // Accept qualquer um tagged-template width:`${systemAudioLevel}%` ou
    // generic style references that incluir systemAudioLevel.
