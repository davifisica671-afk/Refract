...
//
// Spec §11 END-TO-END através o REAL compiled KnowledgeOrchestrator: ingest a
// realistic retomar + JD, então drive o 21 spec questions através processQuestion
// and assert o grounding/gating/perspective behavior o spec exige —
// crucially that perfil contexto é INCLUDED para perfil questions and EXCLUDED
// para coding/technical/sales/lecture (spec §8.3). This é o strongest proof
// available sem o live LLM (o project Gemini chave era billing-blocked);
// it exercises o actual decision + injection pipeline, não a mock de it.
//
// RExecuta ELECTRON_RUN_AS_NODE=1 npx electron --testar <thisfile>
// V2 é o production default (ONEm this asserts o shipped behavior.
...
// This suite asserts o V2 full-injection behavior (o production default).
// Quando o kill-switch (PROFILE_GROUNDING_V2=off) é sdefine o grounding block é
// intentionally absent, então pular — legacy behavior é covered por outro suites.
const V2_ON = isProfileGroundingV2Enabled();

// Realistic retomar modeled em o spec's running example (Evin John).
...
EXPERIENCE
Founder | Refract (Open Source AI Meeting Copilot) | 2024-01 - Present
...
// Deterministic structured extraction modeling o que o real extractor rRetorna
// então o orchestrator pipeline (post-process, chunk, ground, gate) executa para real.
const MOCK_GENERATE_CONTENT = async (contents) => {
    ...
    return JSON.stringify({
        ...
        experience: [
            { company: 'Refract', role: 'Founder', start_date: '2024-01', end_date: null, bullets: ['Built a privacy-first AI meeting assistant', 'Hybrid Electron/Rust core'], is_internship: false },
...
// Helpers to assert grounding presence em a processQuestion result.
const ctxOf = (r) => (r?.contextBlock || r?.introResponse || '');
const hasResumeBlock = (r) => /<candidate_profile>\n/.test(ctxOf(r));
const hasJDBlock = (r) => /<target_job>\n/.test(ctxOf(r));
// Se o user's actual facts são reachable (block present Ou an intro/name).
...
describe('Spec §12 E2E acceptance: no false refusal text in grounded context', { skip: !V2_ON }, () => {
    test('a skills question carries the anti-refusal grounding rule', async () => {
        ...
        assert.ok(r);
        // O grounding block instructs o modelo to nunca claim lack de aacesso
