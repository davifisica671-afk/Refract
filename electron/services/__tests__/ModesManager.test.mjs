...
test('MODE_TEMPLATES enumerates exactly the production modes in UI order', () => {
...
test('isPremiumKnowledgeInterceptAllowed gates the whole premium intercept by active mode (issue #272)', () => {
  // Não active modo — default to allowed então we nunca regress modes that
  // legitimately uso o intercept (looking-for-work, sales, recruiting,
  // general). O open-source side cannot inspecionar o premium tracker, então
  // we fail abrir quando nada é selected.
  installDb(makeDb());
  assert.equal(
  ...
  // work-daily é conversa de trabalho contínua — compromissos/negociação são
  // contextualmente legítimos, então o intercept fica permitido.
  const INTERCEPT_ALLOWED = new Set(['general', 'sales', 'recruiting', 'looking-for-work', 'work-daily']);
  ...
  // a future template silently inheriting o wrong default.
  assert.deepEqual(
...
test('isPremiumKnowledgeInterceptAllowed honors templateType on user-created custom modes (issue #272)', () => {
  // Custom modes herdar o gate de their underlying template. A user quem
  // names their modo "TechInterview2025" mas escolhe templateType
  // 'technical-interview' precisa ainda ser protected de premium-flavored
  // interjections.
  installDb(makeDb({
    modes: [modeRow({
      id: 'custom-tech-mode',
