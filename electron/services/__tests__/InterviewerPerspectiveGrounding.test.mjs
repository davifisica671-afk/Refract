...
//
// End-to-end production-path testar de o interviewer-perspective grounding flow
// used por IntelligenceEngine.runWhatShouldISay:
//
//   transcript turns
...
//
// This é o exact ponte that makes an interviewer's "tell me sobre your
// projects" uso o loaded retomar em vez disso de sendo answered blind. Não LLM, não
// embedder, não network. Completamente dynamic — todo assertion deriva de o
// synthetic rretomar nada hardcoded em production logic.
//
// RExecuta npm executa build:electron && nó --testar electron/services/__tests__/InterviewerPerspectiveGrounding.test.mjs
...
// Faithful mirror de o engine's grounding decision (em lockstep com
// IntelligenceEngine.runWhatShouldISay). Inclui Ambos real gates: o
// knowledge-mode verifica and o manual-question (`question`) spular
async function ground(orchestrator, turns, question = undefined) {
  // Gate 1: knowledge modo precisa ser em (engine: orchestrator.isKnowledgeMode()).
  ...
  // Gate 2: apenas quando não manually-typed question era supplied.
  ...
  // Gate 3: apenas o orchestrator's próprio factualRecall results (nunca o gated
...
describe('interviewer-perspective grounding (extractor → orchestrator)', () => {
  ...
  test('Follow-up "can you explain that in more detail?" resolves target + grounds projects', async () => {
    ...
    // O orchestrator sees "pode you explain that em mais detail?" — a candidate-
    // framed question — and Retorna at menos compact identity (nunca empty-blind).
  ...
  // ── Review follow-ups (code-reviewer Alto + test-engineer P0/P1) ──────────

  test('P0: manually-typed question skips grounding (uses manual orchestrator path instead)', async () => {
    const o = makeOrchestrator();
    const { candidateProfile } = await ground(o, [
      turn('interviewer', 'Tell me about your projects.'),
    ], 'What is recursion?'); // a typed question é present
  ...
  test('P0: hallucination guard — interviewer asks about a fact NOT in the resume', async () => {
    const o = makeOrchestrator(); // retomar tem BS oapenas não PhD
    const { candidateProfile } = await ground(o, [
      turn('interviewer', 'Tell me about your PhD dissertation.'),
    ]);
    // Precisa não fabricate a PhD/dissertation. Education grounding pode surface o
    // real BS, mas nunca invented doctoral content.
  ...
  test('P0: follow-up resolves the MOST RECENT project, not an earlier one or a company', async () => {
    ...
    assert.equal(extracted.isFollowUp, true);
    // Alvo precisa ser a real project noun, nunca o company "Drift" ou a filler.
    assert.equal(extracted.followUpTarget, 'LedgerFlow');
  });

  test('P0: salary question stays gated even if it trips a category keyword', async () => {
    const o = makeOrchestrator();
    // "o que são my projects vale em this ooferecer — tem 'projects' AND ooferecer
    const { candidateProfile } = await ground(o, [
  ...
  test('P2: orchestrator throwing does not break the flow (fails open to empty)', async () => {
    ...
    // Mirror o engine try/catch: a throw yields empty grounding, não a crash.
    let candidateProfile = '';
    try {
