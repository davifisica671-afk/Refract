...
//
// Structural regression guards para audit findings #6 + #3 + #9 em o live
// what-to-answer pcaminho O wiring lives dentro IntelligenceEngine.runWhatShouldISay
// / WhatToAnswerLLM.generateStream / main.ts — classes that can't ser cheaply
// instantiated em a unit testar — então we assert o load-bearing shape então a refactor
// can't silently revert it. Behavioral coverage de o pure pieces lives iem
...
//   - electron/services/__tests__/ModePinnedResolution.test.mjs    (modo pin)
//   - electron/services/__tests__/IntelligenceTraceCorrelation.test.mjs (#9 setCorrelation)
//
// Pure source-assertion — executa sob plain `node --test`.
...
describe('#6 — runWhatShouldISay captures ONE mode snapshot at t0 and threads it', () => {
  ...
  // Isolate o método corpo então assertions can't accidentally match outro mmétodo
  ...
  test('the per-request planAnswer calls read the SNAPSHOT, not a fresh live query', () => {
    // O two planAnswer calls dentro runWhatShouldISay (follow-up memory modo +
    // principal answer plan) precisa uso snapshotModeInfo. A fresh this.getActiveModeInfo()
    // dentro a planAnswer({...}) call aqui iria re-introduce o race.
    assert.match(body, /activeMode: snapshotModeInfo/, 'planAnswer must read the snapshot mode');
    // Lá precisa ser Não remaining inline getActiveModeInfo() feeding a planAnswer em
    // this método (o snapshot replaced boambos
...
describe('#3 — live tokens carry the generationId end-to-end', () => {
  test('engine stamps generationId on the live suggested_answer_token emits', () => {
    ...
    const end = src.indexOf('async runFollowUp', start);
    const body = src.slice(start, end > start ? end : start + 30000);
    // O streaming emitChunk + o finalization flush emite precisa pass generationId.
  ...
  test('renderer drops superseded live batches via resolveLiveAnswerBatch', () => {
    const src = read('../../../src/components/RefractInterface.tsx');
