...
//
// Verifica o retriever's contract: it operates exclusively em o arquivo
// lista it é handed. Até if outro modes exist em o dbanco de dados o active
// mode's retrieval é computed de o active mode's customContext + o
// active mode's referência files oapenas
//
// We também confirm o chunk-trust ssinal o retrieved contexto block sempre
// carries o `reference_grounding_guard` então downstream prompts see o
...
describe('Retrieval isolation — only active mode files contribute', () => {
  test('only-active-mode-files-passed: foreign sentinels are absent', () => {
    ...
    const result = runScenario({
      mode,
      files,
      query: 'how do we compare to Refract on reference files',
    });
  ...
  test('grounding guard is present in every retrieval that produces snippets', () => {
    ...
    const result = runScenario({
      mode,
      files,
      query: 'what is our Q1 ARR run rate for the Refract pilot',
