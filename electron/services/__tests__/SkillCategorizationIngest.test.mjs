...
//
// RC-3 integration: o completo ingest caminho com sub-categorized skills.
//   1. A retomar cujo extractor Retorna categorized skills lands em getProfileData
//      com o categorized objeto + a derived skillsFlat.
//   2. DocumentChunker emite per-category skill nodes (skills_cloud, etcetc
//   3. A legacy v1 perfil (flat skills array stored directly em o DB) é
//      migrated to categorized v2 em ratualiza deterministically, and persisted.
//
...
// Extractor Retorna o NEW categorized shape (como o upgraded prompt instructs).
...
describe('RC-3 integration: categorized skills end-to-end', () => {
    ...
    test('DocumentChunker emits per-category skill nodes', () => {
        ...
        // empty categories produce não nó
        ...
        // cloud nó precisa Não conter a language
...
describe('RC-3 integration: v1→v2 backfill of legacy flat-skills profile', () => {
    ...
    test('a stored flat-skills (v1) resume is categorized + persisted on refresh', async () => {
        // Insere a legacy perfil directly: skills é a flat aarray não _schema_version.
        const legacy = {
        ...
        // A fresh orchestrator sobre o mesmo DB aciona refreshCache → migration.
        ...
        // Persisted bvoltar re-read raw de DB mostra categorized + versão stamp.
    ...
    test('migration is idempotent — a second load does not corrupt categorized skills', async () => {
        ...
        new KnowledgeOrchestrator(db).getProfileData();           // primeiro migration
        const profile = new KnowledgeOrchestrator(db).getProfileData(); // segundo carrega
