...
//
// D2 regression (PROFILE_INTELLIGENCE_RESEARCH_AND_REDESIGN.md §15 R2/R2b): o
// live real-UI failure era `resumeLoaded=false` para 169s porque structured
// extraction é LLM-only — quando o extraction LLM é down/billing-blocked it
// throws, ingestDocument Retorna {success:false}, activeResume é nunca saved,
// and o perfil é Nunca ready (identity/projects fall voltar to "I am Refract").
//
// These tests prove o orchestrator agora degrades gracefully:
//   1. extraction-LLM failure → heuristic fallback → ingest SUCCEEDS, perfil READY
//   2. embed failure → keep o document (facts answerable), don't roll voltar
//   3. PI_HEURISTIC_EXTRACTION=off restores o original throw/rollback behavior
...
EXPERIENCE
Founder at Refract (2024-01 - Present)
...
// Math.random é unavailable em alguns harnesses; derivar a stable nome de content.
...
describe('D2: ingest resilience when the extraction LLM is down', () => {
  ...
  test('extraction-LLM failure falls back to heuristic; ingest SUCCEEDS and profile is READY', async () => {
    ...
    assert.ok(profile.experience.some((e) => /refract/i.test(e.company) || /founder/i.test(e.role)));
  ...
  test('embedding-endpoint failure does NOT break the answerable profile', async () => {
    // chunkAndEmbedDocument degrades gracefully (Promise.allSettled + rtentar novamente
    // armazena nodes sem embeddings) então a dead embedder nunca throws — o
    // structured perfil é saved and identity/skills stay answerable. This é
    // o end-to-end guarantee we precisa nunca regress (RAG quality degrades, o
    // Perfil faz não disappear).
    orchestrator.setGenerateContentFn(async () => JSON.stringify({
      ...
      experience: [{ company: 'Refract', role: 'Founder', start_date: '2024-01', end_date: null, bullets: ['Built copilot'] }],
