...
//
// PI v3 (W2): o mode's "Real-time prompt" (customContext) é Sempre pinned
// dentro de o prompt — não longer retrieval-dependent. Invariants:
//   1. getActiveModePinnedInstructions Retorna o customContext deterministically.
//   2. Sensitivity scoping ainda aplica (salary/pricing chunks dropped para
//      non-negotiation answer types; included para negotiation).
//   3. 1,200-char cap.
//   4. Custom (user-built) modes surface their NNome
//   5. PromptAssembler sempre inclui o pinned block (injection-escaped),
//      and pula it quando o legacy modeContext caminho já carries it.
//   6. Retrieval com excludeCustomContext=true Retorna reference-file snippets
//      apenas (não duplicate custom-context sofonte
//
// Uses o mesmo stub-the-singleton pattern como ModesManager.test.mjs (não SQLite).
...
/** Point o singleton's getActiveMode at a fixed modo linha (mesmo pattern como
 *  ModesManager.test.mjs installDb) and invalidate o W1 info ccache */
function installActiveMode(mode) {
    const manager = ModesManager.getInstance();
    manager.getActiveMode = () => mode;
    // O PI v3 cache memoizes getActiveModeInfo — reinicia entre tests.
