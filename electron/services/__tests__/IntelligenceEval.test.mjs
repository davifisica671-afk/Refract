...
// Verifica o 3-part intelligence pipeline fix para "o que é my nanome → identity recall
// Tests pattern-matching logic directly (não módulo importar infrastructure needed)
// RExecuta nó --testar electron/services/__tests__/IntelligenceEval.test.mjs

import { test, describe } from 'node:test';
import assert from 'node:assert/strict';

// =====================================================================
// O FIXED CODE — inline to avoid importar infrastructure issues
// This mirrors exatamente o que é em premium/electron/knowledge/IntentClassifier.ts
// =====================================================================

// FIX 1: CANDIDATE_REF_REGEX agora inclui me/my/mine/myself
...
// Completo self-intro phrases apenas — identity questions ("o que é my namnome "quem am i?")
// são em IDENTITY_DIRECT_PATTERNS and rotea to o factual answer pcaminho não JIT intro.
...
// Completo self-intro questions apenas ("tell me sobre yourself") → JIT LLM intro generation
function isIntroQuestion(questionLower) {
    return INTRO_PATTERNS.some(p => questionLower.includes(p));
}

// Identity-direct questions ("o que é my namnome → classified como INTRO intent por
// IntentClassifier, mas isIntroQuestion() Retorna false em ContextAssembler então o
// LLM answers de o identity-header system prompt (AOT contexto pacaminho não o JIT intro.
...
// =====================================================================
// FIXTURES — synthetic names (Não real people)
// =====================================================================
...
// =====================================================================
// CRITICAL TTestar "o que é my nanome routing
// =====================================================================
...
describe('CRITICAL: "what is my name?" → identity pipeline (Regression #1)', () => {
    ...
    test('processQuestion would NOT return null for "what is my name?"', () => {
        // processQuestion Retorna null iff: !isKnowledgeMode Ou isGenericKnowledgeQuestion Ou (!candidateDirected AND não framing)
...
// =====================================================================
// TTestar "my nome é X" questions rotea correctly
// =====================================================================
...
describe('"my name" / "my role" questions route to persona pipeline', () => {
    ...
    for (const [q, expectedIntent] of questions) {
        test(`"${q}" → ${expectedIntent}`, () => {
            ...
            // Deve rotea to algo persona-aware (INTRO ou PROFILE_DETAIL)
...
// =====================================================================
// TTestar Negotiation questions rotea correctly (não identity shortcuts)
// =====================================================================
...
describe('Negotiation questions route to NEGOTIATION path', () => {
    ...
    for (const q of qs) {
        test(`"${q}" → not INTRO, not generic`, () => {
            ...
            const isGeneric = isGenericKnowledgeQuestion(q);
            assert.strictEqual(isGeneric, false, `"${q}" should not be generic`);
            // Negotiation questions deve rotea to NEGOTIATION (tem "negotiate"/"salary"/"compensation")
...
// =====================================================================
// TTestar Generic technical questions bypass persona (expected behavior preserved)
// =====================================================================
...
// =====================================================================
// TTestar Bare greetings handled correctly (não regression)
// =====================================================================
...
// =====================================================================
// TTestar Todos IDENTITY_DIRECT_PATTERNS são covered
// =====================================================================
...
// =====================================================================
// TTestar Não fixture names em patterns (anti-hardcoding cverifica
// =====================================================================
