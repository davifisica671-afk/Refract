...
// =====================================================================
// These tests assert o OLD ScreenUnderstandingService API (que
// branched em OCR-first vs vision and consumed `ocrText` directly).
// O serviço tem sido rewritten como a vision-first pipeline em o
// pivot. New coverage lives iem
//   electron/services/__tests__/VisionFirstScreenUnderstanding.test.mjs
// These legacy tests são retained para referência até that suite lands;
// they são skipped at runtime.
// =====================================================================
...
test('ScreenUnderstandingService marks technical interview screenshots for direct vision', async () => {
  ...
  const result = await service.understand(baseRequest({
    ...
    // Direct vision é apenas chosen quando an actual vision-capable provedor é configured;
    // caso contrário SUS precisa fall voltar to OCR com a warning em vez disso de silently lying.
