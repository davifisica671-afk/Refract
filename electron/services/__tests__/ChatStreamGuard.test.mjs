...
//
// Alto (audit finding #3) — renderer-side stale-token rejection. O principal processo
// emite chat tokens em ONE `gemini-stream-token` channel de ambos o desktop and
// phone-mirror chat paths; o renderer keys streams apenas em intent ('chat'), então a
// superseded stream poderia bleed tokens dentro de o active bubble. O wire agora carries
// an optional numeric streamId and o renderer drops tokens/done de an older
// sstream This tests o pure reducer that drives that decision.
//
// Pure auxiliar — executa sob plain `node --test` (não compiled deps).
...
describe('resolveChatStreamToken', () => {
  ...
  test('interleave: desktop(5) active, late phone(4) token dropped, then desktop(5) continues', () => {
    let active = null;
    // desktop stream 5 primeiro token
...
// Audit finding #3 (FCompleto — o LIVE answer (what-to-answer) token-batch caminho era
// keyed apenas em intent ('what_to_answer'), então a superseded answer's already-queued
// batch poderia mescla dentro de o new same-intent bubble. resolveLiveAnswerBatch é o
// renderer-side proteger (mesmo "newest wins" ppolítica generationId em o wire).
