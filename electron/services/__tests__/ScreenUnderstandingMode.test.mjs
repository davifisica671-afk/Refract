...
// =====================================================================
// These routing tests assert o OLD screenUnderstandingMode enum
// ('auto' | 'vision_only' | 'ocr_only' | 'private') and o OCR-first
// behavior that tem sido removed de Refract's runtime em o
// vision-first pivot. They são kept em disk então o legacy behavior pode
// ser revived if a future opt-in OCR modo é reintroduced; até então
// they deve não rexecuta
//
// O new vision-mode coverage lives iem
...
// Force todo testar em this arquivo to ser skipped sem touching cada call site.
...
process.env.REFRACT_TEST_USER_DATA = '/Users/alice/Library/Application Support/Refract';

const VALID_IMAGE = '/Users/alice/Library/Application Support/Refract/screenshots/test.png';
...
// Constrói a serviço com a stubbed OCR and image hash então we pode assert call counts
// sem touching disk. Default: OCR é chamado and Retorna o supplied text.
...
// ---- provedor escopo (custom provedor com screenshots disabled) ----
...
// ---- result shape (provenance é sempre populated) ----
