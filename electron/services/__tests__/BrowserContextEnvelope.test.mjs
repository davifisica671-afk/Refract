...
//
// Tests o desktop-side envelope sanitizer, o prompt formatter, and o
// privacy-safe telemetry builder. Pure modules de dist-electron (node:crypto
// oapenas — não electron stub needed.
//
...
describe('telemetry — privacy-safe', () => {
  ...
  test('event contains ONLY allowlisted fields — no raw content', () => {
    ...
    // exact char count precisa Não appear
  ...
  test('rejects a URL/title smuggled as platform', () => {
    ...
    assert.equal(ev.platform, undefined); // não a simples label → dropped
  });
