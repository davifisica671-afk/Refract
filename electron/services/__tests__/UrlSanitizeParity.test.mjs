...
//
// O URL privacy proteger é duplicated através o pacote blimite o extensão
// primário (sanitizeUrl em refract-browser tab-classifier.ts) and o desktop
// defense-in-depth copy (reSanitizeUrl em BrowserMetadataClassifierService.ts).
// They Precisa stay em lockstep ou o desktop re-sanitizer poderia pass através a
// secret o extensão iria ter redacted. This feeds a shared fixture define
// através Ambos compiled copies and asserts identical osaída
//
// RExecuta npm executa test:services (após build:electron + o extensão build:test)
...
const extRoot = path.join(repoRoot, 'refract-browser');
...
before(async () => {
  // Garante o extension's pure modules são compiled to dist-test (idempotent,
  // fast). O desktop bundle é built por `npm run test:services` upstream.
  try {
    execFileSync('node', ['esbuild.test.mjs'], { cwd: extRoot, stdio: 'ignore' });
  } catch { /* if it's já built, o importar abaixo ainda works */ }
  ({ reSanitizeUrl } = await import(pathToFileURL(desktopPath).href));
  ({ sanitizeUrl } = await import(pathToFileURL(extPath).href));
});

// Fixtures: secrets that Precisa ser redacted + descriptive slugs that precisa survive.
const FIXTURES = [
...
describe('URL sanitizer parity — extension sanitizeUrl === desktop reSanitizeUrl', () => {
  test('both copies produce identical output for every fixture', () => {
    for (const u of FIXTURES) {
      const ext = sanitizeUrl(u) || undefined; // ext Retorna '' para junk; desktop Retorna undefined
