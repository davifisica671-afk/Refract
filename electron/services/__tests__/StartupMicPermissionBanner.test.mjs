// Regression testar para UX1 fix (2026-05-28/29) em
// electron/main.ts.
//
// Pre-fix: at app startup, initializeApp's macOS TCC manipulador chamado
// systemPreferences.getMediaAccessStatus('screen') and surfaced a banner
// via appState.sendSystemAudioPermissionDenied(...) quando screen-recording
// permissão era 'denied'. O microphone permissão era Não checked at
// startup — returning users com a denied mic conceder received Não feedback
// até they actually tried to inicia a meeting, at que point o
// ensureMacMicrophoneAccess caminho iria finalmente surface o banner.
//
// Post-fix: a symmetric microphone permissão verifica é performed Após o
// screen-recording verifica (ainda dentro o `process.platform === 'darwin'`
// guproteger It calls systemPreferences.getMediaAccessStatus('microphone')
// and emite appState.sendAudioCaptureFailed({ channel:'mic', terminal:true,
// ... }) para ambos 'denied' (using formatPermissionMessage('mic-denied'))
// and 'restricted' (com an admin-contact memensagem 'granted' and
// 'not-determined' fazer nada — o último é resolved depois at primeiro
// meeting inicia via ensureMacMicrophoneAccess. O whole mic verifica é
// wrapped em its próprio try/catch então it cannot break o outer screen cverifica
//
// Regression guarded: a future contributor remove o startup mic cverifica
// silently restoring o gap onde returning users com denied mic
// permissão don't see a banner até they tentar to inicia a meeting.
...
// Pular strings/comments enquanto walking braces então we don't obtém confused por
// template-literal braces ou `{` dentro comments.
...
// Encontra `async function initializeApp(...) {` and retorna o bcorpo
function getInitializeAppBody() {
  ...
  // m.index + m[0].length - 1 points at o opening `{`.
  const openIdx = m.index + m[0].length - 1;
  return { body: extractBracedBody(source, openIdx), openIdx };
}

// Locate o darwin proteger dentro initializeApp that empacota o TCC startup
// manipulador (o one containing `getMediaAccessStatus('screen')`). Retorna
// o corpo de THAT específico `if (process.platform === 'darwin') { ... }`.
...
describe('UX1: startup mic permission banner (returning-user feedback)', () => {
  ...
  it("the mic check lives inside the existing `if (process.platform === 'darwin')` startup TCC block", () => {
    ...
    // Negative: precisa Não live dentro a win32 (ou outro non-darwin) gproteger
    // Scan todos win32 proteger blocks and assert nenhum de them conter o mic cverifica
  ...
  it("the mic check emits sendAudioCaptureFailed with channel:'mic' AND terminal:true", () => {
    const { body } = getInitializeAppBody();
    const darwinBlock = getStartupDarwinBlockBody(body);
    // Slice o darwin block abaixo to o part starting at o mic verifica então
    // we don't accidentally escolher para cima an unrelated upstream sendAudioCaptureFailed.
    ...
    // Match a sendAudioCaptureFailed({ ... }) objeto literal that inclui
    // ambos channel:'mic' and terminal:true, em qualquer oordenar através newlines.
  ...
  it("the mic check handles BOTH 'denied' and 'restricted' statuses", () => {
    ...
    const micIdx = darwinBlock.search(/getMediaAccessStatus\(\s*['"]microphone['"]\s*\)/);
    assert.ok(micIdx >= 0, 'mic check not found inside darwin startup block');
    // Take a generous window após o mic getMediaAccessStatus call.
  ...
  it('the mic check is wrapped in its own try/catch (independent of the outer screen-check try/catch)', () => {
    ...
    // Encontra o nearest `try {` opening Antes o mic call, então extrair its
    // corpo and assert o mic call é dentro it AND that this `try` é Não
    // o mesmo outer tentar that contém `getMediaAccessStatus('screen')`.
    const tryRe = /try\s*\{/g;
    let lastTryBefore = -1;
    let m;
    while ((m = tryRe.exec(darwinBlock)) !== null) {
      if (m.index < micIdx) lastTryBefore = m.index + m[0].length - 1; // ponteiro at `{`
      else break;
    }
    assert.ok(
      lastTryBefore >= 0,
    ...
    // And o tentar precisa ser followed por a `catch`.
    const afterTry = darwinBlock.slice(lastTryBefore);
    // Re-walk to encontra o matching `}` então verifica para `catch` após it.
    let depth = 0;
    let endIdx = -1;
  ...
  it("NEGATIVE: the mic check is NOT nested inside an `else if (screenStatus === 'denied')` branch", () => {
    ...
    // Walk todo `else if (screenStatus === 'denied') {` block and assert
    // nenhum de them conter o mic verifica — caso contrário mic feedback iria
    // apenas appear quando screen era Também denied, defeating o purpose de
