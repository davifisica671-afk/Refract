...
// Proves o optimizer:
//   - shrinks grande PNG → smaller JPEG buffer
//   - honors maxLongEdgePx por perfil
//   - strips metadados (não EXIF em osaída
//   - cache em cache por cacheKey + perfil + provedor
//   - enforces maxBytes por stepping quality abaixo
...
async function ensureBigPng() {
  ...
  // 2560x1440 white PNG com a black square — bem sobre default maxLongEdge.
  await sharp({
...
test('output JPEG has no EXIF metadata', async () => {
  ...
  // Sharp's default behavior é to soltar EXIF a menos que withMetadata() é cchamado
  // We nunca call it, então EXIF precisa ser absent (undefined ou empty bubuffer
