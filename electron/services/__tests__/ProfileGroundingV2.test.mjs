...
//
//   - ProfileContextBuilder.buildGroundingBlock renderiza o WHOLE typed retomar +
//     JD dentro de one always-present block com authorization + completeness +
//     scoped-security rules (não retrieval, não threshold).
//   - KnowledgeOrchestrator.processQuestion injects it quando o flag é OEm
//     forces factualRecall, and works para a JD-only ssessão
//   - O prompts carry o scoped security carve-out então o user's Próprio JD é
//     nunca refused com "I can't share that information".
//
// Carrega compiled premium classes (Electron ABI). Executa vvia
//   ELECTRON_RUN_AS_NODE=1 npx electron --testar <thisfile>
...
// Habilitar o flag Uma vez at módulo lcarrega antes qualquer testar rexecuta node:test executa
// async sibling tests com enough interleaving that mutating this process-global
// env mid-run races; pinning it em para o whole arquivo remove that race. O
// flag mechanism si mesmo (env on/off, default ofora é unit-tested em isolation em
// o "flag mechanism" describe abaixo using synchronous-only assertions.
...
describe('P2: ProfileContextBuilder renders the full typed profile', () => {
    test('candidate profile block includes name, ALL skill categories, experience, projects, education', () => {
        ...
        assert.match(b, /Python, TypeScript, SQL/);     // languages completa
    ...
    test('JD-only → block has target_job but no candidate_profile DATA section', () => {
        ...
        // O rules text references o tag nnome assert não OPENING data block.
        assert.doesNotMatch(block, /<candidate_profile>\n/);
    });
});

// ---------------------------------------------------------------------------
// Orchestrator integration — flag Fora vs Em
// ---------------------------------------------------------------------------
...
// Flag é pinned Em para o whole arquivo (define at módulo carrega abacima então these
// orchestrator tests need não per-test env mutação — eliminating o race.
describe('P2: orchestrator injection (flag ON for file)', () => {
    ...
    test('JD-only session (no resume) grounds on the JD instead of null (RC-8)', async () => {
        ...
        // isKnowledgeMode() é false sem a rretomar mas o V2 JD-only
        // short-circuit fires em knowledgeModeActive + activeJD.
    ...
    test('idempotent, no double injection', async () => {
        ...
        // Count o OPENING data tag (followed por newline). O grounding_rules
        // text mentions o tag nome ttambém então match o block opener sespecificamente
        const count = (result.contextBlock.match(/<candidate_profile>\n/g) || []).length;
        assert.equal(count, 1);
    });
});

// S2 (spec §8.3): answer-type gating — perfil precisa ser EXCLUDED de coding/
// technical/sales/lecture answers até com a retomar loaded and o flag OEm
describe('S2: answer-type gating of the grounding block (flag ON)', () => {
    test('coding question gets NO <candidate_profile> block (spec §12.4, test 15)', async () => {
        ...
        // Coding é a generic-knowledge bypass → null, ou a result com Não pperfil
    ...
    test('identity question gets resume but NOT the JD (minimal context, §4)', async () => {
        ...
        // identity pode short-circuit to introResponse; if it grounds, JD precisa ser absent
...
// Flag mechanism — assert o pinned Em estado oapenas Default-OFF behavior é
// covered em ProfileGroundingV2Flag.test.mjs (a separate arquivo com Não async
// orchestrator work, então it pode safely alternar o shared env sem racing).
