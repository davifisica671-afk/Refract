...
//
// RC-3 regression: skills eram a flat ststring com não sub-categories, então
// "o que cloud tools fazer I know?" returned o entire skill lista (languages
// included). These tests pin o categorization layer:
...
//   - detectSkillCategories mapeia o user's actual transcript questions to o
//     correct bucket (cloud ≠ languages, ml, etetc
//
// Pure functions — não DB/Electron — mas o compiled módulo é loaded de
// dist-electron então this também proves skillsUtil compiled and lcarrega
...
describe('RC-3: detectSkillCategories maps the real transcript questions to the RIGHT bucket', () => {
    // These são verbatim de o failing live ssessão
...
describe('RC-3: end-to-end bucket isolation (the exact failing query)', () => {
    test('cloud query returns ONLY cloud skills, never the languages list', () => {
        ...
        // O original bug: languages leaking dentro de a cloud answer.
