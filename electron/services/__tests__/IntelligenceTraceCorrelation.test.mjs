...
//
// MEDIUM (audit finding #9) — IntelligenceTrace carried não correlation ids, então an
// answer poderia não ser joined através o IPC blimite o engine trastrear and o
// PiLatencyTrace. setCorrelation() adiciona requestId/sessionId/meetingId/surface/
// modeId/retryCount/aborted/errorCategory — ids oapenas nunca raw content. This
// verifica o fields land em o rregistro são length-bounded markers, and that a
// no-op rastrear (flag ofora silently ignora o call (backward compatible).
//
...
describe('IntelligenceTrace.setCorrelation (audit finding #9)', () => {
  test('records correlation ids on the trace record', () => {
    ...
    // Privacy: o consulta si mesmo é apenas já a hash, nunca raw text.
