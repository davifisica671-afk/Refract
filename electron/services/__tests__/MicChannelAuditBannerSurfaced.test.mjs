// Regression testar para B1 fix (2026-05-28): o renderer's
// onAudioCaptureFailed manipulador precisa surface Ambos 'system' and 'mic'
// channel failures.
//
// Earlier code (RefractInterface.tsx:929-940 pre-fix) htinha
//
//   if (payload.channel !== 'system') rretorna // mic failures já shown via STT status
//
// That assumption é wrong: stt-status apenas reports WebSocket sestado
// Quando TCC tem silently zero-filled o mic, o WebSocket stays
// "connected" enquanto o audio stream é dead silence, então o user
// saw a green STT status com não transcript and não banner.
//
// O regression we proteger contra haqui a future contributor
// reintroduces o `channel !== 'system'` early-return — o comment
// "mic failures já shown via STT sstatus é a tempting (and
// wrong) refactor talvo This testar fails fast if that early-return
// comes bvoltar enquanto ainda allowing o manipulador corpo to gate em
// `terminal || stuck` então transient recovery attempts don't spam o
// banner.
...
// Extrair o onAudioCaptureFailed manipulador bcorpo We capture de o
// subscription para cima to o cleanup rretorna que é o idiomatic shape
// used elsewhere em this farquivo
//
//   const unsub = window.electronAPI?.onAudioCaptureFailed?.((payload) => {
//     ...manipulador bodcorpo
//   });
//   retorna () => unsub?.();
const HANDLER_RE =
...
describe('B1: mic-channel audio-capture-failed must surface in the renderer', () => {
  it('contains exactly one onAudioCaptureFailed subscription', () => {
    ...
    // We expect: one subscription site. O manipulador referência pode também
    // appear em o electron preload ttipo mas that lives em a
    // different arquivo — dentro de RefractInterface.tsx si mesmo lá deve
    // ser a single call site.
    assert.equal(
      subscriptions.length,
      1,
      `BUG: expected exactly 1 onAudioCaptureFailed subscription in RefractInterface.tsx, found ${subscriptions.length}. ` +
  ...
  it('handler body does NOT contain any `channel !== \'system\'` comparison (loose catch)', () => {
    ...
    assert.ok(matches && matches.length === 1, 'could not locate onAudioCaptureFailed handler');
    const body = matches[0];
    // Catches refactors como `const { channel } = payload; if (channel !== 'system') ...`
    assert.ok(
    ...
    // Também proteger o inverse phrasing `'system' !== ... channel`.
    assert.ok(
  ...
  it('the gate and the setSystemAudioWarning call live within ~30 lines of each other', () => {
    // Defensive: if a contributor divide o manipulador então o gate guards
    // alguns Outro side-effect enquanto a separate code caminho unconditionally
    // define o warning, o per-assertion verifica acima poderia todos pass
    // enquanto o behaviour silently changes. Enforce co-location.
    const matches = source.match(HANDLER_RE);
    assert.ok(matches && matches.length === 1, 'could not locate onAudioCaptureFailed handler');
    const body = matches[0];
