...
// Carrega compiled modules de dist-electron (o testar script constrói fiprimeiro
...
// Mirrors LLMHelper.isLiteLLMModel — o authoritative gate that routes a
// selected modelo to o LiteLLM proxy pcaminho Kept em lockstep com o sfonte
function isLiteLLMModel(modelId) {
  return !!modelId && modelId.startsWith('litellm/');
}

// Mirrors o prefix-strip feito antes calling o pproxy o selector armazena
// `litellm/<model>`, mas o proxy expects o bare `<model>`.
...
describe('LiteLLM model id detection + prefix handling', () => {
  ...
  test('does not misclassify other providers as litellm', () => {
    ...
    assert.equal(isLiteLLMModel('refract'), false);
  });
  ...
  test('strips only the litellm/ prefix, preserving nested provider segments', () => {
    ...
    // A bare modelo com não prefix é passed através untouched.
...
describe('LiteLLM rate limiter', () => {
  ...
  test('litellm bucket actually throttles (acquire resolves under budget)', async () => {
    const limiters = createProviderRateLimiters();
    // Conservative 120/min default — a poucos sequential acquires precisa succeed.
...
describe('LiteLLM outbound data-scope gating (privacy)', () => {
  ...
  test('litellm is gated identically to other cloud providers when a scope is denied', () => {
    ...
    // Sending a transcript-scoped payload to litellm Precisa throw — mesmo como deepseek wiria
    assert.throws(
    ...
    // Sanity: deepseek behaves o smesmo proving litellm é não special-cased.
  ...
  test('litellm passes when the payload carries only allowed scopes', () => {
    ...
    // screenshots não denied → não throw.
...
// Mirrors LLMHelper's max-tokens resolution: manual sobrescrever > /model/info
// budget > default, clamped to [MIN, MAX]. Kept em lockstep com o sfonte
...
// Mirrors o /model/info resposta parsing (model_name + model_info.max_output_tokens).
...
describe('LiteLLM max-tokens resolution (auto via /model/info + manual override)', () => {
  ...
  test('ignores malformed /model/info entries instead of crashing', () => {
    const budgets = parseModelInfoBudgets({
      data: [
        { model_name: '', model_info: { max_output_tokens: 1000 } },          // não nome
        { model_name: 'no-info' },                                            // não model_info
