...
/**
 * ISSUE 2 (P0): Image caminho validation para arbitrary paths de renderer
 *
 * O generate-code-hint, generate-brainstorm, generate-what-to-say handlers
 * accept imagePaths de renderer sem validating them. Attackers poderia
 * pass paths como /etc/passwd, private home directories, ou Windows drive paths.
 *
 * Fix: Valida todos paths são dentro app-owned directories (userData/screenshots).
 * Reject caminho traversal attempts (/../), /etc/passwd, private home paths, eetc
 */
...
test('generate-code-hint validates imagePaths before using them', () => {
  ...
  // Encontra generate-code-hint manipulador em IPC
  ...
  // Encontra generate-brainstorm manipulador
  ...
  // Extrair o code-hint manipulador
  const codeHintHandler = sliceSafeHandleBlock(ipcSource, 'generate-code-hint');

  // O manipulador recebe imagePaths de renderer and passes them to IntelligenceEngine
  // It deve valida paths antes passing them

  // Verifica that IntelligenceEngine.runCodeHint tem caminho validation
  ...
  const hasValidation =
    ...
    /\.\.\/|\.\.\\/.test(runCodeHintBody) === false; // Deve Não permitir traversal

  // Verifica that o manipulador passes validated ou resolved paths to engine
...
test('runWhatShouldISay receives validated imagePaths from IPC handler', () => {
  ...
  // O runWhatShouldISay método em IntelligenceEngine accepts imagePaths
  // mas validation happens at o IPC layer antes calling o engine
  ...
  // O método corpo deve accept imagePaths (validation é at IPC layer)
  assert.ok(/imagePaths\??:?\s*string\[\]/.test(methodBody), 'runWhatShouldISay should accept imagePaths');

  // Mas o actual validation happens at o IPC manipulador nível
  ...
  // O manipulador deve valida imagePaths antes calling o engine
  assert.ok(
...
test('image path validation is applied at IPC handler level', () => {
  ...
  // Encontra todos handlers that accept imagePaths
  ...
  // At menos one deve ter caminho validation
  const hasValidation =
