...
// Comprehensive intelligence pipeline evaluation harness — 200+ cases através 10 profiles
// RExecuta nó --testar electron/services/__tests__/IntelligenceEvalComprehensive.test.mjs
...
// =====================================================================
// PRODUCTION CLASSIFIER LOGIC — inlined de
//   premium/electron/knowledge/IntentClassifier.ts
// Keep em sincronizar com that arquivo eexatamente
// =====================================================================
...
// Replicates o scoring logic de IntentClassifier.ts classifyIntent()
...
// Todos pattern constants joined para anti-hardcoding audit
...
for (const fixture of allFixtures) {
  describe(`[${fixture.id}] ${fixture.candidate.name} — ${fixture.role}`, () => {
    ...
    // Todos 5 identity questions precisa rotea to INTRO (não GENERAL),
    // and precisa Não ser classified como generic knowledge.
    // -----------------------------------------------------------------
    ...
    // -----------------------------------------------------------------
    // CATEGORY 2: Retomar recall routing
    // Precisa rotea to PROFILE_DETAIL ou INTRO, nunca GENERAL.
    // -----------------------------------------------------------------
    ...
    // Precisa Não ser generic. Intent pode ser qualquer persona-aware rotea
    // incluindo GENERAL quando first-person framing aciona ref regex.
    // O chave requirement: isGenericKnowledgeQuestion precisa ser false.
    // -----------------------------------------------------------------
    ...
    // -----------------------------------------------------------------
    // CATEGORY 4: Custom contexto adherence
    // A custom-context question como "Give me a concise answer" deve
    // não ser classified como generic knowledge, então persona wrapping rexecuta
    // -----------------------------------------------------------------
    ...
    // Ambos questions conter forte negotiation keywords.
    // Precisa rotea to NEGOTIATION and precisa Não ser generic.
    // -----------------------------------------------------------------
    ...
    // Pure technical definitional questions com Não first-person framing
    // deve ser classified como generic (bypass persona) — this é correct
    // behavior. Confirm o bypass logic works consistently.
    // -----------------------------------------------------------------
    ...
    // O candidate's nome and alvo company precisa Não appear dentro
    // qualquer pattern cconstante This proves o system é data-driven.
    // -----------------------------------------------------------------
  ...
  }); // termina fixture describe
} // termina for-fixture loop

// =====================================================================
// CROSS-FIXTURE INVARIANT TESTS
// (These testar properties that precisa hold através Todos fixtures tojuntos
// =====================================================================
...
describe('Cross-fixture invariants', () => {
  ...
  test('All identity questions are blocked from generic bypass (via regex OR identity pattern)', () => {
    // isGenericKnowledgeQuestion guards com two gates:
    //   Gate 1: CANDIDATE_REF_REGEX (me/my/mine/you/your/…)
    //   Gate 2: IDENTITY_DIRECT_PATTERNS (explicit phrase llista
    // Qualquer um gate é suficiente — we fazer Não exigir todo identity question
    // to conter a pronoun de Gate 1.
    for (const q of IDENTITY_QUESTIONS) {
      const isGeneric = isGenericKnowledgeQuestion(q);
      assert.strictEqual(
        isGeneric,
  ...
  test('Pattern constants contain no fixture company names', () => {
    ...
    for (const company of companies) {
      // Pular muito curto ou muito generic tokens
...
// =====================================================================
// Salva RESULTS TO iteration-002.json
// =====================================================================
