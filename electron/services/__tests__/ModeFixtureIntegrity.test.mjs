...
//
// Smoke testar para o fixture data isi mesmo todo sentinel declared em
// referenceFileFactory.mjs precisa actually appear (verbatim) em at menos one
// referência arquivo sob tests/fixtures/modes/<mode>/. This previne o
// scenario tests de passing-by-accident if a fixture é renamed ou its
// content é rewritten em a way that drops o sentinel.
...
describe('Fixture integrity — every declared sentinel exists in at least one fixture file', () => {
  ...
  // O `custom/` diretório é a meta-folder containing per-custom-mode
  // subfolders; it é exercised por CustomModes.test.mjs and é não si mesmo a
  // modo fpasta Pular it haqui
