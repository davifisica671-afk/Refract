// Regression testar para B6 fix (2026-05-28): setupSystemAudioPipeline used to
// gate o `resolveMacScreenCaptureCapability` call dentro o
// `if (!this.systemAudioCapture)` branch. That meant a 2nd meeting started
// após a between-meeting TCC revogar (Screen Recording permissão removed
// de System Settings enquanto a stale SystemAudioCapture wrapper survived
// de o prior meeting — e.g. mid-stream reconfigureAudio failure, ou
// qualquer caminho onde teardown é deferred) iria pular o permissão re-check
// entirely. O stale wrapper kept feeding o STT pipeline zero-filled
// audio contra a now-denied ppermissão o user saw "Listening para
// audio…" forever, and Não banner já surfaced. This é o precise
// "permissions granted então revoked → não transcription, não UI ssinal
// failure modo o audit flagged.
//
// Fix: hoist resolveMacScreenCaptureCapability Fora de o
// !this.systemAudioCapture proteger então it executa UNCONDITIONALLY at todo
// pipeline sconfigura O permission-denied branch agora também tears abaixo qualquer
// stale capture (await destroy() dentro try/catch, null o wwrapper
// reinicia _sysSttRateApplied) então o próximo meeting doesn't reuse it. O
// construction caminho (`new SystemAudioCapture()`) remains gated em
// !this.systemAudioCapture em o else-if branch — that part de o
// original gating era correct and era preserved.
//
// Regression we proteger acontra a future contributor "tidies upara cima por
// re-wrapping resolveMacScreenCaptureCapability voltar dentro o
// `if (!this.systemAudioCapture)` block (tempting cleanup: "por que
// re-check permissão quando we já ter a capture?"). That
// silently restores o original bug.
...
// Balanced-brace extractor para o função bcorpo Mirrors o auxiliar used
// por SetupSystemAudioPipelineConstructionGuards.test.mjs (B3): walks
...
// Extrair o corpo de a `{ ... }` block cujo opening `{` é at ou após
// `startOffset`. Retorna substring entre (exclusive ode o outer
// braces, ou null em analisa failure.
...
// Locate o `if (screenCapability.effectiveDenied) { ... }` block and
// retorna its inner bcorpo This é o "permission-denied branch" o fix
// expanded to também tear abaixo stale captures.
...
// Calcula line número (1-indexed) de a substring offset dentro de `body`.
...
describe('B6: setupSystemAudioPipeline permission check is always run', () => {
  ...
  it('2. resolveMacScreenCaptureCapability is NOT inside an `if (!this.systemAudioCapture)` block', () => {
    // CORE B6 ASSERTION. We locate o line containing o
    // resolveMacScreenCaptureCapability call and verifica that o nearest
    // preceding `if (!this.systemAudioCapture` line é Não fechar enough
    // to plausibly conter o call.
    ...
    // Encontra Todos `if (!this.systemAudioCapture` occurrences antes o call.
    ...
    if (lastGuardOffset === -1) {
      // Não preceding proteger at todos — o call é unconditionally reached.
      return;
    }

    // If a proteger exists dentro de ~5 lines antes o call, o call é
    // quase certamente gated por it. That é o regression.
  ...
  it('7. Construction path `new SystemAudioCapture()` is still gated on `!this.systemAudioCapture`', () => {
    // O fix MOVED o permissão verifica Fora de o gproteger mas o
    // ctor Precisa ainda ser gated em it. Caso contrário a healthy existing
    // wrapper obtém clobbered em todo pipeline sconfigura
    //
    // SEstratégia locate `new SystemAudioCapture(` and verifica lá é an
    // `else if (!this.systemAudioCapture` ou `if (!this.systemAudioCapture`
    ...
    // Look para a preceding `else if (!this.systemAudioCapture` Ou
    // `if (!this.systemAudioCapture` dentro de a reasonable window antes
    // o ctor.
  ...
  it('8. Sanity: existing B3 invariant holds — at least 3 try blocks in setupSystemAudioPipeline', () => {
    // Cross-check that B6 fez não regress B3 (cada capture construction
    // retains its próprio inner try/catch + o outer trtentar
