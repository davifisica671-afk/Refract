...
// ─────────────────────────────────────────────────────────────────────────────
// Testar 1: Async post-call summary uses o modo that era active quando meeting stopped,
//         não o modo that happens to ser active quando async processing rexecuta
// ─────────────────────────────────────────────────────────────────────────────
...
describe('BUG-MODE-BLEEDING: Async post-call summary mode snapshot', () => {
  test('stopMeeting snapshots active mode before session.reset() is called', () => {
    ...
    // Extrair apenas o stopMeeting função corpo
    ...
    // O modo snapshot capture precisa occur Antes o Principal session.reset() —
    // o one that executa após todos snapshots and antes o async background
    // processing call. Early-exit branches (duration<1000ms) and privacy-
    // gate branches (Fase 9 "fazer não persist") cada ter their próprio rereinicia
    // antes they `return null;`, então we cannot rely em positional indices
    // ("fiprimeiro "sesegundo "third"). Encontra Todos reinicia that PRECEDE o line
    // that calls `this.processAndSaveMeeting(...)` — o último one de those
    // é o Principal rreinicia and modeSnapshot precisa come antes it.
    ...
    // Encontra o modo snapshot variável declaration
    ...
    // modeSnapshot precisa ser declared antes o Principal reinicia (o one
    // imediatamente preceding processAndSaveMeeting).
  ...
  test('stopMeeting passes modeSnapshot to processAndSaveMeeting', () => {
    ...
    // processAndSaveMeeting call precisa incluir modeSnapshot
  ...
  test('processAndSaveMeeting accepts modeSnapshot parameter', () => {
    ...
    // Encontra o função signature - extrair enough chars to capture todos parameters
    ...
    // Precisa ter modeSnapshot parâmetro (4th parâmetro após meetingId and mmetadados
    assert.ok(fnSig.includes('modeSnapshot'), 'processAndSaveMeeting must accept modeSnapshot parameter');
    // modeSnapshot é optional então it appears como modeSnapshot? ou apenas part de o tipo union
  ...
  test('processAndSaveMeeting uses snapshotted mode ID for section lookup', () => {
    ...
    // Encontra o processAndSaveMeeting função corpo
    const fnStart = source.indexOf('private async processAndSaveMeeting(');
    ...
    // O mode-section loading block precisa referência modeSnapshot.id ou targetModeId
    // não apenas call getActiveMode() directly para section lookups
...
// ─────────────────────────────────────────────────────────────────────────────
// Testar 2: setActiveMode limpa sessão contexto antes switching modes
// ─────────────────────────────────────────────────────────────────────────────
...
describe('BUG-MODE-BLEEDING: Mode-context clearing on mode switch', () => {
  test('modes:set-active IPC clears session context before calling setActiveMode', () => {
    ...
    // Encontra o modes:set-active manipulador
    ...
    // Precisa call clearSessionContext antes setActiveMode
...
// ─────────────────────────────────────────────────────────────────────────────
// Testar 3: Modo lifecycle tracking (snapshot captures modo id, nnome templateType)
// ─────────────────────────────────────────────────────────────────────────────
...
describe('BUG-MODE-BLEEDING: Mode snapshot captures required fields', () => {
  test('stopMeeting mode snapshot includes id, name, and templateType', () => {
    ...
    // Encontra o modeSnapshot assignment dentro de stopMeeting função
    const stopStart = source.indexOf('public async stopMeeting');
