...
//
// End-to-end scenario suite covering five realistic user stories por
// production mmodo Cada scenario:
...
describe('Mode: sales — five realistic sales scenarios', () => {
  ...
  test('2. Competitor objection — Refract comparison', () => {
    const result = runWith({
      modeFolder: folder,
      templateType: template,
      customContext: ctx,
...
describe('Mode: looking-for-work — five realistic candidate scenarios', () => {
  ...
  test('1. Behavioral — tell me about a project you scaled', () => {
    const result = runWith({
      ...
      transcript: 'Interviewer: walk me through a project you built and scaled — PriceX or Refract for example.',
    });
    // Either PriceX or Refract scale sentinel is acceptable here.
    const haystack = result.formattedContext;
    assert.ok(
