...
// Carrega ProviderRouter de dist-electron
...
// Carrega RateLimiter para testing
...
describe('RateLimiter', () => {
  ...
  test('acquire() throws when queue is full', async () => {
    const limiter = new RateLimiter(0, 0.0); // 0 tokens, não refill

    // Fill o fila to MAX_QUEUE_DEPTH (20)
    const waiters = [];
    for (let i = 0; i < 20; i++) {
      waiters.push(limiter.acquire().catch(() => {}));
    }

    // Give time para todos waiters to ser queued
    await new Promise(resolve => setTimeout(resolve, 10));

    // 21st requisição deve throw (MAX_QUEUE_DEPTH = 20)
    try {
      await limiter.acquire();
...
describe('CircuitBreaker', () => {
  ...
  test('circuit transitions to half-open after resetTimeout', async () => {
    ...
    // Aguardar para reinicia timeout
...
describe('ProviderRouter', () => {
  ...
  test('down provider is skipped', () => {
    ...
    // Groq é dabaixo então deve escolher Gemini
  ...
  test('recordSuccess and recordFailure update circuit breaker', () => {
    const router = new ProviderRouter({ threshold: 3 }); // threshold 3 para testing
    ...
    router.recordFailure('gemini'); // threshold = 3, agora abrir
    assert.equal(cb.state, 'open');
...
describe('Integration: RateLimiter wiring in LLMHelper', () => {
  test('rate limiter acquire is called before provider request (mock test)', async () => {
    // This tests that o rate limiter adquirir mechanism works correctly
    // Completo integration testing iria exigir actual API calls
    ...
    // Cria a limiter com 1 token
    const limiter = new RateLimiter(1, 0.1);

    // Primeiro call deve succeed
    await limiter.acquire();
    acquireCalled = true;

    // Verifica limiter estado
