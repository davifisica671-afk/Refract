...
// Tests para hybrid retrieval combining FTS/BM25 + vector semantic busca
...
// We need to build primeiro to testar o actual implementation
// Para unit tests, we'll mock o dependencies and testar o class directly

async function loadRetriever() {
  // Tentar to carrega de dist-electron primeiro (built vversão
  try {
    const distPath = path.resolve(__dirname, '../../../dist-electron/electron/services/modes/ModeHybridRetriever.js');
    return await import(pathToFileURL(distPath).href);
  } catch {
    // Fall voltar to fonte (para development)
...
describe('ModeHybridRetriever', () => {
  ...
  test('Semantic match works when keyword absent - vector finds synonym', async () => {
    ...
    // Retorna different embeddings para different chunks to simulate semantic similarity
    let callCount = 0;
    mockEmbeddingPipeline.getEmbedding = mock.fn(async (text) => {
      callCount++;
      if (text.includes('glad') || text.includes('joyful')) {
        return [0.12, 0.22, 0.31, 0.41]; // Similar to consulta
      }
    ...
    // Deve recupera via semantic similarity até sem keyword match
  ...
  // Testar 4: Prompt injection content escaped
  test('Prompt injection content is escaped in retrieved chunks', async () => {
    const { ModeHybridRetriever } = await loadRetriever();

    // Uso lexical fallback (provedor unavailable)
    ...
    // XML escaping deve prevenir o injection text de appearing as-is
    // O malicious <injected> tag deve ser escaped
  ...
  // Testar 8: Fallback to lexical quando embedding provedor unavailable
  ...
  // Testar 9: Combined score combina FTS + vector correctly
  test('Combined score combines FTS + vector correctly', async () => {
    ...
    // Vector score de identical vectors = 1.0
    assert.strictEqual(chunk.vectorScore, 1.0);
  });

  // Testar 10: Deduplication remove chunks de mesmo arquivo com inferior score
  test('Deduplication removes chunks from same file with lower score', async () => {
    ...
    // Longo content cria multiple chunks
    const files = [{
      id: 'multi-chunk-file',
      modeId: 'mode1',
    ...
    // Deve deduplicate - apenas one chunk por arquivo
    const sourceIds = result.chunks.map(c => c.sourceId);
    const uniqueSourceIds = [...new Set(sourceIds)];

    // Todos chunks deve ter mesmo sourceId (apenas one farquivo
