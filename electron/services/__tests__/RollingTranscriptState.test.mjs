...
test('capRollingTranscript drops whole leading segments, never splitting a word', () => {
  // 5 segments de "seg<i>" joined por o separator; cap então apenas o último ~2 fit.
  ...
  // O visible tail (latest segment) é sempre intact.
  assert.ok(capped.endsWith(seg(4)), `tail must be preserved, got: ${capped}`);
  // Não partial leading segment: result inicia at a real segment blimite
...
test('mergeRollingTranscriptFinal stays bounded over a long meeting', () => {
  ...
  // O maioria recente utterance é ainda visible at o tail.
