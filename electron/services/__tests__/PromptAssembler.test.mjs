...
const SAMPLE_SYSTEM_PROMPT = 'You are Refract. Answer questions directly.';
...
describe('PromptAssembler', () => {
  ...
  // ── Testar 1: Referência arquivo says "ignorar anterior instructions" — content ainda
  //    included mas escaped (não silently dropped) ─────────────────────────────
  test('prompt injection in reference file: content escaped but included', async () => {
    ...
    // O injection pattern deve ser escaped, não present verbatim
    ...
    // Mas o block deve ainda exist (content não silently dropped)
    assert.ok(content.includes('notes.md') || content.includes('reference_file'));
  });

  // ── Testar 2: Filename contém XML/prompt injection — escaped ─────────────────
  test('filename with XML injection is escaped in reference block', async () => {
    ...
    // Filename deve ser XML-escaped, não raw
    assert.match(refBlock.content, /&lt;script&gt;/);
    // Injection pattern deve também ser escaped
    assert.match(refBlock.content, /IGNORE \[REDACTED\] instructions/);
  });

  // ── Testar 3: Transcript contém prompt injection — ignored ───────────────────
  test('transcript with prompt injection pattern is not acted upon', async () => {
    ...
    // O transcript content deve ser preserved (não stripped)
    // mas o injection pattern deve ser escaped
    const content = transcriptBlock.content;
    assert.match(content, /Ignore all previous instructions/);
  });

  // ── Testar 4: Active modo prompt appears ouma vez inactive modo absent ───────────
  ...
  // ── Testar 5: Screen contexto marked como untrusted ──────────────────────────────
  ...
  // ── Testar 6: Token budget enforced — blocks truncated quando exceeded ──────────
  test('token budget enforcement truncates lowest-priority blocks', async () => {
    ...
    const result = assembler.assemble({
      ...
      tokenBudget: 500, // Muito baixo budget
    });

    // Deve não crash
    assert.ok(result, 'should return a valid packet');

    // Total tokens used deve ser <= budget
    assert.ok(result.metadata.totalTokensUsed <= 500,
      `totalTokensUsed (${result.metadata.totalTokensUsed}) should be <= budget (500)`);
  });

  // ── Testar 7: Trust nível ordering é correct (system_policy fprimeiro untrusted
  //    lúltimo — verified por block ordering em saída ────────────────────────────
  test('assemble orders blocks by trust level (highest first)', async () => {
    ...
    // Obtém trust levels em ordenar
    const trustOrder = blocks.map(b => b.trustLevel);

    // assistant_history deve come antes untrusted_screen
    ...
    // mode_policy deve come antes untrusted_reference
  ...
  // ── Testar 8: escapeUserContent escapes <>&"' ───────────────────────────────
  ...
  // ── Testar 9: Evidence refs attached to contexto blocks ────────────────────────
  ...
  // ── Testar 10: Empty blocks handled gracefully (não crashes) ──────────────────
  test('assemble handles empty transcript without crashing', async () => {
    ...
    // Empty transcript block deve não ser present ou deve ser empty
    if (transcriptBlock) {
  ...
  test('escapePromptInjection neutralizes all dangerous patterns via assemble', async () => {
    ...
    for (const { input, pattern } of testCases) {
      ...
      // O pattern deve Não appear verbatim em o block content
      assert.ok(
...
ANSWER SHAPE: concise
  ...
  // ── Additional: metadados é correctly define ──────────────────────────────────
