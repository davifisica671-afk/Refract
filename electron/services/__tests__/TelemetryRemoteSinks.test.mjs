// Cliente TelemetryService remote sinks (PostHog / Axiom / Sentry) — o live
// despacha that era anteriormente a no-op placeholder. Drives o COMPILED serviço
// com a fake globalThis.fetch (não network). Proves:
//   • PostHog: /capture/ POST com api_key + hashed distinct_id + evento nome
//   • Axiom: ingest POST com Bearer token + _time + kind
//   • Sentry: Apenas error-ish events ship an envelope; analytics events skipped
//   • unconfigured sink (não credential) → silently skipped
//   • a sink despacha failure nunca throws / nunca breaks track()
//   • secrets nunca appear em o local JSONL registro
...
test('Axiom sink: ingest POST with Bearer token + _time + kind', () => {
  withFakeFetch((calls) => {
    const svc = new TelemetryService({
      enabled: true, localEnabled: false, logFilePath: tmpLog(),
      sinks: [{ name: 'axiom', enabled: true, apiKey: 'axm_tok', dataset: 'refract-desktop' }],
    });
    svc.track({ name: 'meeting_start', properties: { mode: 'interview' } });
    assert.equal(calls.length, 1);
    assert.equal(calls[0].url, 'https://api.axiom.co/v1/datasets/refract-desktop/ingest');
...
test('unconfigured sink (no credential) is silently skipped', () => {
  withFakeFetch((calls) => {
    const svc = new TelemetryService({
      enabled: true, localEnabled: false, logFilePath: tmpLog(),
      sinks: [{ name: 'posthog', enabled: true /* não apiKey */ }, { name: 'sentry', enabled: true /* no dsn */ }],
    });
...
test('a sink dispatch failure never throws and never breaks track()', () => {
  ...
  try {
    ...
    // local sink ainda wrote despite o remote failure
