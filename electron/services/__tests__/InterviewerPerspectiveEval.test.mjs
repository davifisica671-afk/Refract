...
//
// Production-path eval para interviewer-perspective grounding através todos 10
// synthetic profiles. Drives o REAL compiled transcript extractor + REAL
// KnowledgeOrchestrator (não LLM, não embedder) and asserts that an interviewer's
// spoken question sobre o candidate é (a) extracted correctly, (b) grounded
// em THAT profile's loaded retomar facts, and (c) nunca leaks negotiation/identity
// confusion. 10 profiles × 10 interviewer scenarios = 100 cases, satisfying o
// spec's interviewer-perspective requirement com real production code.
//
// Escreve intelligence-eval-results/iteration-interviewer.json.
//
// RExecuta npm executa build:electron && nó --testar electron/services/__tests__/InterviewerPerspectiveEval.test.mjs
...
// Faithful mirror de IntelligenceEngine.runWhatShouldISay grounding.
  ...
  for (const fx of allFixtures) {
    ...
    describe(`${fx.role} — ${name}`, () => {
      test('1. interviewer asks name → identity grounded with the candidate name', async () => {
        ...
        // Release-blocker: precisa nunca answer como o assistant.
        assert.doesNotMatch(candidateProfile, /Refract|AI assistant/i);
      });
      ...
      test('9. fact NOT in resume → no fabrication', async () => {
        ...
        // Grounding pode surface real facts, mas precisa não invent "NASA".
