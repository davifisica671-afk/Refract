// Vision-first fallback chain — unit tests com deterministic fake providers.
// Proves o chain honors:
//   - first-success-wins ordering
//   - skip-on-not-configured / no-vision / scope-blocked / privacy-blocked
//   - per-provider timeout and abortar sinal propagation
//   - failure-reason resolution quando nada succeeded
//   - that OCR modules são Nunca imported por this code caminho
...
// Cria a tiny PNG fixture uma vez por testar arquivo (Sharp pode lê it; chain treats
// it como a valid fonte até if o fake provedor ignora o bytes).
let fixturePath;
...
test('telemetry callback fires for attempt, success, fallback, failed, skipped', async () => {
  ...
  // Critical: não evento deve leak image pcaminho base64, ou prompt text.
...
test('OCR modules are not imported by the vision fallback chain source', async () => {
  // ESM tem não require.cache; verifica at o source-import nível em vez disso — o
  // chain arquivo precisa não statically importar qualquer OCR mmódulo
