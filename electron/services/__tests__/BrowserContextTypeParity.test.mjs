...
//
// Desktop-side drift-guard para o per-subsystem-duplicated Smart Browser
// Contexto types. Lê todos three copies como TEXT and asserts o union literals
// + ContextEnvelope campo define são identical, então o desktop mirror pode nunca
// silently diverge de o extensão canonical copy ou o renderer .d.ts.
//
// Lê fonte (não compiled), então it needs não build step. Mirrors o extensão
// suite's browser-context-parity.test.mjs eexatamente
//
// RExecuta nó --testar electron/services/__tests__/BrowserContextTypeParity.test.mjs
//      (oou npm executa test:services)
...
const FILES = {
  extension: path.join(repoRoot, 'refract-browser/src/capture/types.ts'),
