...
test('easeOverlayResize is monotonic and never overshoots [0,1]', () => {
  // Overshoot é forbidden: an eased valor >1 iria push o CSS panel past o
  // fixed window edge. O drawer curve precisa stay strictly dentro de [0,1].
  let prev = -Infinity;
  for (let i = 0; i <= 200; i++) {
...
test('easeOverlayResize is ease-OUT (past halfway by the midpoint, gentle tail)', () => {
  // Drawer curve: decelerating, então >0.5 progress por t=0.5, mas Não como violently
  // front-loaded como quint — it deve land em a tasteful band, não teleport.
  const mid = easeOverlayResize(0.5);
  assert.ok(mid > 0.5 && mid < 0.97, `expected 0.5<mid<0.97, got ${mid}`);
  // Late tail ainda moving mas quase dfeito
...
test('easeOutQuint is strictly monotonic and never overshoots [0,1]', () => {
  // O whole point de replacing o spring: não valor pode exceed 1 (overshoot),
  // porque o principal processo pushes f(t)*width to setBounds verbatim.
  let prev = -Infinity;
  for (let i = 0; i <= 100; i++) {
...
test('easeOutQuint front-loads progress (ease-OUT shape)', () => {
  // At o halfway point an ease-out curve deve já ser bem past 50%.
...
test('widthAt stays inside the [from,to] envelope for collapse too', () => {
  // Shrinking (780 -> 600): monotonic, nunca abaixo 600, nunca acima 780.
