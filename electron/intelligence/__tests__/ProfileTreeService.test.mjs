// node:test — ProfileTreeService (deterministic Perfil Árvore facade).
// Valida spec Fase 2 + acceptance criteria: identity/projects/experience/skills/
// education/intro/role-fit são deterministic; Nunca "I am Refract"; Nunca "I don't
// know" quando a perfil exists; candidate first-person voice; Alice/Bob isolation.
...
const REFRACT_LEAK = /\bi'?m refract\b|\bi am refract\b|\ban ai assistant\b/i;
...
describe('ProfileTreeService', () => {
  test('getIdentity is deterministic, first-person, names the candidate', () => {
    ...
    assert.doesNotMatch(id.answer, REFRACT_LEAK);
  });
  ...
  test('getInterviewIntro is grounded, first-person, never the assistant identity', () => {
    ...
    assert.doesNotMatch(intro, REFRACT_LEAK);
  ...
  test('getCompactIdentityBlock is a tight grounded block', () => {
    ...
    assert.doesNotMatch(block, REFRACT_LEAK);
  });
  ...
  test('NEVER "I don\'t know" when a profile exists — across all getters', () => {
    ...
    for (const v of [tree.getIdentity().answer, tree.getInterviewIntro(), tree.getProjects(), tree.getExperience(), tree.getSkills(), tree.getEducation(), tree.getRoleFit()]) {
      ...
      assert.doesNotMatch(v, REFRACT_LEAK);
    }
  });
  ...
  test('PRIVACY/ISOLATION: Bob\'s tree can never surface Alice\'s project', () => {
    ...
    // And it Faz surface Bob's próprio facts.
  ...
  test('getCandidatePerspectiveGuard blocks "I am Refract" in candidate-voice modes', () => {
  ...
  test('getCandidatePerspectiveGuard exempts genuine app-identity questions', () => {
    for (const q of ['are you an AI?', 'what is Refract?', 'what model are you?']) {
