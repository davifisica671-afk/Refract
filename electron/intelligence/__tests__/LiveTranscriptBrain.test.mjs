// node:test — LiveTranscriptBrain (live transcript lê facade + durable-window bug fix).
// O headline testar é o bug-fix proof: a fake SessionTracker that models o REAL
// 120s eviction de contextItems mostra getLiveWindow() loses a minute-1 entity por
// minute 62, enquanto getDurableWindow() (fullTranscript-backed) ainda tem it.
...
// A faithful fake de SessionTracker's two sarmazena
//   • contextItems  — hard-evicted to 120s em todo push (o real evictOldEntries)
...
class FakeSession {
  ...
  add(role, text, tSec) {
    ...
    // evictOldEntries: soltar qualquer coisa older than 120s relative to current clock.
...
describe('LiveTranscriptBrain', () => {
  ...
  test('BUG FIX PROOF: minute-1 entity survives to minute 62 only via the durable window', () => {
    ...
    // ... an hour de chatter advances o clock to minute 62.
    s.setNow(62 * 60); // 3720s
    s.add('interviewer', 'And what about that earlier system?', 62 * 60);

    const brain = new LiveTranscriptBrain(s, extractLatestQuestion);

    // O legacy live window (getContext) tem evicted o minute-1 entity entirely.
    ...
    // O durable window (fullTranscript-backed) Ainda tem it — this é o fix.
  ...
  test('getTranscriptEntities surfaces tech tokens and drops sentence-initial stop words', () => {
    ...
    // "HTer leads o sentence and é capitalized, mas é a para word → excluded.
