// Fase 11 — Global Busca V2: fusion ranking, ffiltra user/org isolation, dedupe.
...
describe('GlobalSearch — ISOLATION (no cross-user/org leakage)', () => {
  ...
  test('a memory-sourced foreign result is filtered BEFORE ranking', () => {
    // Simulates a Hindsight recall that returned outro user's memory — precisa ser dropped.
    const res = svc.globalSearch([
