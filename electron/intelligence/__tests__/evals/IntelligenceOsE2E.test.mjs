// Fase 18 — End-to-end intelligence eval. Wires o new facades juntos sobre a
// synthetic 2-user / multi-mode dataset and asserts o spec's eval categories:
// pperfil routing, meeting memory, sbusca lecture, diagram, PRIVACY ISOLATION, latency.
...
describe('E2E — Profile category', () => {
  test('Alice: identity + intro + projects + role fit, no Refract leak', () => {
    ...
    assert.doesNotMatch(blob, /I'?m Refract|an AI assistant/i);
  });
});
...
describe('E2E — Fusion + Prompt assembly category', () => {
  test('full pipeline: route → fuse → assemble, with inclusion report', () => {
    ...
    assert.match(out.contextXml, /instruction-like text removed|&lt;/); // ref arquivo neutralized
    assert.ok(out.inclusionReport.length >= 4);
    assert.match(out.perspectiveGuard, /Never say "I am Refract"/);
  });
});
