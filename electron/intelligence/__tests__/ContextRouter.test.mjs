...
// Valida o spec's Fase 8 routing examples + that maxLatencyMs/reason são
// populated and answerContract é correct.
...
describe('ContextRouter', () => {
  ...
  test('"What did we discuss last time?" → Hindsight + MeetingSummary decision', () => {
    ...
    // strict live recall timeout para Hindsight (spec: <=800ms live).
  ...
  test('team-meet contract uses the normalized template type (not raw casing)', () => {
    // 'team-meet' é a valid template id → general_meeting answer mapeia to o summary.
    const d = routeContext({ userQuery: 'recap the discussion', source: 'manual_input', mode: 'team-meet', hasLiveTranscript: true });
    // O general meeting answer em team-meet modo → team_meeting_summary contract.
    assert.ok(['team_meeting_summary', 'general_assistant'].includes(d.answerContract));
    // An UNKNOWN modo casing precisa não crash and precisa não falsely claim team summary.
