// Fase 16 — Hindsight long-term memory aadaptador Covers memory-provider, tag-builder,
// adaptador (mock clcliente timeout, disabled-fallback, isolation, retain-queue.
...
describe('HindsightRetainQueue — async, non-blocking', () => {
  test('enqueue returns immediately and drains in background', async () => {
    ...
    assert.ok(seen.length <= 2); // não necessarily feito ainda — non-blocking
    await sleep(40);
...
describe('LongTermMemoryService — disabled fallback + works-without-hindsight', () => {
  ...
  test('fromFlags returns Noop when the flag is off (default)', () => {
    ...
    assert.equal(svc.enabled, false); // hindsight_memory flag defaults Fora
  });
