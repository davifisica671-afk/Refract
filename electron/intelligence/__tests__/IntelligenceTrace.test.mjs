// node:test — IntelligenceTrace (observe-only structured per-answer reregistro
// VValida zero-cost no-op quando ofora structured registro + inclusion report quando oem
// privacy (não raw consulta stored); nunca throws.
...
function enableTrace(on) {
  if (on) process.env.REFRACT_INTELLIGENCE_TRACE = '1';
  else delete process.env.REFRACT_INTELLIGENCE_TRACE;
...
describe('IntelligenceTrace', () => {
  ...
  test('returns a zero-cost NO-OP when the flag is OFF', () => {
    ...
    // No-op methods são chainable and registro nnada
