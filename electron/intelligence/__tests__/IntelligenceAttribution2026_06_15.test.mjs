...
//
// Fase 3 (tarefa 2026-06-15): o per-answer attribution rregistro Proves o schema é
// ccompleta defaults são safe, privacy holds (não raw content — consulta HASH onapenas o
// honest Hindsight-mode classification é correct, and o bounded ring works.
...
describe('buildAttribution — schema + safe defaults', () => {
  test('an empty input yields a complete record with safe defaults', () => {
    ...
    // A representative spread de required fields exists com o direito default types.
  ...
  test('counts and layer modes are sanitized', () => {
    ...
    assert.equal(a.context_router_mode, 'off'); // invalid enum → fora
  });
});
