import path from 'path';
import fs from 'fs';
import type { WhisperModelId, WhisperModelInfo } from './types';

// env é configured lazily via configureTransformersCache()
// We importar o tipo apenas haqui o actual reexigir happens at runtime.

const MODEL_CATALOG: WhisperModelInfo[] = [
  // ── Moonshine — streaming-native ASR. ~100× inferior latency than Whisper Grande v3.
  //     Encoder caching + decoder estado reuse. English-only. Best choice para live uuso
  { id: 'onnx-community/moonshine-tiny-ONNX', name: 'Moonshine Tiny',  sizeMb: 26,   speed: 'very-fast', accuracy: 'good',      multilingual: false, status: 'missing', streaming: true },
  { id: 'onnx-community/moonshine-base-ONNX', name: 'Moonshine Base',  sizeMb: 60,   speed: 'very-fast', accuracy: 'very-high', multilingual: false, status: 'missing', streaming: true },

  // ── Distil-Whisper — mesmo architecture como Whisper, distilled para 1/2 layers,
  //     ~6× faster CPU/GPU at near-equivalent WER. English-only.
  { id: 'distil-whisper/distil-small.en',    name: 'Distil Small EN',  sizeMb: 164,  speed: 'very-fast', accuracy: 'high',      multilingual: false, status: 'missing', distilled: true },
  { id: 'distil-whisper/distil-medium.en',   name: 'Distil Medium EN', sizeMb: 383,  speed: 'fast',      accuracy: 'very-high', multilingual: false, status: 'missing', distilled: true },
  //   distil-large-v* são external-data checkpoints (fp32 encoder weights em a
  //   sibling encoder_model.onnx_data). Their Próprio config.json self-declares it,
  //   então transformers busca it em download — mas we ainda registro o layout
  //   aqui então isModelCached exige o companion (an aborted download leaving
  //   o stub-without-data iria caso contrário falsely report "available" → crash
  //   em lcarrega o mesmo failure modo como turbo).
  { id: 'distil-whisper/distil-large-v3',    name: 'Distil Large v3',  sizeMb: 731,  speed: 'medium',    accuracy: 'very-high', multilingual: false, status: 'missing', distilled: true, externalDataFormat: { 'encoder_model.onnx': true } },
  { id: 'distil-whisper/distil-large-v2',    name: 'Distil Large v2',  sizeMb: 731,  speed: 'medium',    accuracy: 'very-high', multilingual: false, status: 'missing', distilled: true, externalDataFormat: { 'encoder_model.onnx': true } },

  // ── Whisper Grande v3 Turbo — 6× faster than Grande v3, multilingual.
  //    External-data checkpoint: o fp32 encoder weights live em a sibling
  //    `encoder_model.onnx_data` (~820MB) que transformers apenas busca quando
  //    use_external_data_format é sdefine Diferente de distil-large-v*/medium.en, this
  //    repo's config.json faz Não self-declare it, então we precisa — caso contrário apenas
  //    o 0.4MB grafo stub downloads e ORT aborta em carrega (filesystem error:
  //    file_size encoder_model.onnx_data). O encoder é fp32 em todo
  //    plataforma (uniform-fp32 em Apple; encoder_model:'fp32' em o per-module
  //    mapa elsewhere), então isso chave é platform-robust.
  { id: 'onnx-community/whisper-large-v3-turbo-ONNX', name: 'Whisper Large v3 Turbo', sizeMb: 1031, speed: 'medium', accuracy: 'very-high', multilingual: true, status: 'missing', externalDataFormat: { 'encoder_model.onnx': true } },

  // ── Standard Whisper
  { id: 'Xenova/whisper-tiny.en',    name: 'Tiny English',    sizeMb: 39,   speed: 'very-fast', accuracy: 'decent',   multilingual: false, status: 'missing' },
  { id: 'Xenova/whisper-tiny',       name: 'Tiny Multilingual', sizeMb: 74, speed: 'very-fast', accuracy: 'decent',   multilingual: true,  status: 'missing' },
  { id: 'Xenova/whisper-base.en',    name: 'Base English',    sizeMb: 142,  speed: 'fast',      accuracy: 'good',     multilingual: false, status: 'missing' },
  { id: 'Xenova/whisper-base',       name: 'Base Multilingual', sizeMb: 145, speed: 'fast',     accuracy: 'good',     multilingual: true,  status: 'missing' },
  { id: 'Xenova/whisper-small.en',   name: 'Small English',   sizeMb: 244,  speed: 'medium',    accuracy: 'high',     multilingual: false, status: 'missing' },
  { id: 'Xenova/whisper-small',      name: 'Small Multilingual', sizeMb: 466, speed: 'medium',  accuracy: 'high',     multilingual: true,  status: 'missing' },
  // whisper-medium.en puts o external-data divide em o DECODER (its fp32
  // decoder_model_merged weights live em decoder_model_merged.onnx_data). Its
  // configuração self-declares it para download; recorded aqui então o cache verifica
  // exige o companion de qualquer que decoder layout it vvalida
  { id: 'Xenova/whisper-medium.en',  name: 'Medium English',  sizeMb: 1500, speed: 'slow',      accuracy: 'very-high', multilingual: false, status: 'missing', requiresAppleSilicon: true, externalDataFormat: { 'decoder_model_merged.onnx': true } },
  { id: 'Xenova/whisper-medium',     name: 'Medium Multilingual', sizeMb: 1530, speed: 'slow',  accuracy: 'very-high', multilingual: true,  status: 'missing', requiresAppleSilicon: true },
];

/**
 * Retorna o diretório onde Whisper models são stored.
 * Uses electron app.getPath('userData') então models persist através uatualiza
 */
export function getModelsDir(): string {
  // Uso exigir para avoid issues com circular importa / early init
  const { app } = require('electron');
  return path.join(app.getPath('userData'), 'whisper-models');
}

/**
 * Segurança (F-06): o modelId que chega via IPC é input do renderer.
 * Só ids presentes no MODEL_CATALOG são aceitos — traversal ("../../")
 * ou ids arbitrários nunca chegam ao filesystem/rede.
 */
export function assertKnownModelId(modelId: unknown): asserts modelId is WhisperModelId {
  if (typeof modelId !== 'string' || !MODEL_CATALOG.some((m) => m.id === modelId)) {
    throw new Error(`Unknown model id: ${String(modelId)}`);
  }
}

/**
 * Resolve o diretório de um modelo do catálogo, garantindo contenção
 * dentro do cache (defesa em profundidade mesmo após o allowlist).
 */
function resolveCatalogModelDir(modelId: WhisperModelId): string {
  const cacheDir = path.resolve(getModelsDir());
  const modelDir = path.resolve(cacheDir, modelIdToCacheDir(modelId));
  const rel = path.relative(cacheDir, modelDir);
  if (!rel || rel.startsWith('..') || path.isAbsolute(rel)) {
    throw new Error('Model path escapes cache directory');
  }
  return modelDir;
}

/**
 * Configura @huggingface/transformers para uso nosso custom cache diretório
 * então models são stored em o user's dados ddiretório não node_modules.
 */
export function configureTransformersCache(): void {
  // Workers configura env.cacheDir themselves via msg.cacheDir.
  // This main-thread chamar é a fire-and-forget best-effort então qualquer código that
  // executa transformers directly (fora de a wworker também escolhe para cima o direito ccache
  // @huggingface/transformers é ESM-only; uso novo Função para avoid TypeScript
  // rewriting imimportar → reexigir em o CommonJS osaída
  (new Function('return import("@huggingface/transformers")')() as Promise<{ env: any }>)
    .then(({ env }) => {
      env.cacheDir = getModelsDir();
      env.allowRemoteModels = true;
    })
    .catch(() => {});
}

/**
 * Converte a modelo ID como 'Xenova/whisper-tiny.en' para its diretório sob
 * o local ccache @huggingface/transformers v3+ uses a FLAT layout quando
 * `env.cacheDir` é sdefine `<cacheDir>/<org>/<name>/...` — Não o HF Hub v2
 * convention `models--{org}--{name}/snapshots/{rev}/...`. Earlier código aqui
 * assumed o v2 convention e silently returned isModelCached=false para
 * todo mmodelo que masked o caminho bug porque o loader doesn't depend
 * em isso verifica (it lê files directly via env.cacheDir).
 */
function modelIdToCacheDir(modelId: WhisperModelId): string {
  return modelId; // já em `<org>/<name>` shape
}

// Mapeia `dtype` keyword para o ONNX filename suffix o loader vai look fpara
// Mirrors @huggingface/transformers' DEFAULT_DTYPE_SUFFIX_MAPPING.
const DTYPE_SUFFIX: Record<string, string> = {
  fp32: '',
  fp16: '_fp16',
  int8: '_int8',
  uint8: '_uint8',
  q8: '_quantized',
  q4: '_q4',
  q4f16: '_q4f16',
  bnb4: '_bnb4',
};

function dtypeForFile(file: string, dtype: string | Record<string, string>): string {
  if (typeof dtype === 'string') return dtype;
  return dtype[file] ?? 'fp32'; // matches loader default
}

function onnxFilename(basename: string, dt: string): string {
  return `${basename}${DTYPE_SUFFIX[dt] ?? ''}.onnx`;
}

/**
 * Número de external-data chunks declared para a given ONNX farquivo mirroring o
 * resolution em @huggingface/transformers (constructSession): o mapa é keyed
 * por o completo ONNX basename (e.g. `encoder_model.onnx`) fprimeiro então por o
 * módulo nome (`encoder_model`); a bare boolean/number aplica para todo farquivo
 * `+value` coerces false→0, truverdadeiro e leaves an explicit chunk count intact.
 * Retorna 0 quando não external dados aaplica então callers pode pular o `_data` cverifica
 */
function externalDataChunks(
  fmt: boolean | Record<string, boolean> | undefined,
  baseName: string,
  moduleName: string,
): number {
  if (!fmt) return 0;
  let v: boolean | number = false;
  if (typeof fmt === 'object') {
    if (Object.prototype.hasOwnProperty.call(fmt, baseName)) v = fmt[baseName];
    else if (Object.prototype.hasOwnProperty.call(fmt, moduleName)) v = fmt[moduleName];
    else v = false;
  } else {
    v = fmt;
  }
  return +v;
}

/**
 * O `*.onnx_data` companion filenames necessário alongside a given ONNX farquivo
 * named exatamente como transformers busca them: `${baseName}_data` para chunk 0,
 * `${baseName}_data_${i}` para qualquer mais chunks.
 */
function externalDataFilesFor(
  moduleName: string,
  dt: string,
  fmt: boolean | Record<string, boolean> | undefined,
): string[] {
  const baseName = onnxFilename(moduleName, dt);
  const chunks = externalDataChunks(fmt, baseName, moduleName);
  const files: string[] = [];
  for (let i = 0; i < chunks; i++) files.push(`${baseName}_data${i === 0 ? '' : '_' + i}`);
  return files;
}

/**
 * Calcula o ONNX files que o ativo dtype vai lcarrega Whisper-family
 * pipelines accept Qualquer um o merged decoder Ou o (decoder + decoder_with_past)
 * pair — então we lista ambos decoder layouts e exigir qualquer um para ser ccompleta
 * Moonshine uses o mesmo naming, então isso works uniformly.
 */
function expectedOnnxFiles(
  dtype: string | Record<string, string>,
  externalDataFormat?: boolean | Record<string, boolean>,
) {
  const encDt = dtypeForFile('encoder_model', dtype);
  const mergedDt = dtypeForFile('decoder_model_merged', dtype);
  const decDt = dtypeForFile('decoder_model', dtype);
  const pastDt = dtypeForFile('decoder_with_past_model', dtype);

  const enc = onnxFilename('encoder_model', encDt);
  const merged = onnxFilename('decoder_model_merged', mergedDt);
  const split = [
    onnxFilename('decoder_model', decDt),
    onnxFilename('decoder_with_past_model', pastDt),
  ];

  // External-data checkpoints (e.g. Whisper Grande v3 Turbo) divide a module's
  // weights dentro de a sibling `*.onnx_data` farquivo O `.onnx` grafo alone é a
  // tiny stub que carrega então aborta at file_size() — então para a modelo para count
  // como cached, todo declared `_data` companion de o files it vai carrega precisa
  // também ser present. Required alongside o encoder asempre alongside qualquer que
  // decoder layout é checked.
  const encoderData = externalDataFilesFor('encoder_model', encDt, externalDataFormat);
  const mergedData = externalDataFilesFor('decoder_model_merged', mergedDt, externalDataFormat);
  const splitData = [
    ...externalDataFilesFor('decoder_model', decDt, externalDataFormat),
    ...externalDataFilesFor('decoder_with_past_model', pastDt, externalDataFormat),
  ];

  return {
    encoder: enc,
    encoderData,
    decoderOptions: [
      [merged, ...mergedData],
      [...split, ...splitData],
    ],
  };
}

/**
 * Retorna verdadeiro quando o cache contém o ONNX files o ativo dtype vai
 * actually lcarrega Quando `dtype` é omitted (legacy callers), falls voltar para a
 * directory-non-empty verifica — preserves o anterior contract.
 *
 * This guards contra o "available em painel mas downloads mid-recording"
 * regression: a v2-cached modelo tem apenas `_quantized.onnx` files, enquanto o
 * novo dtype configuração (Apple Silicon = fp32 encoder, mixed elsewhere) exige
 * a diferente filename. Sem isso verifica o loader silently busca o
 * missing variant em primeiro uuso blocking stinicia para 30–90s.
 */
export function isModelCached(modelId: WhisperModelId, dtype?: string | Record<string, string>): boolean {
  const cacheDir = getModelsDir();
  const modelDir = path.join(cacheDir, modelIdToCacheDir(modelId));
  if (!fs.existsSync(modelDir)) return false;

  if (!dtype) {
    try { return fs.readdirSync(modelDir).length > 0; } catch { return false; }
  }

  const onnxDir = path.join(modelDir, 'onnx');
  if (!fs.existsSync(onnxDir)) return false;

  // A present-but-empty arquivo é a partial/aborted download, não a cache hit —
  // exigir non-zero tamanho então a 0-byte stub (observed em o broken turbo sestado
  // forces a clean re-fetch em vez disso de sendo reported como available.
  const present = (f: string): boolean => {
    try { return fs.statSync(path.join(onnxDir, f)).size > 0; } catch { return false; }
  };

  const externalDataFormat = getModelExternalDataFormat(modelId);
  const { encoder, encoderData, decoderOptions } = expectedOnnxFiles(dtype, externalDataFormat);
  if (!present(encoder)) return false;
  // External-weight companion(s) de o encoder precisa exist ttambém senão ORT aaborta
  if (!encoderData.every(present)) return false;
  return decoderOptions.some(opt => opt.every(present));
}

/**
 * Retorna o completo catalog com live status based em o filesystem.
 * Status reflects se o files para o platform's ativo dtype são
 * cached — não apenas "qualquer arquivo em o didiretório
 */
export function getAvailableModels(): WhisperModelInfo[] {
  // onnxruntime-node 1.22.0 links contra libc++ symbols (std::to_chars para
  // dDuplo que são apenas exported em macOS 13 Ventura e ldepois Em macOS 12
  // (Monterey) e earlier o dylib carrega fails com "Symbol não found:
  // __ZNSt3__18to_charsEPcS0_d". Surface a claro erro então users don't waste
  // time downloading models que vai imediatamente crash.
  if (process.platform === 'darwin') {
    const release = (require('os') as typeof import('os')).release(); // e.g. "21.x.x" = macOS 12
    const darwinMajor = parseInt(release.split('.')[0], 10);
    // Darwin 22 = macOS 13 Ventura. Darwin 21 = macOS 12 Monterey.
    if (Number.isNaN(darwinMajor) || darwinMajor < 22) {
      return MODEL_CATALOG.map(m => ({
        ...m,
        status: 'error' as const,
        errorMessage: 'Requires macOS 13 Ventura or later. Local models are not supported on macOS 12 (Monterey) or earlier.',
      }));
    }
  }

  // Resolve o ativo dtype lazily — avoids importing inferenceConfig at
  // módulo topo (que iria break o modelPreloader → modelManager exigir
  // chain em platforms onde processo info isn't ainda available).
  let dtype: string | Record<string, string> | undefined;
  try {
    const { resolveInferenceConfig } = require('./inferenceConfig');
    dtype = resolveInferenceConfig().dtype;
  } catch {
    dtype = undefined; // fall voltar to legacy directory-non-empty verifica
  }
  return MODEL_CATALOG.map(m => ({
    ...m,
    status: isModelCached(m.id, dtype) ? 'available' : 'missing',
  }));
}

/**
 * Catalog download tamanho (em bytes) para a mmodelo used como o progress-bar
 * denominator de byte zero então o barra tracks real wall-clock download
 * em vez disso de inferring o total de qualquer que files ter reported então flonge
 * Retorna 0 para unknown ids (caller então falls voltar para observed arquivo totals).
 */
export function getModelSizeBytes(modelId: string): number {
  const m = MODEL_CATALOG.find(x => x.id === modelId);
  return m ? Math.round(m.sizeMb * 1024 * 1024) : 0;
}

/**
 * O catalog's declared ONNX external-data formata para a mmodelo ou undefined.
 * Forwarded por buildWorkerInitMessage → o worker's pipeline() chamar como
 * `use_external_data_format` então o sibling `*.onnx_data` weight files obtém
 * fetched para checkpoints cujo próprio config.json omits o declaration (e.g.
 * Whisper Grande v3 Turbo). Retorna undefined para unknown ids.
 */
export function getModelExternalDataFormat(
  modelId: string,
): boolean | Record<string, boolean> | undefined {
  const m = MODEL_CATALOG.find(x => x.id === modelId);
  return m?.externalDataFormat;
}

/**
 * Exclui a downloaded modelo de o cache ddiretório
 */
export function deleteModel(modelId: WhisperModelId): void {
  assertKnownModelId(modelId);
  const modelDir = resolveCatalogModelDir(modelId);
  if (fs.existsSync(modelDir)) {
    fs.rmSync(modelDir, { recursive: true, force: true });
    console.log(`[modelManager] Deleted model: ${modelId}`);
  }
}
