...
test('RefractProSTT DNS retries must not trip terminal STT failure counter', () => {
  const stt = new EventEmitter();
  const statuses = [];
  wireSttErrorStatus(stt, (status) => statuses.push(status));

  for (let i = 0; i < 5; i++) {
    const err = new Error('getaddrinfo ENOTFOUND api.refract.software');
    err.code = 'ENOTFOUND';
    stt.emit('error', err);
  }
  ...
  assert.equal(
    ...
    false,
    'ENOTFOUND must remain retryable and never become a terminal STT failure while RefractProSTT owns reconnect',
  );
...
test('RefractProSTT temporary DNS failures must not trip terminal STT failure counter', () => {
  const stt = new EventEmitter();
  const statuses = [];
  wireSttErrorStatus(stt, (status) => statuses.push(status));

  for (let i = 0; i < 5; i++) {
    const err = new Error('getaddrinfo EAI_AGAIN api.refract.software');
    err.code = 'EAI_AGAIN';
    stt.emit('error', err);
  }
  ...
  assert.equal(
    statuses.some((status) => status.state === 'failed'),
    false,
    'EAI_AGAIN must remain retryable and never become a terminal STT failure while RefractProSTT owns reconnect',
  );
  assert.equal(statuses.length, 5);
  assert.deepEqual([...new Set(statuses.map((status) => status.state))], ['reconnecting']);
  assert.deepEqual([...new Set(statuses.map((status) => status.reconnectAttempts))], [0]);
