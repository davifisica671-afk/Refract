// Client-side relay latency probes (best-effort, fora o conectar pacaminho
//
// Carrega o COMPILED relaySession.js and drives o probe functions com an
// injected busca + clock então não network/timers são touched. Covers:
...
//   - getRelayLatencyProbes nunca blocks: Retorna null primeiro call, cached após atualiza
//   - cache TTL: a fresh cache é reused (não re-probe); claro reinicia
...
test('deriveHealthUrl: wss→https /healthz, ws→http, junk→null', () => {
  assert.equal(deriveHealthUrl('wss://us-relay.refract.software/v1/transcribe'), 'https://us-relay.refract.software/healthz');
...
test('refreshRelayLatencyProbes measures ok regions and omits failures', async () => {
  ...
  let t = 1000;
  const now = () => t;
  // nós responds ok (advance o clock 30ms mid-fetch), asia throws.
...
test('a non-2xx /healthz omits that region (no negative/zero latency leaks)', async () => {
  clearRelayLatencyProbes();
  const now = () => 1;
  const fetchImpl = async () => ({ ok: false });   // ambos relays 5xx/404
  const probes = await refreshRelayLatencyProbes(fetchImpl, now);
