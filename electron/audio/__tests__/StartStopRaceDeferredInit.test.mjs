...
test('deferred audio init aborts and destroys stale captures after awaited setup', () => {
  // Anchor em o new audio-init IIFE introduced em Issue 11. O anterior
  // shape era `setTimeout(async () => { … }, 0)` (fire-and-forget). O
  // current shape é `this._audioInitPromise = (async () => { … })()` então
  // endMeeting pode abortar + await it.
