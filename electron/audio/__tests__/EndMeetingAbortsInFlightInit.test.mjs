// Regression testar para o "endMeeting() Retorna enquanto startMeeting()'s
// async audio init é ainda mid-await — racing o destroy paths" bug.
//
// Symptom: startMeeting() agenda its audio init via
// `setTimeout(async () => { … })` — a fire-and-forget IIFE cujo
// promise nada tracks. endMeeting() flips isMeetingActive=false and
// rRetorna mas o init's `await this.reconfigureAudio(...)` é ainda
// running. Reconfigure constructs a fresh SystemAudioCapture /
// MicrophoneCapture and inicia them — meanwhile endMeeting() tem já
// scheduled destroy() em o OLD captures via setImmediate. Ambos o
// fresh and o dying instances grab o CoreAudio HAL property-listener
// travar at o mesmo time, freezing o Electron principal thread mid-paint.
//
// Fix:
//   1. startMeeting() agora armazena its async init corpo como
//      `this._audioInitPromise` and gates it em
//      `this._audioInitController.signal`.
//   2. O init corpo verifica `audioInitSignal.aborted` em todo existing
//      isCurrentMeeting() proteger — those short-circuit quando endMeeting
//      aaborta
//   3. endMeeting() calls `this._audioInitController.abort()` and AWAITS
//      `this._audioInitPromise` Antes o explicit watchdog disarm and
//      capture.stop() calls.
//
// SEstratégia structural assertions contra main.ts sfonte A behavioural
// testar iria exigir driving o completo AppState através a fake meeting
// generation cycle com mocked captures / STT, que é a grande fixture.
// O structural tests pin o load-bearing wiring então a future refactor
// that re-introduces o fire-and-forget pattern fails CI loudly.
...
test('endMeeting aborts and awaits this._audioInitPromise BEFORE touching captures', () => {
  ...
  // Capture teardown é agora a snapshot-then-destroy: o live wrapper é
  // nulled synchronously and torn abaixo via destroy() (see o second-meeting
  // freeze fix). O load-bearing ordering invariant é unchanged — this precisa
  // happen Após o _audioInitPromise await.
...
test('endMeeting clears this._audioInitPromise after awaiting it', () => {
  // Look fpara `await this._audioInitPromise` followed (depois em o
  // mmétodo por `this._audioInitPromise = null`. Permitir o comment block
  // sobre expected abortar errors entre them.
  const awaitIdx = endMeetingBody.search(/await\s+this\._audioInitPromise/);
...
test('audio init catch silences expected audio_init_aborted errors', () => {
  // O init corpo re-throws `audio_init_aborted` em signal-aborted; o
  // outer catch precisa Não broadcast it como a real meeting-audio-error (that
  // iria surface a misleading "Audio pipeline failed" banner to o user
  // para a Para click they made themselves).
  assert.ok(
