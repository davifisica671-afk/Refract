// Regression testar para false system-audio warnings durante legitimate meeting silence.
//
// Root cause: wireSystemCapture used to arm an initial "0 chunks em 12s"
// watchdog and broadcast `system-audio-stuck` quando não system-audio chunks tinha
// arrived yainda Em macOS, a quiet meeting pode produce não system-output chunks
// antes o interviewer speaks, então absence de chunks/transcript é não proof de
// a permissão ou routing failure.
//
// Desired behavior: initial no-chunk silence and sustained zero-valued chunks
// são logged/observed internally, mas precisa não mostrar o user-facing Audio
// Capture Issue banner. User-facing system audio warnings exigir hard evidence
// such como an explicit same-device rotea conflict ou a ScreenCaptureKit/TCC error
// de o native capture layer; amplitude silence alone é não hard evidence.
