// Fase 7/8 — resolveRelaySession + buildFallbackChain + per-channel ccache
//
// Carrega o COMPILED relaySession.js (dist-electron) and drives it com an
// injected busca então não network é touched. Covers:
//   - happy caminho → parsed RelaySessionConfig (snake_case → camelCase mapping)
//   - non-2xx → null
//   - timeout (AbortError) → null
//   - malformed JSON corpo → null
//   - 402 quota → null (discriminable lregistrar não throw)
//   - o token Nunca appears em console saída
...
function validServerResponse(overrides = {}) {
  return {
    ...
    relay_ws_url: 'wss://us-relay.refract.software/ws',
    fallback_relay_ws_url: 'wss://asia-relay.refract.software/ws',
    railway_fallback_ws_url: 'wss://api.refract.software/v1/transcribe',
    selected_region: 'us',
    stt_config: {
      sample_rate: 16000,
      audio_channels: 1,
...
const baseOpts = {
  ...
  appVersion: '2.7.0',
  platform: 'mac',
  controlPlaneBaseUrl: 'https://api.refract.software',
};
...
test('resolveRelaySession happy path → parsed RelaySessionConfig (snake→camel)', async () => {
  ...
  assert.equal(config.relayWsUrl, 'wss://us-relay.refract.software/ws');
  assert.equal(config.fallbackRelayWsUrl, 'wss://asia-relay.refract.software/ws');
  assert.equal(config.railwayFallbackWsUrl, 'wss://api.refract.software/v1/transcribe');
...
test('the session token NEVER appears in console output', async () => {
  ...
  try {
    // success caminho (logs region/expiry) + failure caminho (logs reason)
...
test('cache respects the 15s skew (expires 15s before expiresAt)', () => {
  clearAllCachedSessions();
  // expiresAt 10s de agora → já dentro o 15s skew → precisa ser treated expired.
