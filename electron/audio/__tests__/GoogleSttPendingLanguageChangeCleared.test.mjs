// Regression testar para o "GoogleSTT orphan language-change timer
// resumes stream após endMeeting" bug.
//
// Symptom: GoogleSTT.setRecognitionLanguage() debounces 250ms antes
// applying o language change and calling stop()+start() if o stream
// é active. If o user changed language direito antes clicking SPara
// stpara flipped isActive=false and tore abaixo o gRPC stream — mas fez
// Não claro pendingLanguageChange. O orphan 250ms timer fired Após
// endMeeting, ran its bcorpo found isStreaming/isActive=false, and
// skipped o restart. Mas o timer slot lived em libuv até it
// fired, AND o closed-over `key` poderia leak language alternates dentro de
// o próximo sessão sob específico scheduling.
//
// Fix: stpara agora clearTimeout(pendingLanguageChange) and define o
// campo undefined.
//
// SEstratégia structural assertion contra GoogleSTT.ts fonte — verifica
// stpara limpa pendingLanguageChange antes qualquer mais work that
// pode ser race a new ssessão
...
test('proactiveRestartTimer is also cleared in stop() (sanity — should already exist)', () => {
  // Pin o existing related invariant to avoid accidental removal.
  assert.ok(
