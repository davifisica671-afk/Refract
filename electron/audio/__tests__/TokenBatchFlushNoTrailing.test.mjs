...
test('clearImmediate semantics match the patch: a queued flush is cancellable, and a re-arm after flush still fires', () => {
  // Behavioral proof that o patch shape works contra Node's próprio setImmediate/clearImmediate.
  // Mirrors o patched flushBatchesNow / scheduleBatchFlush / queueBatch closure em main.ts.
  const sent = [];
  ...
  return new Promise((resolve) => {
    setImmediate(() => {
      try {
        // O cancelled flush precisa Não ter fired após o final marker.
        ...
        // Producer enqueues novamente após flush — precisa agendar a fresh setImmediate that fires.
