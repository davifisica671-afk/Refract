/**
 * =============================================================================
 * llm/index.ts — PONTO CENTRAL DE EXPORTAÇÃO DO SISTEMA LLM
 * =============================================================================
 * 
 * DESCRIÇÃO:
 * Este arquivo é o BARREL (ponto de exportação central) para TODOS os
 * módulos do sistema LLM. Qualquer arquivo que precise de funcionalidade
 * LLM importa de './llm' e este arquivo re-exporta tudo.
 * 
 * MÓDULOS EXPORTADOS (organizados por categoria):
 * 
 * 🎯 MÓDULOS DE MODO (cada um gera um tipo de resposta):
 *    - AnswerLLM: Resposta direta a uma pergunta
 *    - AssistLLM: Assistente geral durante reunião
 *    - WhatToAnswerLLM: "O que devo dizer?" (resposta ao vivo)
 *    - FollowUpLLM: Refinamento de resposta anterior
 *    - RecapLLM: Resumo da reunião
 *    - ClarifyLLM: Esclarecimento de pergunta ambígua
 *    - CodeHintLLM: Dica de código (entrevista técnica)
 *    - BrainstormLLM: Brainstorming de ideias
 *    - FollowUpQuestionsLLM: Sugere perguntas de acompanhamento
 * 
 * 🧹 PÓS-PROCESSAMENTO:
 *    - clampResponse, validateResponse: Validação de resposta
 *    - StreamingDashReducer: Remove traços excessivos de streaming
 *    - cleanTranscript: Limpeza de transcrição antes de enviar ao LLM
 * 
 * 🧠 CLASSIFICAÇÃO E ROTEAMENTO:
 *    - classifyIntent: Detecta intenção do usuário
 *    - planNextAssistantAction: Decide próxima ação do assistente
 *    - planAnswer: Planeja tipo de resposta
 *    - IntentClassifier: Classificador de intenção baseado em transformer
 * 
 * 💬 SESSÃO E MEMÓRIA:
 *    - SessionMemory: Memória de sessão ao vivo
 *    - liveSessionMemory: Configuração de memória de sessão
 *    - FollowUpResolver: Resolve perguntas de acompanhamento
 * 
 * 📝 VALIDAÇÃO:
 *    - AnswerValidator: Valida estrutura da resposta
 *    - ProfileOutputValidator: Valida que resposta mantém perspectiva do candidato
 *    - CodeSanityCheck: Verifica código gerado por bugs óbvios
 * 
 * 🔍 CONTEXTO:
 *    - contextRoute: Roteamento de contexto (qual contexto enviar)
 *    - customContextClassifier: Classifica contexto personalizado
 *    - ProviderRouter: Roteamento entre provedores LLM
 * 
 * ⏱️ TIMEOUTS:
 *    - liveDeadlines: Limites de tempo para respostas ao vivo
 *    - triggerGate: Controle de quando acionar/responder
 * =============================================================================
 */
...
export { LanguageLearningLLM } from "./LanguageLearningLLM";
export type { TranslationResult } from "./LanguageLearningLLM";
