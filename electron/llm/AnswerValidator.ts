...
const validateImplAnswer = (answer: string): AnswerValidationResult => {
  ...
  const codeBlock = extractFirstCodeBlock(trimmed);
  if (!codeBlock) {
    // No code at all — fall back to the full structural repair so the caller
    // still gets a presentable answer (with the honest missing-code marker).
    return { ok: false, missingSections: ['Code'], hasCodeBlock: false, hasComplexity: false, repaired: repairCodingMarkdown(trimmed) };
  }
