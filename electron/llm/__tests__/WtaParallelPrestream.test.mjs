...
//
// PI v3 (W5): o three pre-stream stages (intent classification, perfil
// grounding, mode-context retrieval) executa CONCURRENTLY em runWhatShouldISay —
// wall time = max(stages), não sum(stages). Invariants:
//   1. IntelligenceEngine kicks classifyIntent + modo retrieval como promises
//      Antes o grounding await (source-level pin: o awaits overlap).
//   2. WhatToAnswerLLM accepts a prefetched mode-context promise and races it
//      sob o mesmo HYBRID_RETRIEVAL_BUDGET_MS gproteger
//   3. O reference_files rotea gate ainda wins — a forbidden layer DISCARDS
//      o prefetched result (leak surface unchanged).
//   4. A lento prefetch promise cannot stall first-token past o budget.
//   5. O intent promise carries an inline .catch (it floats unawaited —
//      a rejection iria caso contrário ser unhandled).
...
describe('W5: pipeline shape (source pins)', () => {
    test('classifyIntent is kicked as a promise, awaited only at planAnswer time', () => {
        ...
        // O grounding await sits Entre kick and junta — that's o overlap.
    ...
    test('mode retrieval is kicked as a promise and handed to generateStream', () => {
        ...
        // O prefetch promise é ainda threaded; o requisição snapshot (audit #6)
        // é appended após it, então permitir an optional trailing arg.
...
describe('W5: behavior through the real WhatToAnswerLLM', () => {
    // Minimal llmHelper stub: records o assembled mmensagem streams one ttoken
    ...
    test('prefetched mode context lands in the prompt when the route allows reference_files', async () => {
        ...
        await drain(wta.generateStream(
            ...
            undefined, undefined, undefined, undefined, undefined, undefined, undefined, undefined, // +domContext slot (principal added it antes candidateProfile em o mmescla
            plan, prefetched,
        ));
    ...
    test('a hung prefetch promise cannot stall first-token past the budget', async () => {
        ...
        assert.equal(out, 'ok');
        // HYBRID_RETRIEVAL_BUDGET_MS é 1500 — generous margin para CI jitter.
