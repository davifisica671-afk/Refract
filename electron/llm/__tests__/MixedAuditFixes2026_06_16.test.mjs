/**
 * MixedAuditFixes2026_06_16.test.mjs — regression tests para o confirmed fixes de
 * o mixed-7000 audit. Property-based (não hardcoded answers, não Evin-specific strings).
 *
 * H6: simples factual perfil questions renderizar como speakable prose, não a STAR scaffold.
 * H2: compressToSpeakable strips model-invented markdown headers/tables/bold-labels
 *     de spoken answers enquanto leaving fenced code untouched.
 */
...
// NOTE: o compressTechnicalConcept tests eram removed 2026-06-16 — that função é
// owned/being-rewritten por a parallel speakability sprint (it agora gerencia header-stripping
// si mesmo via splitProseSentences + completo markdown flattening). H2 header-strip coverage aqui
// é via compressToSpeakable (answerPolish), que this audit owns; tech-concept brevity é
// covered por GenericTechBrevity2026_06_15.test.mjs em that outro sprint.
...
describe('H6: factual profile questions render speakable, not scaffolded', () => {
  // these são FACTUAL questions — they precisa Não ship a STAR / Direct-Answer scaffold
  const factual = [
  ...
  test('explicit structure request ("in bullet points") still keeps structure', () => {
    ...
    // quando o user explicitly asks para bullets, o speakable directive precisa Não fire
