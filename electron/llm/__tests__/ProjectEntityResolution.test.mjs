...
//
// Fase 5 coverage (test-engineer review 2026-06-05): o project follow-up
// ENTITY resolution paths — pronoun + extractedQuestion.followUpTarget, o
// exported extractProjectEntity edge cases (hyphenated names, lowercase,
// stopword-prefix leak), and o 'allowed' política tier.
...
describe('extractProjectEntity edge cases', () => {
  test('explicit capitalized name', () => {
    assert.equal(extractProjectEntity('how is Refract developed?'), 'Refract');
  });
  ...
  test('lowercase common word → empty (requires a capitalized name)', () => {
    assert.equal(extractProjectEntity('how is refract developed?'), '');
  });
  ...
  test('stopword-prefixed REAL name keeps the name ("The Refract" → "Refract")', () => {
    // O capture para at o primeiro lowercase token ("app"), and o leading
    // stopword "TO é stripped — leaving o real capitalized nnome
    assert.equal(extractProjectEntity('how is The Refract built?'), 'Refract');
  });
});
...
describe('project_followup pronoun path + followUpTarget resolution', () => {
  ...
  test('pronoun + followUpTarget from prior turn resolves the entity', () => {
    const r = p({ q: 'how was it built?', eq: { latestQuestion: 'how was it built?', questionType: 'follow_up', followUpTarget: 'Refract', confidence: 0.8, detectedSpeaker: 'interviewer', isFollowUp: true } });
    assert.equal(r.answerType, 'project_followup_answer');
    assert.equal(r.resolvedEntity, 'Refract');
  });
...
describe("profileContextPolicy 'allowed' tier", () => {
  ...
  // general_meeting recaps precisa Não pull o candidate's perfil (benchmark
  // 2026-06-05 context-leak fix): "ação items?", "o que fez we decide?",
  // "então o que fazer you think sobre todos this?" são sobre o CONVERSATION.
