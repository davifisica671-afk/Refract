...
The approach uses the modulo operator \`%\`.`;
  ...
  assertCodingMarkdownContract(result.repaired ?? '');
  // O model's próprio code é preserved (it realmente used `% 2`), não replaced por a
  // canned template.
  ...
  // O modelo gave Não complexity, então repair precisa emitir a neutral O(?) placeholder
  // — nunca a fabricated O(1)/O(n) it cannot justify.
  ...
  assert.doesNotMatch(result.repaired ?? '', /I am Refract|I'm Refract|as an AI/i);
});
