...
//
// Release 2026-06-07c — structured, time-aware, mode-aware sessão memory para
// long-range follow-up resolution. Covers o Fase 2 directive: immediate/short/
// longo memory, time decay, supersession, corrections, and cross-mode boundaries.
...
const MIN = 60; // seconds por minute
...
describe('Time-aware recall — immediate / short / long range', () => {
  test('immediate (1 min ago) project recalls easily', () => {
    ...
    m.note('project', 'Refract', 1 * MIN, 'technical-interview');
    const r = m.recall({ now: 2 * MIN, kind: 'project', mode: 'technical-interview' });
    assert.equal(r.item?.value, 'Refract');
  ...
  test('60 min later, a project is still recallable if no newer project superseded it', () => {
    ...
    assert.equal(r.item?.value, 'Refract', 'one-hour-later project follow-up resolves');
  });
  test('pinned items survive long decay', () => {
    const m = new SessionMemory();
    m.note('project', 'Refract', 0, 'technical-interview', { pinned: true });
...
describe('Corrections override earlier memory', () => {
  test('"actually use TalentScope" corrects the best project', () => {
    const m = new SessionMemory();
    m.note('project', 'Refract', 1 * MIN, 'looking-for-work');
...
describe('Cross-mode boundaries — memory must not leak into the wrong mode', () => {
  ...
  test('but an EXPLICIT cross-mode request allows it ("have you used this in Refract?")', () => {
    const m = new SessionMemory();
    m.note('project', 'Refract', 1 * MIN, 'technical-interview');
  ...
  test('meeting action item / person does NOT recall in sales mode', () => {
    ...
    // sales Pode see person (customer contact) — mas a meeting-introduced person é
    // visible apenas porque sales permite person; o real proteger é decision/objection.
    // Decisions precisa não leak to sales:
...
describe('Compensation is gated to negotiation mode ONLY', () => {
  ...
  // code-review 2026-06-07c: a salary Valor mislabeled sob outro kind precisa ainda ser
  // gated — adadiciona auto-promotes it to comp então it can't leak via o wrong kind.
...
describe('Adversarial: competing entities + stale-vs-fresh + double-correction (test-engineer review)', () => {
  ...
  test('double correction — the LATEST correction wins (Refract → TalentScope → Refract)', () => {
    const m = new SessionMemory();
    m.note('project', 'Refract', 2 * MIN, 'looking-for-work');
    m.note('project', 'TalentScope', 4 * MIN, 'looking-for-work', { corrects: true });
    m.note('project', 'Refract', 8 * MIN, 'looking-for-work', { corrects: true });
    const r = m.recall({ now: 12 * MIN, kind: 'project', mode: 'looking-for-work' });
    assert.equal(r.item?.value, 'Refract', 'latest correction is authoritative');
    assert.equal(r.reason, 'correction_override');
  });
  test('a fresh non-correction does NOT override an explicit correction', () => {
    ...
    m.note('company', 'Acme', 1 * MIN, 'sales');
    m.note('company', 'Globex', 2 * MIN, 'sales', { corrects: true });
    m.note('company', 'Acme', 10 * MIN, 'sales'); // depois stray mention, Não a correction
...
describe('Memory bounds + reset', () => {
  ...
  test('reset clears memory', () => {
    const m = new SessionMemory();
    m.note('project', 'Refract', 1, 'technical-interview');
    m.reset();
