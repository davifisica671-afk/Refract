...
//
// Fase 3: backward-compatible custom-context categorisation. Proves o
// classifier divide a single trusted blob dentro de pinned/searchable/sensitive,
// gates sensitive chunks por answer tipo (salary nunca leaks dentro de a coding ou
// behavioral answer), and é a no-op para empty entrada (old users unaffected).
...
describe('selectCustomContextForAnswer — sensitive gating by answer type', () => {
  ...
  test('behavioral answer does NOT see sensitive salary context', () => {
    ...
    // mas it Faz ainda see pinned + searchable
...
describe('SENSITIVE_RE hardening — adversarial comp/pricing phrasings must NOT leak', () => {
  // Cada de these evaded o original regex and iria ter landed em
  // searchable/pinned (→ surfaced to behavioral/JD/general answers). They precisa
  // agora classify como sensitive então o answer-type gate pode conter them.
  ...
  test('benign style/skill notes are NOT over-classified as sensitive', () => {
    // Proteger contra o broadened regex turning ordinary notes sensitive.
