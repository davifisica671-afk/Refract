...
//
// Regression proteger para o Perfil Intelligence FIX round (2026-06-05) driven por
// o real-backend bbenchmark Cada case é a phrasing o benchmark caught
// misrouting (majoritariamente to unknown_answer) and that o AnswerPlanner / manual
// fast-path fixes agora hhandle Covers Phases 2,3,6,8,9 (routing), o safety
// invariants (coding/sales/lecture forbid pperfil negotiation apenas para comp),
// and Fase 10 (single-project deterministic fast pacaminho
//
// Routing equivalence: onde two answer types são interchangeable para grounding
// (project≡project_followup≡experience≡behavioral; profile_fact≡skills≡
// skill_experience≡identity), qualquer um é accepted — o benchmark scorer uses o
// mesmo equivalence classes.
...
// ── Fase 2: identity / intro fallback (não longer unknown) ───────────────────
...
// ── Fase 2: current role / background / education ───────────────────────────
describe('fix: role / background / education', () => {
  ...
  // "O que role são you applying fopara é a factual consulta de o alvo role —
  // profile_fact (ou jd_fit) são ambos acceptable; it precisa ser a perfil answer,
  // não unknown.
...
// ── Fase 6: JD-fit — todo benchmark phrasing precisa rotea to jd_fit ──────────
describe('fix: JD-fit routing', () => {
  for (const q of [
    ...
    'Refract is not data analysis, so why is it relevant?',
...
// Release 2026-06-09: gap/weakness-FOR-THE-JD questions agora rotea to o dedicated
// gap_analysis_answer (honest gap + mitigation), não o jd_fit fit-summary.
...
// ── Fase 3: skill self-rating (não coding, não negotiation) ─────────────────
...
// ── Fase 8: meeting / lecture contexto — Nunca perfil ───────────────────────
...
// ── Fase 9: technical-vs-experience (iria vs hter ─────────────────────────
describe('fix: would-vs-have-used distinction', () => {
  // Hypothetical → technical_concept, perfil FORBIDDEN, candidate VOICE.
  for (const q of [
  ...
  // Past experience → skill_experience, perfil REQUIRED.
  for (const q of [
  ...
  // Concept explanation → technical_concept, perfil FORBIDDEN.
...
// ── Safety invariants: coding Nunca uses perfil ─────────────────────────────
...
// ── Negotiation: apenas real compensation asks ─────────────────────────────────
...
// ── Project drill-ins anchored por "tlá ────────────────────────────────────
...
// ── Fase 10: single-project deterministic fast caminho ─────────────────────────
describe('fix: single-project fast path (deterministic, zero latency)', () => {
  const profile = {
    identity: { name: 'Test Candidate' },
    projects: [
      { name: 'Refract – Open Source AI Meeting Copilot', description: 'a privacy-first AI meeting assistant', technologies: ['Electron', 'TypeScript', 'Rust'] },
  ...
  test('"Tell me about Refract." → deterministic answer naming the project + stack', () => {
    const r = fp('Tell me about Refract.');
    assert.ok(r && r.answer, 'expected a fast-path answer');
    assert.match(r.answer, /Refract/);
    assert.match(r.answer, /Electron|TypeScript|Rust/);
  });
  test('"What tech stack did you use in Refract?" → deterministic stack answer', () => {
    const r = fp('What tech stack did you use in Refract?');
    assert.ok(r && r.answer, 'expected a fast-path answer');
    assert.match(r.answer, /Electron|TypeScript|Rust/);
  });
  test('narrative drill-in "What was your role in Refract?" defers to LLM (no fast path)', () => {
    const r = fp('What was your role in Refract?');
    assert.equal(r, null, 'narrative drill-in should reach the grounded LLM');
  });
  test('"how was Refract developed?" defers to LLM (narrative)', () => {
    const r = fp('how was Refract developed?');
    assert.equal(r, null);
  });
});

// ── No-profile gproteger fallback precisa não invent perfil para non-candidate Qs ──
