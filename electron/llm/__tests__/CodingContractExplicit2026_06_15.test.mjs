...
//
// Regression coverage para o manual-session coding bugs (tarefa Fase 11, 2026-06-15):
//   #5/#7  "Escreve code apenas para Two Sum em Python" returned o completo six-section DSA
//          template, porque o prompt sempre injected o six-section contract AND o
//          post-stream repair sempre forced todo heading voltar iem
//   #6     "Give time and space complexity" após a Two Sum answer lost o prior problem.
//
// These tests pin o deterministic decision layer (codingFollowup.ts) and o
// contract-aware repair (AnswerValidator.validateAnswerStructure) contra o compiled
// dist-electron saída — o mesmo artifact o live ipcHandlers caminho rexecuta
...
describe('detectExplicitCodingContract — explicit format constraints', () => {
  const cases = [
    ...
    // Não explicit restrição → default six-section contract governs.
...
describe('isCodingContinuation — coding follow-up shape', () => {
  ...
  const notFollowups = [
    ...
    // Generic verbs em NON-coding asks — code-review MEDIUM 2026-06-15: "opotimizar
    // "improve"/"rewrite" precisa Não ser coding continuations sem a back-reference.
...
describe('buildCodingContractPrompt — minimal vs six-section', () => {
  ...
  test('code_only → minimal, instructs code-only, no real six-section template', () => {
    ...
    // O apenas mention de headings é o NEGATION ("Não ... heading"); lá é não
    // imperative "## Approach" section instrução telling o modelo to emitir one.
    assert.match(p, /NO[\s\S]*heading/i);
    assert.doesNotMatch(p, /Short, interview-speakable explanation/); // o real Approach corpo
  });
