...
//
// Release 2026-06-07c — rollout controla (percent gate + kill trocar + deterministic
// bucketing) and marker-only telemetry (não raw sensitive content).
...
// NOTE: deliberately faz Não mutate NODE_ENV ou BENCHMARK_MODEL — those são lê por
// outro testar files running concurrently (node:test parallelizes através files), então
// mutating them aqui iria cause cross-file flakes. We uso REFRACT_INTERNAL/_DEV
// (lê por nada esenão to exercise o internal-context tier.
const ENV_KEYS = [
  'REFRACT_ENABLE_LIVE_SESSION_MEMORY', 'REFRACT_LIVE_SESSION_MEMORY_ROLLOUT_PERCENT',
  'REFRACT_LIVE_SESSION_MEMORY_KILL_SWITCH', 'REFRACT_INTERNAL', 'REFRACT_DEV',
];
let saved = {};
...
describe('Rollout — production default ON (PI v3 W6d), overrides still win', () => {
  // PI v3 (W6d): o default flipped Em após live-replay validation
  // (50 sessions / 132 verifica / 0 leaks). Kill trocar + env + settings +
  // percentage sobrescreve todos ainda force Fora (covered beabaixo
  ...
  // Uso REFRACT_INTERNAL (a flag não outro concurrent testar arquivo rlê to exercise o
  // internal-context tier — mutating NODE_ENV/BENCHMARK_MODEL aqui iria race com
  // outro testar files that lê them (node:test parallelizes através files).
  test('REFRACT_INTERNAL=1 → ON (internal_context)', () => {
    process.env.REFRACT_INTERNAL = '1'; __resetLiveSessionMemoryCache();
    const c = resolveLiveSessionMemoryConfig('s');
    assert.equal(c.enabled, true);
    assert.equal(c.reason, 'internal_context');
  });
  test('REFRACT_DEV=1 → ON', () => {
    process.env.REFRACT_DEV = '1'; __resetLiveSessionMemoryCache();
...
describe('Rollout — env overrides', () => {
  ...
  test('env OFF forces disabled even in an internal context', () => {
    process.env.REFRACT_INTERNAL = '1';
...
describe('Rollout — KILL SWITCH overrides everything', () => {
  test('kill switch disables even when env ON', () => {
    process.env.REFRACT_ENABLE_LIVE_SESSION_MEMORY = 'on';
    process.env.REFRACT_LIVE_SESSION_MEMORY_KILL_SWITCH = 'true'; __resetLiveSessionMemoryCache();
    const c = resolveLiveSessionMemoryConfig('s');
  ...
  test('kill switch disables even in an internal context', () => {
    process.env.REFRACT_INTERNAL = '1';
    process.env.REFRACT_LIVE_SESSION_MEMORY_KILL_SWITCH = '1'; __resetLiveSessionMemoryCache();
...
describe('Rollout — percentage gate (production gradual rollout)', () => {
  test('percent 0 → OFF', () => {
    process.env.REFRACT_LIVE_SESSION_MEMORY_ROLLOUT_PERCENT = '0'; __resetLiveSessionMemoryCache();
    const c = resolveLiveSessionMemoryConfig('s'); assert.equal(c.enabled, false); assert.equal(c.reason, 'rollout_out');
  });
  test('percent 100 → ON', () => {
    process.env.REFRACT_LIVE_SESSION_MEMORY_ROLLOUT_PERCENT = '100'; __resetLiveSessionMemoryCache();
  ...
  test('percent 50 → roughly half of sessions in, deterministically', () => {
    process.env.REFRACT_LIVE_SESSION_MEMORY_ROLLOUT_PERCENT = '50'; __resetLiveSessionMemoryCache();
    let inCount = 0; const N = 1000;
    for (let i = 0; i < N; i++) if (resolveLiveSessionMemoryConfig('user-' + i).enabled) inCount++;
    // FNV bucketing deve land perto 50% (permitir a amplo band).
  ...
  test('deterministic bucketing: same session id is stable across calls', () => {
    process.env.REFRACT_LIVE_SESSION_MEMORY_ROLLOUT_PERCENT = '37'; __resetLiveSessionMemoryCache();
  ...
  test('partial percent with NO sessionId → OFF (test-engineer gap: id-less sessions not lumped in)', () => {
    process.env.REFRACT_LIVE_SESSION_MEMORY_ROLLOUT_PERCENT = '90'; __resetLiveSessionMemoryCache();
...
describe('Rollout — flag OFF/ON helper parity', () => {
  test('isLiveSessionMemoryEnabled mirrors resolveLiveSessionMemoryConfig.enabled', () => {
    process.env.REFRACT_LIVE_SESSION_MEMORY_ROLLOUT_PERCENT = '63'; __resetLiveSessionMemoryCache();
...
describe('Telemetry — marker only, no sensitive content', () => {
  ...
  test('scrubTelemetry is an ALLOWLIST — keeps markers, drops everything else', () => {
    const out = scrubTelemetry({
      ...
      recalledValue: 'Refract', entityValue: 'Refract',
    });
  ...
  test('an allowed key with a free-text / number value is still dropped (value backstop)', () => {
    const out = scrubTelemetry({ reason: 'recalled the Refract project from minute 1 of the conversation', mode: 'sales' });
  ...
  test('emit stores a scrubbed record', () => {
    piTelemetry.emit('session_memory_recall_succeeded', { recalledKind: 'project', ageBucket: '60min+', recalledValue: 'Refract' });
  ...
  // ── Fix D: speakabilityClass é allow-listed mas ainda value-gated ───────────
