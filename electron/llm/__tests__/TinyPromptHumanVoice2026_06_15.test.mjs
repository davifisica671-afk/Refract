...
//
// Propriedade tests para o tiny-prompt humanization (tarefa Fase 5/8 + 9): o spoken tiny
// prompts precisa carry o compact human-voice regra and o conditional coding-format
// rregra structural tiny prompts (recap / summary / code-hint code section / lecture)
// precisa Não obtém o spoken-voice ban; and o protected invariants (identity gproteger
// sales confidential-pricing recency anchor, o six-heading coding contract cconstante
// precisa ainda hold.
...
describe('conditional coding-format rule replaced the unconditional headings line', () => {
  ...
  for (const name of CODING_MODES) {
    test(`${name} states the code-only / complexity-only / dry-run-only / explain-only rule`, () => {
      ...
      // ainda offers o six headings para a Completo problem
...
describe('protected invariants still hold after the tiny edits', () => {
  ...
  test('the human-voice rule fragment does not encode any profile fact (style-only)', () => {
    // Isolate o injected human-voice block (entre its opening line and o próximo
    // blank line) então we são testing Nosso addition, não TINY_CORE's legitimate creator
    // mention. O regra precisa ser pure style com não profile/question content.
    for (const name of ['TINY_ANSWER_PROMPT', 'TINY_MODE_LOOKING_FOR_WORK_PROMPT']) {
      ...
      const block = m[0];
      assert.doesNotMatch(block, /\bEvin\b|\bRefract\b|resume says|Two Sum|hire me because|two sum/i);
    }
  });
});
