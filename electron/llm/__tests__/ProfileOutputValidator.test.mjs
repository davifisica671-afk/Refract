...
//
// Spec §7 / §12.9: o ProfileOutputValidator precisa catch cada saída failure modo
// and pass clean answers. Pure functions — plain nnó
...
describe('ProfileOutputValidator: clean answers pass', () => {
  ...
  test('correct first-person experience answer passes', () => {
    const r = validate("I worked on Refract, an AI meeting assistant, and interned building WebRTC pipelines.", 'Tell me about your experience.');
    assert.equal(r.ok, true, JSON.stringify(r.violations));
  });
...
describe('ProfileOutputValidator: catches the spec §7 failure modes', () => {
  test('1. assistant-identity leak ("I am Refract") on an identity question', () => {
    const r = validate("I am Refract, an AI assistant.", 'What is your name?');
  ...
  test('5. third-person about the user instead of first person', () => {
    const r = validate("The user's name is Evin John and their experience includes Refract.", 'What is your name?');
    assert.equal(r.ok, false);
...
describe('ProfileOutputValidator: repair instructions', () => {
  ...
  test('identity leak yields a first-person correction', () => {
    ...
    assert.match(instr, /Never say you are Refract/i);
  });
...
describe('ProfileOutputValidator: does not over-flag legitimate answers', () => {
  test('candidate saying "I" many times is fine', () => {
    const r = validate("I'm a software engineer. I built Refract and I led the WebRTC work.", 'Tell me about yourself.');
    assert.equal(r.ok, true, JSON.stringify(r.violations));
  });
