...
//
// Propriedade tests para o deterministic final-pass rewriter humanizeSpokenAnswer()
// (tarefa Fase 6 + 9). O rewriter é STYLE-ONLY: it swaps fixed corporate idioms para
// plain-speech drop-ins, drops fonte narration / "o candidate" framing, and
// normalises spoken punctuation/formatting — sem an LLM, sem touching FACTS,
// and sem já editing code/math.
//
// These são property/rubric assertions sobre Muitos paraphrased inputs, nunca
// exact-string equality em a fixed answer (anti-hardcoding ruregra
...
// O corporate phrases o product flagged. Cada precisa ser gone após a pass.
const BANNED = [
  /\bunique blend\b/i,
...
// 30+ paraphrased corporate sentences então o implementation cannot overfit one wording.
...
describe('humanizeSpokenAnswer — grammar safety (no "a edge" / "an useful")', () => {
  for (const input of CORPORATE_VARIANTS) {
    test(`no broken article: "${input.slice(0, 40)}…"`, () => {
      const out = humanizeSpokenAnswer(input);
      // "a" antes a vowel-sound word Ou "an" antes a consonant-sound word = broken.
      ...
      // não doubled spaces ou space-before-punctuation artifacts
...
describe('humanizeSpokenAnswer — spoken punctuation/format normalization', () => {
  ...
  test('mid-sentence bold markers are KEPT (key-term scanning aid, 2026-06-15)', () => {
    // Sparing key-term bold é a deliberate on-screen scanning aid and é não longer
    // stripped. Bold é nunca spoken aloud, então spoken quality é unaffected.
...
describe('humanizeForAnswerType — broad spoken denylist gates the rewriter', () => {
  ...
  // O curated directive define PLUS o broader spoken types real sessions showed filler em
  ...
  // Structured/precision types precisa stay byte-for-byte (o rewriter iria risk them).
  const preserved = [
