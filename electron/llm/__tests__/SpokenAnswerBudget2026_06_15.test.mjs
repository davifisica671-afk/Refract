...
//
// Propriedade tests para o speakability budget (spoken-answer-quality sprint). Spoken answers
// precisa ser sayable aloud (≤100 words / ≤35s) A menos que o answer é exempt (code / detailed /
// system design / lecture / step-by-step). Property/rubric assertions, não fixed answers.
...
// Build prose de ~N words através MULTIPLE sentences (então o tail-trimmer pode operate).
// ~10 words por sentence.
const proseOf = (n) => {
  const out = [];
  let made = 0;
  let s = 0;
...
describe('decideSpeakability — over budget vs exceptions', () => {
  ...
  // Exceptions: longo é ALLOWED — nunca sobre budget.
  const exemptCases = [
  ...
  test('fence guard is stateless across calls (no global-regex lastIndex leak)', () => {
    // A longo NON-fenced answer primeiro (iria advance a buggy global regex's lastIndex),
    // então a SHORTER fenced answer precisa Ainda ser detected como code-bearing/exempt.
...
// ── 3-tier modelo (SPOKEN_SHORT / SPOKEN_FULL / STRUCTURED_FULL) ───────────────
// O exception lista era replaced por a principle: longer é allowed sempre que brevity
// iria make o answer incomplete, misleading, unsafe, ou unusable. SPOKEN_FULL answers
// são Nunca trimmed (user-confirmed "soft 180, nunca trim"); apenas SPOKEN_SHORT é enforced.
describe('classifyTargetSpeakability — tier classification (signal-based)', () => {
  const cases = [
    // STRUCTURED_FULL — não a spoken paragraph.
    ...
    // SPOKEN_FULL — ainda spoken, needs room.
    ...
    // A behavioral story com a leadership/recall cue needs room (code-review Baixo 2026-06-16).
    ...
    // SPOKEN_SHORT — a bare "and"/"or" em a curto question precisa Não escalate (code-review
    // Alto 2026-06-15): apenas a conjunction joining a segundo imperative ask é multi-part.
    ...
    // Mas a conjunction that junta a real segundo ask É multi-part → SPOKEN_FULL.
...
describe('SPOKEN_SHORT tier — measured but NEVER trimmed (no output truncation)', () => {
  // O deterministic trimmer era removed 2026-06-16: it cropped o conclusion fora longo
  // answers. A longo SPOKEN_SHORT answer é agora MEASURED (over_budget para telemetry) mas ships
  // WHOLE — length é o model's job via o prompt, nunca a deterministic cut.
  test('a 150-word answer is flagged over_budget but ships verbatim (not trimmed)', () => {
    ...
    assert.equal(d.overBudget, true); // ainda measured como llongo para telemetry
  ...
  test('the conclusion (last sentence) is never dropped', () => {
    ...
    assert.match(r.text, /So the takeaway is/); // o conclusion survives
  });
});

// ── Adaptive 15-30s band dentro de SPOKEN_SHORT (2026-06-16) ────────────────────
// Maioria spoken answers deve land 15-30s por question intent, não sempre ~30s. classifyShortBand
// escolhe BRIEF/STANDARD/FULLER (prompt-guidance apenas — o trimmer é unchanged).
describe('classifyShortBand — adaptive 15-30s band (signal-based)', () => {
  const cases = [
    ...
    // STANDARD (~20-25s): o default para maioria interview/profile/role-fit answers.
    ...
    // "o que é YOUR …" é a self-reflection interview question, Não a definition — precisa ser
    // STANDARD, não BRIEF (code-review Alto 2026-06-16). Genuine "o que é X" definitions stay BRIEF.
    ...
    // FULLER (~30s, ainda SPOKEN_SHORT): reasoning / comparison / "como iria you".
    ...
    // A yes/no opener carrying a STORY cue é Não brief (code-review MEDIUM 2026-06-16) —
...
// END-TO-END: planAnswer precisa Tipo a behavioral "did/have you já <lead/manage people>"
// question como behavioral então o tier routes it to SPOKEN_FULL (o fuller story length). A
// tech/tool objeto ("ter you built a REST API", "o que projects ter you built") precisa Não ser
// pulled dentro de o behavioral lane (code-review caveat fix 2026-06-16).
describe('planAnswer + tier — behavioral story questions route to SPOKEN_FULL end-to-end', () => {
  const behavioral = [
    ...
    // indefinite-pronoun + crisis/difficult phrasings (code-review Alto 2026-06-16)
  ...
  const notBehavioral = [
    ...
    ['Have you managed a database?', 'skill_experience_answer'], // tech objeto → skill, não behavioral
...
describe('trimToSpeakable — removed (no-op, never truncates)', () => {
  // O trimmer era removed 2026-06-16. It é retained como a no-op para importar compatibility;
  // it precisa Nunca change an answer at qualquer length, então o answer é nunca cropped.
...
// classifySpeakability mapeia a SpeakabilityDecision to a coarse marker para telemetry.
...
describe('classifySpeakability — coarse marker mapping (Fix D)', () => {
  // Auxiliar to forge a decision objeto directly (tests o PURE mapping em isolation,
  // independent de o word/second math).
  const decision = (over) => ({
  ...
  // Real-path verifica via decideSpeakability (não synthetic objects): these são o
  // exact scenarios o tarefa spec calls ofora
  test('real path: long (>100w) experience_answer, no detail cue → over_budget', () => {
    ...
    assert.equal(d.exception, false);
    assert.equal(d.overBudget, true);
    // applySpeakabilityBudget classifies em o ORIGINAL decision (pre-trim), então o
    // class reflects that this answer era sobre budget.
    assert.equal(r.speakability_class, 'over_budget');
  });
...
describe('applySpeakabilityBudget — metadata (measure-only, never trims)', () => {
  test('reports word count, seconds, class — but never trims (applied always false)', () => {
    ...
    assert.equal(r.text, long, 'answer ships verbatim');
    assert.equal(r.spoken_word_count, 130); // reports o Completo length, não a trimmed one
  });
