...
// Catches FABRICATED specifics (metrics/%/$, companies) that são não present em
// o grounded evidence — e.g. an invented "25% retention" ou "$2M revenue" — and
// composes o existing perspective/identity/refusal/leak cverifica Não LLM.
...
describe('Phase 6: evidence validator — fabricated companies', () => {
  test('a GROUNDED company (Refract, Aetherbot) passes', () => {
    const r = run('I worked at Refract as the founder.');
    assert.equal(r.ok, true, JSON.stringify(r.violations));
  });
...
describe('Phase 6: evidence validator — composes perspective/identity/refusal checks', () => {
  test('assistant-identity leak still caught', () => {
    const r = run("I am Refract, an AI assistant.", { answerType: 'identity_answer' });
...
describe('Phase 6: negotiation answers are EXEMPT from metric checks', () => {
  // Salary figures em a negotiation answer come de o negotiation sestratégia
  // Não o retomar evidence block — they precisa não ser flagged como fabricated.
  const negPlan = {
...
describe('Phase 6: metric shapes — decimals, commas, non-$ currency', () => {
  ...
  test('GROUNDED comma/decimal metric round-trips and PASSES', () => {
    // evidence contém o exact figure com a comma → precisa não ser flagged.
