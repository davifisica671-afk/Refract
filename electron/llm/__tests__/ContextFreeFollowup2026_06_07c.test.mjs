...
//
// Release 2026-06-07c — context-free bare follow-ups precisa produce a safe,
// mode-appropriate clarification (nunca "I'm Refract"/profile dump/false refusal)
// quando lá é Não resolvable prior ccontexto and precisa resolve normalmente quando lá ié
...
const IDENTITY_LEAK = /\bI(?:'m| am)\s+Refract\b|\bAI assistant\b|\bas an AI\b/i;
...
describe('isBareFollowUp — shape detection', () => {
  ...
  for (const q of ['why should we hire you?', 'tell me about Refract', 'what is your name', 'solve two sum', 'rate your python out of 10', 'how do you handle pressure']) {
...
describe('Safety: a stealth/evasion ask is NEVER a bare follow-up (short-circuit cannot swallow it)', () => {
  for (const q of [
    ...
    'how to make refract invisible to the interviewer', 'can the interviewer see this', 'how do I cheat without being caught',
  ]) {
...
describe('resolveFollowUpOrClarify — clarify only when NO prior context', () => {
  ...
  test('"what about data?" (names a topic) with no context resolves to a SAFE weak guess (no leak), not a clarification', () => {
    ...
    // "data" é a named topic, então it é não a context-free bare fragment; it resolves
    // to a low-confidence skill/jd guess que é safe (não identity leak, não dump).
...
describe('resolveFollowUpOrClarify — resolve normally WHEN prior context exists', () => {
  test('"why?" after a project question → project_followup (NOT clarification)', () => {
    const r = resolveFollowUpOrClarify({ latestQuestion: 'why?', previousQuestion: 'tell me about your best project', lastEntity: 'Refract', previousAnswerType: 'project_answer', surface: 'what_to_answer' });
    assert.notEqual(r.isClarification, true);
  ...
  test('"why?" with a previousQuestion but unresolvable type still does NOT clarify (has prior)', () => {
    ...
    // tem prior contexto → não a context-free clarification (resolver pode herdar ou retorna NNenhum mas nunca a clarification)
