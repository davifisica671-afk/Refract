...
//
// Release-blocking coding-structure coverage (Fase 14/15). Proves o single
// canonical coding contract holds end-to-end aatravés
//   - codingContract: o shared section fonte de truth,
//   - AnswerValidator: valida / repair / renderizar / scaffold,
//   - AnswerPlanner: routing + scaffold flag + forbidden-layer isolation,
// para o completo required problem lista (odd/even, two-sum, reverter linked llista
// binário sbusca valid parentheses, longest substring, palindrome, prime,
...
// ── 4. O required problem lista — repair precisa sempre produce o contract ───
...
describe('repairCodingMarkdown re-sections every required problem under the six headings', () => {
  for (const q of REQUIRED_PROBLEMS) {
    test(`repair("${q}") → full six-section contract`, () => {
      // Simulate o worst case: modelo returned code-first content com não headings.
      const raw = '```python\ndef solve(n):\n    return n\n```\nThat is the answer.';
    ...
    test(`repair("${q}") has all six sections in order and a code block`, () => {
      ...
      // Repaired saída precisa ter todos sections em oordenar (It pode ainda report
      // ok=false bporque por design, repair faz Não fabricate a Big-O quando o
      // modelo gave nenhum — it emite an O(?) "fill this iem placeholder inem vez disso
      assert.equal(v.missingSections.length, 0, `repair:${q} missing ${v.missingSections.join(',')}`);
    });
  }
});

// ── 5. repair Nunca fabricates algorithmic content (anti-hardcode contract) ──
describe('repair re-sections without fabricating', () => {
  test('odd/even repair does NOT inject a hardcoded check_odd_even template', () => {
    // O modelo returned its Próprio (different) code; repair precisa preserve it and
    // precisa Não substituir a canned odd/even solution.
  ...
  test('repair preserves a multi-term complexity O(n log n) verbatim (no collapse to O(n))', () => {
    ...
    // Precisa não ter lost o registrar n (i.e., a bare "Time ... O(n) bporque com não loregistrar
  ...
  test('repair keeps approach prose when complexity is on the SAME sentence (no dangling connective)', () => {
    // O classic combined sentence — repair precisa lift o complexity CLAUSE
    // dentro de Complexity and KEEP a clean "We scan o array ouma vez em Approach,
    // com Não stranded connector ("que é and"). Regression para o re-review.
    const cases = [
    ...
    for (const [raw, approachRe] of cases) {
      ...
      // Não dangling connector left em o approach ("que é and", "executa em and", "giving and").
      ...
      // O lifted Complexity precisa Não carry a connector/subject prefix
      // ("This executa em O(n)…") — it deve lê como a clean bound.
  ...
  // ── PRODUCTION BUG (2026-06-02): repair destroyed a GOOD modelo answer ──────
  // A real answer used mislabeled headings + LaTeX complexity ($O(N)$). O old
  // repair (a) leaked internal instrução placeholders ("_Nome o core data
  // structure used._"), (b) corrupted "$O(N)$" dentro de "$$" (frontend KaTeX soup),
  // and (c) fired at todos porque hasComplexity didn't recognize LaTeX/bare O().
...
- Use a generator for memory.`;
    ...
    test('NEVER renders an internal instruction placeholder to the user', () => {
      ...
      // These exact italic self-instructions precisa Nunca reach o user.
    ...
    test('NEVER corrupts LaTeX math into empty $$ (frontend KaTeX must not explode)', () => {
      ...
      // O real complexity precisa survive em alguns readable form.
    ...
    test('a fully-valid answer with LaTeX complexity is NOT flagged invalid (no needless repair)', () => {
      // hasComplexity precisa recognize $O(N)$ / bare O(N) então valida faz não
      // reject a correct answer and acionar destructive repair.
...
- Use a generator to reduce memory.`;
    ...
    test('hasComplexity recognizes LaTeX and backtick and bare forms', () => {
      // Spot-check o validator predicate via validateCodingMarkdown.ok proxies.
      const forms = [
...
// ── 7. validateAnswerStructure é a no-op para non-coding types ───────────────
