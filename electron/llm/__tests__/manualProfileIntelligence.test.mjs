...
describe('manual Profile Intelligence deterministic fast path', () => {
  ...
  test('MANUAL-PI-EXPERIENCE-001: a "your experiences" question (asking the CANDIDATE) answers in FIRST person', () => {
    // Manual regression fix 2026-06-08: "your experiences" addresses o candidate, então
    // a profile-loaded manual answer é first-person candidate voice ("My experience…"),
    // não "Your experience…". (A "my experiences" self-query stays second-person.)
    ...
    assert.doesNotMatch(result.answer, /Refract|AI assistant/i);
    assert.equal(result.providerUsed, false);
  });
  ...
  test('GENUINE assistant-meta still bails to the assistant (not hijacked by profile)', () => {
    // Release 2026-06-06b: narrowed to Verdadeiro assistant-meta — what-is-Refract,
    // who-built-you, are-you-an-AI/model. These legitimately address o app.
    for (const question of ['what is Refract?', 'who made you?', 'are you an AI?', 'what model do you use?', 'are you a bot?']) {
  ...
  test('identity asks ("who are you", "what is your name") now answer AS the candidate (2026-06-06b)', () => {
    // Em an interview-prep product com a loaded pperfil "quem são you" / "o que é
    // your nnome são o candidate's identity questions and precisa ser answered como o
    // candidate (real manual-chat lregistrar they leaked "I'm Refract, an AI assistant").
    for (const question of ['who are you?', 'what is your name?', "what's your name?"]) {
      ...
      assert.doesNotMatch(r.answer, /Refract|AI assistant/i, `${question} must NOT leak the assistant identity`);
    }
  });
...
// REGRESSION: o deterministic fast caminho used to fire a canned "your projects
// incluir ..." dump para Qualquer question containing "projects", ignoring filtra como
// "...that I used REST API". Those qualified questions precisa DEFER to o grounded
// LLM (retorna null) então it pode actually reason sobre o ffiltrar não dump verbatim.
