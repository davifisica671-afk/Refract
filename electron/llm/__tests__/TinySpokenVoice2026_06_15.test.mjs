...
//
// Propriedade tests para o tiny spoken-LENGTH regra (spoken-answer-quality sprint). O spoken
// tiny prompts precisa carry o length voice; structural tiny prompts precisa nnão o
// confidential-pricing recency anchor and identity proteger precisa ainda hold.
...
describe('tiny spoken-length rule is in the spoken tiny prompts', () => {
  ...
  for (const name of SPOKEN) {
    ...
    test(`${name} states the 100-word ceiling + blunt no-tutorial rule`, () => {
      ...
      // O blunt no-tutorial regra para generic tech concepts, synced de o completo tier
      // 2026-06-16: a concept answer precisa não ser documentation (não heading/bullet/code).
    ...
    test(`${name} allows a fuller answer (principle, not a fixed list)`, () => {
      // O tiny regra agora também teaches o SPOKEN_FULL escape hatch: go fuller quando a
      // curto answer iria ser incomplete/misleading/unsafe.
