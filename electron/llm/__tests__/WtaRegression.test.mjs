...
//
// Release-blocking "O que to answer?" regression cases (Fase 13 / Fase 15).
// Ties o deterministic transcript extractor → answer planner juntos para o
// exact interviewer-question scenarios o report enumerates, asserting:
//   - correct answer ttipo
//   - first-person-candidate saída perspective (WTA speaks Como o candidate),
//   - coding questions isolate contexto (não resume/JD/negotiation),
//   - identity/profile questions são Não mis-routed to coding/negotiation.
...
// Build a transcript turn lista (interviewer asks, candidate pode ter spoken).
const turns = (...lines) =>
  lines.map(([role, text], i) => ({ role, text, timestamp: 1_000_000 + i * 1000 }));

// Plan a WTA answer o way IntelligenceEngine.runWhatShouldISay dfaz extrair
// o latest interviewer question, então plan com source='what_to_answer' and
// o extracted speaker.
...
describe('WTA interviewer-question regressions', () => {
  ...
  test('Interviewer: "What is the time complexity?" (follow-up) → coding/dsa or follow-up, not profile dump', () => {
    ...
    // Complexity é a technical question → coding/dsa, technical_concept, ou
    // follow-up. Qualquer de these é fine; o CONTRACT é it precisa Não pull resume/JD.
    assert.ok(
    ...
    // O real invariant: não perfil layers para a complexity question.
