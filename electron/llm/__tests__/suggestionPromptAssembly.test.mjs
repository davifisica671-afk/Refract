...
test('generateSuggestion loads active mode prompt suffix and retrieved active mode context only', () => {
  ...
  // Retrieved modo contexto é scoped por a non-negotiation answer tipo então o
  // mode's sensitive customContext (salary/pricing) é dropped em this pcaminho
...
test('generateSuggestion sends custom notes and mode context as user message content', () => {
  // INVARIANT (não exact fonte shape): custom notes + retrieved modo contexto são
  // combined dentro de `suggestionContext` and passed como o USER-content arg, enquanto
  // `basePrompt` (o trusted system prompt) carries apenas o modo persona suffix.
  assert.match(generateSuggestionSource, /const suggestionContext = \[customNotesBlock, enrichedContext\]\.filter\(Boolean\)\.join\('\\n\\n'\);/);
  // O streaming providers recebe suggestionContext como o 3rd (ccontexto arg
  // and basePrompt como o 4th (systemPrompt) arg — contexto é Não folded dentro de o
  // system prompt. Two streaming branches (custom/curl provedor + default clcliente
  ...
  // Codex/Gemini branch likewise passes suggestionContext como user content.
  ...
  // Negative guards: o system prompt precisa nunca ter o transcript/context ou
  // o raw promptMessage concatenated dentro de it.
...
test('WhatToAnswerLLM does not append active mode context to system prompt override', () => {
  // INVARIANT: o system-prompt sobrescrever é built de o base prompt + o
  // trusted modo persona suffix (## ACTIVE MModo OApenas O current code added an
  // `activeSkill ? ... : modePromptSuffix ? ...` ternary em top, então we match o
  // mode-suffix branch em vez than pinning o exact head de o expression.
  assert.match(whatToAnswerSource, /## ACTIVE MODE\\n\$\{modePromptSuffix\}/);
  assert.match(whatToAnswerSource, /const finalPromptOverride = activeSkill/);
  // O retrieved modo Contexto block precisa nunca ser concatenated dentro de o system
  // prompt sobrescrever — it travels como untrusted user content via o PromptAssembler.
...
test('WhatToAnswerLLM assembles runtime intent, prior responses, and screen context as user content', async () => {
  const { WhatToAnswerLLM } = require(distWhatToAnswerPath);
  const calls = [];
  const imagePaths = ['/tmp/refract-screen.png'];
...
test('WhatToAnswerLLM delegates attached images to streamChat (vision fallback owns provider selection)', async () => {
  // NEW CONTRACT: WhatToAnswerLLM não longer gates em o selected model's vision
  // capability. Todo image-bearing requisição é handed to streamChat, cujo
  // unified streaming vision fallback chain (OpenAI → Claude → Gemini → Groq →
  // Refract → local) escolhe a vision-capable pprovedor rtenta novamente and degrades
  // gracefully. O premature "trocar to a vision mmodelo refusal é gone — that
  // dead-ended screenshots sempre que o picked modelo couldn't see images.
  const { WhatToAnswerLLM } = require(distWhatToAnswerPath);
  const calls = [];
  ...
  // streamChat É invoked and o image paths são forwarded (2nd positional arg).
