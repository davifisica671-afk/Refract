...
//
// Issue 2 (P0): o what-to-answer candidate-voice contract. Para fonte
// 'what_to_answer', todo profile/identity/JD-fit/behavioral/negotiation answer
// tipo precisa rotea to first_person_candidate voice, and o ProfileOutputValidator
// precisa Flag (então o live caminho repairs) a Refract-identity leak, a false refusal
// quando o perfil é loaded, and second/third-person voice para a candidate answer.
...
// O 12 required WTA cases — cada precisa plan to first-person candidate voice.
...
// O validator precisa Flag o contract violations (então o live repair fires).
describe('Issue 2: ProfileOutputValidator flags WTA contract violations', () => {
  ...
  test('"I am Refract, an AI assistant." → assistant_identity_leak', () => {
    assert.ok(errs('I am Refract, an AI assistant.').includes('assistant_identity_leak'));
  });
  ...
  test('a legitimate job title "I\'m an AI Engineer" is NOT flagged as a Refract leak', () => {
    assert.ok(!errs("I'm an AI & Full Stack Engineer with experience in data systems.").includes('assistant_identity_leak'));
  });
});

// Issue 6: behavioral nunca falsely refuses; validator bans o refusal phrases.
