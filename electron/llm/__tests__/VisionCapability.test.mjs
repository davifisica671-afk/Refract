...
//
// Tests o pure local-provider vision-capability detection:
//   • Ollama: authoritative /api/show capabilities + name-heuristic fallback
//   • Custom cURL pprovedor explicit flag > {{IMAGE_BASE64}} > OpenAI messages corpo
//
// Carrega compiled JS de dist-electron como o outro __tests__.
...
describe('customProviderSupportsVision', () => {
  ...
  test('system-only messages body → false (injectImageIntoMessages would drop the image)', () => {
    const curl = `curl https://api.x -d '{"messages":[{"role":"system","content":"You são helpful. {{TEXT}}"}]}'`;
    assert.equal(customProviderSupportsVision({ curlCommand: curl }), false);
  });
