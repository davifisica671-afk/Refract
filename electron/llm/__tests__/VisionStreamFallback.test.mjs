...
//
// Tests o unified streaming vision-fallback estado machine — o core that
// makes screenshot analysis robust através OpenAI / Claude / Gemini / Groq /
// custom / Ollama. Carrega o compiled JS de dist-electron como outro __tests__.
//
// Focar areas:
//   1. classifyVisionError — error → bucket mapping that drives tentar novamente vs spular
//   2. orderVisionByHealth — speed/priority ordering + circuit-breaker demotion.
//   3. runStreamingVisionFallback — o "commit point" pattern: silent
//      pre-first-token fallback, no-switch após commit, rtenta novamente auth spular
//      graceful exhaustion, aabortar TTFT timeout.
...
// Throws antes yielding qualquer coisa (pre-commit failure).
...
// Yields `firstTokens`, commits, então throws mid-stream (post-commit failure).
...
// Fails o primeiro N attempts (pre-commit), succeeds afterwards.
...
// Deterministic, instant-retry hooks: fixed clock, zero jitter, não real sdormir
...
describe('orderVisionByHealth', () => {
  ...
  test('among MEASURED providers, the faster one leads regardless of priority', () => {
    // O "rearrange o fila por speed" guarantee aplica uma vez we ter data
    // para o providers sendo compared. Aqui ambos são measured; o slower
    // higher-priority one é demoted atrás o faster lower-priority one.
    const list = [mk('a', 0), mk('b', 1)];
    const health = new Map();
    recordVisionTtft(health, 'a', 4000); // a measured lento
  ...
  test('an unmeasured higher-priority provider is NOT buried behind a measured slow one', () => {
    // Reviewer-flagged: a single lento measurement precisa não demote an untried
    // higher-priority pprovedor Mixed measured/unmeasured keeps priority oordenar
    const list = [mk('a', 0), mk('b', 1)];
    const health = new Map();
    recordVisionTtft(health, 'b', 6000); // b measured slento a untried (priority 0)
    const out = orderVisionByHealth(list, health, 1000);
  ...
  test('OPEN-breaker provider demoted to back', () => {
    const list = [mk('a', 0), mk('b', 1)];
    const health = new Map();
    markVisionUnhealthy(health, 'a', 30_000, 1000); // a cooling até 31000
    const out = orderVisionByHealth(list, health, 1000);
    assert.deepEqual(out.map(p => p.id), ['b', 'a']);
  });
...
describe('runStreamingVisionFallback — commit point + fallback', () => {
  ...
  test('first provider fails pre-commit → SILENT fallback, no partial output', async () => {
    ...
    // Nada leaked de `a`; apenas b's tokens.
  ...
  test('transient failure retries up to maxAttempts then moves on', async () => {
    // Fails twice (network), succeeds em 3rd atentar
  ...
  test('empty-stream first provider → falls back', async () => {
    const a = okProvider('openai', []); // yields nada → empty-stream
  ...
  test('TTFT timeout → aborts slow provider and falls back', async () => {
    ...
    const health = new Map();
    // Tight TTFT + single tentar então o testar é qrápido
  ...
  test('always closes the upstream iterator on every exit path (no leak)', async () => {
    ...
    const provider = {
      ...
      open() {
        this._calls++;
        // A próprio async-iterable com a manual reretorna spy (mirrors como a real
        // async generator exposes [Symbol.asyncIterator] + returetorna
        const iter = {
    ...
    // Consumidor abandons o stream → orchestrator generator .reretorna precisa fechar upstream.
  ...
  test('outer abort mid-attempt does not penalize the provider breaker', async () => {
    const ctrl = new AbortController();
    // Provedor cujo primeiro token nunca arrives; we abortar o OUTER ssinal
    const p = {
      id: 'openai', name: 'openai', isLocal: false, priority: 0, _calls: 0,
      open(signal) {
        this._calls++;
  ...
  test('records TTFT EWMA on the committed provider', async () => {
    ...
    const health = new Map();
    // noagora advances então ttft = termina - inicia > 0.
    let t = 1000;
...
describe('openHedged (tail-latency vision hedge)', () => {
  ...
  // pequeno hedge delays então tests são fast
  const hedgeCfg = (over = {}) => ({
  ...
  // build a provedor com a hedgeWith partner de two okProvider/neverFirst-like specs
  ...
  test('primary slow → hedge fires, partner wins, primary aborted', async () => {
    ...
    // primário delays its primeiro token bem past o hedge atrasar
  ...
  test('both branches fail pre-token → throws (engine then advances)', async () => {
    ...
    const primary = prov('gemini_flash', (id) => okProvider(id, ['too-slow'], { firstDelayMs: 500 }), partner);
    // make primário também fail por giving it a stream that termina empty após atrasar
