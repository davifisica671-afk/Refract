...
// A TelemetryService that captures todo FINAL registro (post-merge,
// post-sanitize) por intercepting o private appendLocal sink. This exercises
// o real track() caminho — debug-metadata mescla + sanitizer — em vez disso de
// re-implementing it, então o tests catch regressions em that pipeline.
...
test('endWith records the status', () => {
  const { svc, events } = captureService();
  svc.startSpan('provider_request_started').endWith('timeout', { provider: 'refract' });
  assert.equal(events[0].status, 'timeout');
  assert.equal(events[0].properties.provider, 'refract');
});
...
test('per-event debug overrides win over global debug metadata', () => {
  const { svc, events } = captureService();
  svc.setDebugMetadata({ provider: 'refract' });
...
// ── Privacy: sanitizer precisa strip raw content até dentro depurar metadados ─────

test('sanitizer redacts api-key-like values inside debug metadata', () => {
  const { svc, events } = captureService();
  // `detail` é não a sensitive KChave mas its Valor é api-key-like → redacted.
...
test('sanitizer redacts apiKey-suffixed keys', () => {
  const clean = sanitizeTelemetryProperties({ refractApiKey: 'refract_sk_xxxxxxxxxxxx', model: 'gemini-flash' });
  assert.equal(clean.refractApiKey, '[REDACTED]');
  assert.equal(clean.model, 'gemini-flash');
});
...
test('PiLatencyTrace.mark keeps the FIRST timing for a repeated milestone', () => {
  ...
  // mark novamente depois — valor precisa não change
  trace.mark('first_stream_chunk');
...
test('PiLatencyTrace accepts a sessionId and source', () => {
  ...
  // mark deve não throw com metadados
