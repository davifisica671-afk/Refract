...
//
// Fase 11: o completo routing matrix — answerType + voicePerspective +
// profileContextPolicy para todo category em o improvement spec. Proves Ambos
// inclusion (perfil answers ground) AND exclusion (coding/technical/sales/
// lecture forbid prperfil plus o keystone voice/policy sdivide
//   "como iria you uso GraphQL?"  → candidate voice, perfil FORBIDDEN
//   "como ter you used GraphQL?"   → candidate voice, perfil REQUIRED
...
describe('matrix: projects + project follow-ups', () => {
  ...
  for (const q of ['How is Refract developed?', 'What was your role in Refract?', 'What was the hardest part of that project?', 'What tech stack did you use?', 'Why did you build it?', 'What did you learn from that project?']) {
  ...
  test('explicit entity is resolved', () => {
    assert.equal(p('How is Refract developed?').resolvedEntity, 'Refract');
  });
});
