...
// FollowUpResolver. Proves o Fase 4 scenario classes (A–G) termina to termina at o
// resolver layer (deterministic; não LLM).
...
describe('C. One-hour-later follow-ups resolve the remembered entity', () => {
  test('C1: 60-min later "hardest part of that project?" → Refract', () => {
...
describe('B. Delayed follow-ups after filler still resolve "that/there/it"', () => {
  for (const phrase of ['what was the tech stack there?', 'tell me about that project', 'how did you build it?']) {
    test(`"${phrase}" after 8 filler turns → Refract`, () => {
      const m = new SessionMemory();
      m.note('project', 'Refract', 1 * MIN, 'technical-interview');
      ...
      assert.equal(r.recalledEntity, 'Refract', `→ via ${r.resolvedVia}`);
    });
  }
});
...
describe('E. Corrections override earlier memory', () => {
  test('E1: "use TalentScope" correction → that project, not Refract', () => {
    const m = new SessionMemory();
