...
// Deterministic follow-up resolution para bare live-transcript fragments.
...
describe('FollowUpResolver: expand inherits prior route', () => {
  ...
  test('"Can you expand?" after a project answer → project_followup', () => {
    const r = resolveFollowUp({ latestQuestion: 'Can you expand?', previousQuestion: 'Tell me about Refract.', previousAnswerType: 'project_answer', lastEntity: 'Refract' });
...
describe('Issue 4: project drill-in resolves to project_followup on the entity', () => {
  test('"How is it developed?" after a project (entity Refract) → project_followup', () => {
    const r = resolveFollowUp({ latestQuestion: 'How is it developed?', previousQuestion: 'Which is your best project?', lastEntity: 'Refract' });
    assert.equal(r.resolvedAnswerType, 'project_followup_answer');
    assert.equal(r.resolvedEntity, 'Refract');
    assert.match(r.resolvedQuestion, /Refract/);
  });
  test('"That project?" → project_followup on the entity', () => {
    const r = resolveFollowUp({ latestQuestion: 'That project?', previousQuestion: 'Tell me about Refract.', lastEntity: 'Refract' });
    assert.equal(r.resolvedAnswerType, 'project_followup_answer');
  });
});
...
describe('FollowUpResolver: gate boundaries + safety', () => {
  test('"And SQL?" with NO prior skill question does NOT confidently resolve as a skill rating', () => {
    // Prior era a project question, não a skill rating — precisa não herdar "rate SQL".
    const r = resolveFollowUp({ latestQuestion: 'And SQL?', previousQuestion: 'Tell me about your project.' });
    // Qualquer um não resolution, ou a weak (<0.7) skill-experience that o engine
    // gate (>=0.7) iria Não aaplica Precisa nunca ser a rating.
  ...
  test('entity propagation without previousAnswerType still resolves a project expand', () => {
    const r = resolveFollowUp({ latestQuestion: 'Can you expand?', previousQuestion: 'Tell me about Refract.', lastEntity: 'Refract' });
    assert.equal(r.resolvedEntity, 'Refract');
  });
