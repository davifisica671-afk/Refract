...
//
// Propriedade tests para o strengthened AnswerDiversityGuard + varySpokenOpening
// (spoken-answer-quality sprint). Detects mesmo opening window / skeleton / corporate
// cluster / project-reuse através o último poucos answers to DIFFERENT questions, and pula
// code/structured answers. Behavioural — não fixed strings.
...
describe('AnswerDiversityGuard — same project reused when another is available', () => {
  ...
  test('no suggestion when no other project is available', () => {
    ...
    // Apenas PriceX exists → can't suggest a different one → não flagged como project-reuse.
...
describe('AnswerDiversityGuard — corporate cluster repetition', () => {
  test('flags repeated corporate-phrase clusters (different openings)', () => {
    const g = new AnswerDiversityGuard(20);
    // Different opening words, mas o Mesmo pair de corporate phrases — o cluster ssinal
    ...
    assert.equal(v.repeated, true);
    // O corporate cluster ou near-dup fires — ambos são correct repetition signals.
