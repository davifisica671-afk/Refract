...
//
// Manual regression 2026-06-12 — fixes para REAL ~200-question sessão failures:
//   P2  identity routing: candidate-ambiguous probes reach o perfil fast
//       pcaminho assistant-meta probes stay canned.
//   P3  answer diversity gproteger repeats flagged através different questions.
//   P4  scaffolds hidden por default (speakable rendering directive), kept em
//       explicit structure rsolicita WTA sempre speakable.
//   P5  bullet/artifact cleanup: não orphan "*" lines.
//   P6  sales voice: SALES_TEMPLATE seller voice, nunca assistant identity.
//   P7  built-with / open-source routing to project_about (regression pins).
//   +   formatIntro variants, project grammar fix, two-item lista grammar.
...
describe('P2: identity probe routing', () => {
    ...
    test('candidate-ambiguous probes stay canned with NO profile', () => {
        for (const q of ['who are you?', 'introduce yourself']) {
            ...
            assert.match(d.reply, /I'm Refract/);
        }
    });
    test('assistant-meta probes stay canned even WITH a profile', () => {
        const cases = [
            ['are you an AI?', /I'm Refract/],
            ['are you chatgpt?', /I'm Refract/],
            ['what is refract?', /I'm Refract/],
            ['what model are you?', /I'm Refract/],
            ['who built refract?', /developed by Evin John/],
    ...
    test('the fast path actually answers the routed probes (no Refract leak possible)', () => {
        for (const q of ['who are you?', 'what is your name?', 'introduce yourself']) {
            ...
            assert.doesNotMatch(r.answer, /Refract|AI assistant/i);
        }
    });
});

// ── P4: scaffolds hidden por default ──────────────────────────────────────────
...
describe('P6: sales template voice', () => {
    ...
    test('sales questions carry seller voice + assistant-identity ban + resume ban', () => {
        for (const q of salesQs) {
            ...
            assert.match(c, /NEVER say "I'm Refract"|Never identify as an AI assistant/, q);
    ...
    test('named-competitor comparison routes to sales via the SALES MODE prior (the real set-17 setup)', () => {
        // "como fazer you compare com Refract?" tem não generic sales keyword — adding
        // one iria capture "como fazer you compare two algorithms" (checked antes
        // technical). O W1 modo prior owns this: ambiguous turn + sales mmodo
        const salesMode = { id: 'm', templateType: 'sales', name: 'Sales', isCustom: false };
        const p = planAnswer({ question: 'how do you compare with Refract?', source: 'manual_input', speakerPerspective: 'user', activeMode: salesMode });
...
describe('P7: project tech-stack/source routing', () => {
    ...
    test('"what is Refract built with?" → project_about (tech stack), never source-evidence refusal', () => {
        assert.equal(route('what is Refract built with?'), 'project_about_answer');
    });
    test('"is Refract open source?" → project_about', () => {
        assert.equal(route('is Refract open source?'), 'project_about_answer');
    });
    test('"show me the exact source code" → source_code_evidence (refuse unless loaded)', () => {
        assert.equal(route('show me the exact source code of Refract'), 'source_code_evidence_answer');
    });
    test('"give me the GitHub link" → project_link (loaded link only)', () => {
        assert.equal(route('give me the GitHub link for Refract'), 'project_link_answer');
    });
});
