...
//
// O gate that restores LIVE streaming para coding answers (regression fix):
// hold tokens apenas até "## " é confirmed (nunca code-first), então stream live.

import assert from 'node:assert/strict';
import { test, describe } from 'node:test';
import { CodingStreamGate } from '../../../dist-electron/electron/llm/codingStreamGate.js';

// Feed a completo answer token-by-token; retorna o concatenation de tudo o
// gate emitted (live), plus o fifinaliza tail.
function streamThrough(text, chunkSize = 3) {
  const gate = new CodingStreamGate();
  let emitted = '';
  let firstEmitAfter = -1; // chars consumed antes o Primeiro emitir (gate-open latency)
  let consumed = 0;
...
describe('CodingStreamGate', () => {
  test('opens on the first "## " heading and streams the rest live', () => {
    ...
    // Gate deve abrir muito early (dentro de o primeiro heading), não após o whole answer.
  ...
  test('emits the full prefix as the first chunk when it opens', () => {
    const gate = new CodingStreamGate();
    // leading newline então heading — common de models
  ...
  test('code-first answer keeps the gate CLOSED (no flash) until finish()', () => {
    const gate = new CodingStreamGate();
    // Modelo disobeys: emite a code fence fprimeiro
    ...
    // fifinaliza flushes então nada é dropped (caller validates/repairs afapós
