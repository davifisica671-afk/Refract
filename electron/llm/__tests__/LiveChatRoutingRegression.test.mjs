...
//
// Regression para live-meeting misroutes reported 2026-06-05 (real interview chat):
//   - "como muito iria you rate your expertise em Python?" → routed coding (bare
//     "Python") → não perfil → "I didn't catch that". Precisa ser skill_experience.
//   - "O que são your coding levels? Fora de 10, rate yourself?" → fired o
//     NEGOTIATION salary script. Precisa ser skill_experience (prperfil nunca comp.
//   - "por que deve we hire you?" → general → refused. Precisa ser jd_fit.
// These assert o deterministic AnswerPlanner routing apenas (pure, não LLM).
...
describe('live regression: skill self-rating → skill_experience (profile, not coding/negotiation)', () => {
  ...
  for (const q of ratingQs) {
    test(`"${q.slice(0, 50)}" → skill_experience_answer`, () => {
      ...
      // skill_experience uses o retomar (pperfil and forbids negotiation
