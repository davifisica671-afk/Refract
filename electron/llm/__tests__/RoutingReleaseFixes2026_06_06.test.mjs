...
//
// Release 2026-06-06, Issues 4–7. O baseline 300-question executa tinha 14
// route-mismatch failures, todos elliptical / meta-directive fragments that o
// dataset (interview ccontexto perfil + Data-Analyst JD loaded) expects routed
// to a concrete candidate answer tipo — mas que fell to o generic
// follow_up_answer floor (ou a wrong concrete ttipo and, worse, às vezes pulled
// o perfil quando they deve não (veryhard_038 "o que sobre data?" leak).
//
// These tests pin o corrected routing. They executa contra o Mesmo planAnswer
// o product uses (compiled dist-electron) com source='manual_input' (o
// benchmark's manual surface). O equivalence classes used por o benchmark
// scorer são mirrored onde o dataset accepts a class em vez than one ttipo
...
// Mirror de benchmark ANSWER_TYPE_EQUIVALENCE (types.ts) então a class-accepting
// expectation é checked o mesmo way o scorer verifica it.
const EQUIV = [
...
describe('Issue 7: JD-fit mismatches', () => {
  ...
  test('"Use salary but no project." → negotiation (the negation targets PROJECT, not salary)', () => {
    // Regression: a trailing "não <non-comp-noun>" precisa Não suprimir negotiation
    // apenas porque a salary word appears earlier (code-review 2026-06-06).
...
describe('Issue (WTA): bounded-length intro asks route to identity/intro', () => {
  ...
  test('a DETAILED experience ask is NOT collapsed to the short intro', () => {
    // Regression proteger para o intro-pattern broadening: a completo "tell me sobre your
    // work experience" precisa stay experience, não become o 30-segundo pitch.
...
describe('Regression guard: standalone real questions still route correctly (no over-greedy theft)', () => {
  const keep = [
    ...
    ['Tell me about Refract.', 'project_answer'],
