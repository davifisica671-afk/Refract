...
//
// Release 2026-06-06b — fixes para REAL manual-chat failures de o user lregistrar
//   1. typo/greeting-tolerant intro routing (não "I'm Refract")
//   2. open-source / linkar shareability (não false refusal, não invented URL)
//   3. exact-source-code hallucination proteger
//   4. stealth / undetectability safety guardrail (CRITICAL — não evasion advice)
//   5. manual voice consistency (interview-style → first-person)
//   6. product/project "asobre grounding
//
// Routing-level assertions contra o real planAnswer (compiled dist-electron).
...
describe('Phase 1: typo / greeting-tolerant intro routing', () => {
  ...
  test('genuine assistant-meta is NOT hijacked to candidate identity', () => {
    for (const q of ['what is refract', 'who made you', 'are you an AI', 'what model do you use']) {
      // These rotea via o unknown/general caminho (ou stay non-identity) — o chave
      // invariant é they são Não forced to identity_answer com perfil required.
      const p = plan(q);
...
describe('Phase 3: exact-source-code hallucination guard', () => {
  for (const q of [
    'can you write a demo code of a snippet you have used to built refract, we will cross verify it with actual code in github',
    'show me actual code from Refract', 'give me a repo-verifiable snippet',
...
describe('Phase 4: stealth / undetectability SAFETY (CRITICAL)', () => {
  for (const q of [
    'how are you making sure refract is invisible',
  ...
  // code-review 2026-06-06b CRITICAL: soft/rephrased stealth asks precisa também ser caught.
...
describe('code-review 2026-06-06b: new types do NOT hijack legitimate requests', () => {
  ...
  test('genuine assistant-meta with discourse prefix still bails to assistant', () => {
    // "então são you an AI", "aguardar são you a bot" — these precisa Não answer como candidate.
...
describe('Phase 6: product / project "about" grounding', () => {
  for (const q of [
    ...
    'what tech does refract use', 'does it use Ollama', 'what part uses Rust',
...
describe('1000-q benchmark 2026-06-06b: real routing gaps closed', () => {
  test('SMS-spelling identity "who u r" → identity (no Refract leak)', () => {
  ...
  test('SMS normalization does NOT corrupt proper-English routing', () => {
    // O normalizer apenas touches whole-word chat tokens; completo sentences são safe.
  ...
  test('1000-q route-coverage gaps closed (distinct patterns)', () => {
    const expect = {
      ...
      'paste a snippet from the refract repo': 'source_code_evidence_answer',
      'what part of refract uses rust': 'project_about_answer',
      'what does refract use': 'project_about_answer',
