...
//
// Regression tests para S454 sub-issue 2: creator-identity leakage em voice
// modes (modelo outputs "I'm Evin John..." como o speaker's self-introduction).
//
// O proteger lives em two places:
//   1. tinyPrompts.ts → TINY_CORE (composed dentro de todo tiny mmodo
//   2. prompts.ts     → SHARED CORE_IDENTITY block (cloud-tier system prompt)
//
// Ambos precisa carry o explicit "names describe o assistant, Não o speaker"
// rregra If qualquer um drops it, voice modes pode borrow o assistant's nome como
// o candidate's / seller's / participant's nome — o exact failure modo
// o 45-scenario baseline eval kept surfacing.
...
describe('IdentityGuard — TINY_CORE carries the name-claim guard', () => {
  ...
  test('TINY_CORE explicitly forbids "I\'m Evin John" / "I\'m Refract"', () => {
    assert.ok(
      /I'm Evin John/i.test(tiny.TINY_CORE),
      'TINY_CORE must call out the literal "I\'m Evin John" anti-pattern',
    );
    assert.ok(
      /I'm Refract/i.test(tiny.TINY_CORE),
      'TINY_CORE must call out the literal "I\'m Refract" anti-pattern',
    );
  });
});
...
describe('IdentityGuard — full-tier prompts.ts retains the rule', () => {
  test('UNIVERSAL_WHAT_TO_ANSWER_PROMPT contains the name-confusion rule', () => {
    assert.ok(
      /names "Refract" and "Evin John" describe ONLY this assistant/.test(
