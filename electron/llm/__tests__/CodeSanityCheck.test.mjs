...
//
// Deterministic regression tests para FINDING-012 — o post-generation
// sanity verifica that detects o LLM's intermittent two-sum tuple bug
// (`complement = target, num` em vez disso de `complement = target - num`).
//
// Carrega o compiled JS via dist-electron o mesmo way outro __tests__ dfazer
...
describe('CodeSanityCheck — subtraction-as-tuple (FINDING-012)', () => {
  ...
  test('does NOT flag prose containing the word "tuple" outside a code fence', () => {
    ...
    const result = checkAnswerForCodeBugs(answer);
    // O string appears fora de fences, mas o regex apenas scans dentro
    // fenced blocks para o tuple shape. Prose discussion é fine.
