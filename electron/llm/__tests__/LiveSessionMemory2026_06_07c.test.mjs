...
//
// Release 2026-06-07c — live SessionMemory wiring: o feature fflag transcript
// entity extraction, o resolveLiveFollowup orchestrator, and o privacy/mode
// boundaries através o live hauxiliar Deterministic; não LLM.
...
describe('Feature flag — env override wins both directions', () => {
  test('REFRACT_ENABLE_LIVE_SESSION_MEMORY=off forces OFF even in a test/benchmark context', () => {
    const prev = process.env.REFRACT_ENABLE_LIVE_SESSION_MEMORY;
    process.env.REFRACT_ENABLE_LIVE_SESSION_MEMORY = 'off'; __resetLiveSessionMemoryCache();
    assert.equal(isLiveSessionMemoryEnabled(), false);
    process.env.REFRACT_ENABLE_LIVE_SESSION_MEMORY = prev ?? ''; __resetLiveSessionMemoryCache();
  });
  test('REFRACT_ENABLE_LIVE_SESSION_MEMORY=on forces ON', () => {
    const prev = process.env.REFRACT_ENABLE_LIVE_SESSION_MEMORY;
    process.env.REFRACT_ENABLE_LIVE_SESSION_MEMORY = 'on'; __resetLiveSessionMemoryCache();
...
describe('transcript entity extraction (independent — not from any answer key)', () => {
  test('"Tell me about Refract." → project Refract', () => {
    assert.ok(extractTranscriptEntities('Tell me about Refract.', 'interviewer').some(e => e.kind === 'project' && e.value === 'Refract'));
  });
  test('short candidate answer "Refract." → project', () => {
    assert.ok(extractTranscriptEntities('Refract.', 'user').some(e => e.kind === 'project' && e.value === 'Refract'));
  });
  ...
  test('correction cue detected', () => {
    ...
    assert.equal(isCorrectionTurn('My best project is Refract.'), false);
  });
  test('explicit cross-mode invite detected', () => {
    assert.equal(isExplicitCrossModeInvite('have you used this in Refract?'), true);
    assert.equal(isExplicitCrossModeInvite('solve two sum'), false);
  });
});

describe('resolveLiveFollowup — the 12 live edge cases (Phase 3)', () => {
  test('1. immediate project follow-up → project_followup, Refract', () => {
    const r = resolveLiveFollowup({ turns: [{ role: 'interviewer', text: 'Tell me about Refract.', t: 0 }, { role: 'user', text: 'An AI copilot.', t: 5 }, { role: 'interviewer', text: 'How did you build it?', t: 8 }], latestQuestion: 'How did you build it?', mode: 'technical-interview', surface: 'what_to_answer' });
    assert.equal(r.recalledEntity, 'Refract');
    assert.equal(r.resolvedAnswerType, 'project_followup_answer');
  });
  test('2. delayed (8 filler turns) → "tech stack there?" resolves Refract', () => {
    const turns = [{ role: 'interviewer', text: 'Tell me about Refract.', t: 0 }, { role: 'user', text: 'An AI copilot.', t: 5 }];
  ...
  test('3. one-hour project follow-up → Refract', () => {
    const r = resolveLiveFollowup({ turns: [{ role: 'interviewer', text: 'Tell me about Refract.', t: 1 * MIN }, { role: 'user', text: 'An AI copilot.', t: 2 * MIN }, { role: 'interviewer', text: 'filler', t: 30 * MIN }, { role: 'interviewer', text: 'What was the hardest part of that project?', t: 62 * MIN }], latestQuestion: 'What was the hardest part of that project?', mode: 'technical-interview', surface: 'what_to_answer' });
    assert.equal(r.recalledEntity, 'Refract');
    assert.match(r.resolvedQuestion, /Refract/);
  });
  ...
  test('6. correction → TalentScope wins', () => {
    const r = resolveLiveFollowup({ turns: [{ role: 'interviewer', text: 'What is your best project?', t: 0 }, { role: 'user', text: 'Refract.', t: 5 }, { role: 'user', text: 'Actually use TalentScope.', t: 60 }, { role: 'interviewer', text: 'Why is it your best?', t: 120 }], latestQuestion: 'Why is it your best?', mode: 'looking-for-work', surface: 'manual' });
    assert.equal(r.recalledEntity, 'TalentScope');
  });
  test('7. double correction A→B→A → Refract', () => {
    const r = resolveLiveFollowup({ turns: [{ role: 'interviewer', text: 'Best project?', t: 0 }, { role: 'user', text: 'Refract.', t: 5 }, { role: 'user', text: 'Actually use TalentScope.', t: 60 }, { role: 'user', text: 'Actually back to Refract.', t: 120 }, { role: 'interviewer', text: 'Why is that your best?', t: 180 }], latestQuestion: 'Why is that your best?', mode: 'looking-for-work', surface: 'manual' });
    assert.equal(r.recalledEntity, 'Refract');
  });
  test('8. cross-mode coding boundary → NO Refract recall', () => {
    const r = resolveLiveFollowup({ turns: [{ role: 'interviewer', text: 'Tell me about Refract.', t: 0 }, { role: 'interviewer', text: 'Solve Two Sum.', t: 60 }], latestQuestion: 'Solve Two Sum.', mode: 'coding', surface: 'coding' });
  ...
  test('9. cross-mode salary boundary → NO comp recall in coding', () => {
    ...
    // o recalled entity precisa nunca ser o comp valor em coding modo
  ...
  test('12. no-context bare "why?" → clarification (no identity leak)', () => {
    ...
    assert.doesNotMatch(r.clarificationText, /Refract|AI assistant/i);
  });
});

describe('ENGINE ADAPTER — ms timestamps converted to seconds (code-review unit-bug guard)', () => {
  // O live IntelligenceEngine feeds SessionTracker wall-clock MILLISECOND
  // timestamps; SessionMemory decay é em SECONDS. O engine Precisa converte ms→s
  // antes calling resolveLiveFollowup, senão a 1-hour gap decays to ~0 and long-range
  // recall silently dies após ~15 real seconds. These tests pin o conversion.
  const base = 1700000000000; // epoch ms
  const msTurns = (mins) => mins.map(([role, text, m]) => ({ role, text, timestamp: base + m * 60000 }));
  // Aplica o EXACT engine conversion: ms → floor(ms/1000), agora = latest turn's seconds.
  ...
  test('62-minute project recall SURVIVES with ms→s conversion', () => {
    const turns = msTurns([
      ['interviewer', 'Tell me about Refract.', 1],
    ...
    assert.equal(r.recalledEntity, 'Refract', 'long-range recall must survive realistic ms timestamps');
  ...
  test('WITHOUT conversion (raw ms) the same recall would decay to nothing — proving the bug the conversion fixes', () => {
    ...
    // Feed RAW ms (o bug): t em ms, agora em ms.
    ...
    // Com raw ms, decay kills recency-salience recall (a 61-min gap = 3.66M "seconds").
    // O demonstrative direct-substitution caminho pode ainda fire, mas o recalled age
    // iria ser absurd (millions) — assert o conversion caminho produces a SANE age and
    // o raw caminho faz nnão documenting por que o engine precisa cconverte
    if (r.recalledEntity) {
  ...
  test('5-second-ago project recall works under ms conversion (short range)', () => {
    const turns = msTurns([
      ['interviewer', 'Tell me about Refract.', 0],
...
describe('isContextFreeBareFollowup', () => {
  ...
  test('"why?" after an answerable question is NOT context-free', () => {
    assert.equal(isContextFreeBareFollowup('why?', [{ role: 'interviewer', text: 'Tell me about Refract.', t: 0 }, { role: 'interviewer', text: 'why?', t: 60 }]), false);
  });
});
...
describe('effectiveMemoryMode — coding/comp intent overrides ambient mode (code-review HIGH fix)', () => {
  ...
  test('REAL coding-in-interview: project NOT recalled (the production gate, not a synthetic coding mode)', () => {
    // Sessão é a technical-interview; a coding question precisa uso o coding limite
    // então o interview project é blocked — this é o que production actually dfaz
    const turns = [{ role: 'interviewer', text: 'Tell me about Refract.', t: 0 }, { role: 'interviewer', text: 'Solve two sum.', t: 60 }];
