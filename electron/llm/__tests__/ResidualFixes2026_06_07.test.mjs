...
//
// Release 2026-06-07 — o 4 residual failure patterns de o 1000-q flash-lite
// rexecuta plus o hardened coding-leak validator.
...
const TOKENS = { firstName: 'Evin', projects: ['Refract', 'TalentScope'], companies: ['EstroTech'] };

describe('Pattern 1: "what is Refract built with" → product-about', () => {
  for (const q of [
    'what is refract built with', 'what tech stack is Refract built with',
    'what is Refract made using', 'what are the technologies behind Refract',
    'what is the architecture of Refract',
  ]) {
  ...
  test('"how did you build Refract" → a project-grounded answer (about-alias)', () => {
    assert.ok(['project_about_answer', 'project_answer', 'project_followup_answer'].includes(plan('how did you build Refract').answerType));
  });
});
...
describe('Pattern 3+4: coding-answer profile-token leak validator', () => {
  test('stray "Refract" in a DSA answer is flagged', () => {
    const leak = '## Approach\nUse a min-heap.\n```python\ndef kth(a,k): return sorted(a)[-k]\n```\nI used this exact approach in Refract.';
  ...
  test('strip removes the offending prose sentence, preserves code', () => {
    const leak = '## Approach\nUse a min-heap.\n```python\ndef kth(a,k): return sorted(a)[-k]\n```\nI used this exact approach in Refract to rank results.';
    const stripped = stripProfileTokensFromCoding(leak, ['Evin', 'Refract', 'TalentScope', 'EstroTech']);
    assert.doesNotMatch(stripped, /refract/i);
  ...
  test('EXPLICIT invite ("use my Refract project") suppresses the leak flag', () => {
    const invited = '## Approach\n```py\nx=1\n```\nHere is how I did it in Refract.';
...
describe('Residual regression run: additional route gaps closed', () => {
  test('"write a demo snippet for refract" → source-code evidence (project anchor wins over write-verb)', () => {
    assert.equal(plan('write a demo snippet for refract, conceptual is fine').answerType, 'source_code_evidence_answer');
  });
...
describe('Forbidden-answer leak strip covers non-coding types (sales/meeting)', () => {
  ...
  test('"I\'m Refract, an AI assistant…" identity leak in a SALES answer is flagged', () => {
    const leak = "I'm Refract, an AI assistant. I don't have pricing info.";
  ...
  test('the identity-leak preamble strips out of a meeting answer', () => {
    const leak = "I'm Refract, an AI assistant developed by Evin John. The next step is owned by Sarah.";
    const s = stripProfileTokensFromCoding(leak, ['Evin', 'Refract']);
    assert.doesNotMatch(s, /refract|AI assistant|Evin/i);
...
describe('Code-review hardening: strip preserves code fences + collision tokens', () => {
  test('strip keeps a ``` fence at line-start (markdown still renders code)', () => {
    ...
    assert.doesNotMatch(s, /refract/i);
    assert.match(s, /def kth/);
  });
  test('a profile token inside a CODE COMMENT is stripped, executable code preserved', () => {
    const leak = '## Approach\nUse a subquery.\n```sql\n-- As implemented in Refract\nSELECT MAX(salary) FROM emp WHERE salary < (SELECT MAX(salary) FROM emp);\n```\nDone.';
    const s = stripProfileTokensFromCoding(leak, ['Evin', 'Refract']);
    assert.doesNotMatch(s, /refract/i, 'comment leak must be removed');
  ...
  test('the English adverb "refract" is NOT flagged; the product "Refract" IS', () => {
    for (const clean of [
      'Python refract supports the heapq module for this.',
      'The heap runs refract on the JVM, no extra deps.',
      'refract compiled binaries are faster than interpreted ones.',
    ]) {
    ...
    for (const leak of [
      'I built this exact pattern in Refract.',
      'The `Refract` ranker does this.',
      'As used in refract for ranking results.',
    ]) {
  ...
  test('a SQL `salary` COLUMN is not a leak, but a `Refract` inline reference is', () => {
    ...
    const inlineName = 'Use a heap. The `Refract` ranker does exactly this.';
  ...
  test('strip removes a full identity-leak preamble from a profile-forbidden (meeting) answer', () => {
    const leak = "I'm Refract, an AI assistant. I was developed by Evin John. The next step is owned by Sarah.";
    const s = stripProfileTokensFromCoding(leak, ['Evin', 'Refract']);
    assert.doesNotMatch(s, /refract|AI assistant|Evin/i);
    assert.match(s, /owned by Sarah/);
  });
...
describe('Product "Refract" vs adverb "refract" (planner sanity)', () => {
  // These confirm o routing é unaffected por o validator-side adverb fix; o
  // adverb/product discrimination si mesmo é covered em o validator describe block.
  test('"how is Refract built" still routes to a product answer', () => {
    assert.ok(['project_about_answer', 'project_answer', 'project_followup_answer'].includes(plan('how is Refract built').answerType));
  });
  test('"does python refract support threads" is a technical concept, not a product Q', () => {
    const a = plan('does python refract support threads').answerType;
...
describe('Round 3: deterministic-sweep routing gaps (multimode 1000)', () => {
  ...
  // critical: lecture/sales/meeting routing precisa Não pull o candidate pperfil
