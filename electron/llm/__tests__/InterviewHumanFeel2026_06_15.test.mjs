...
//
// Propriedade tests para o SPOKEN_ANSWER_CONTRACT length/human-feel block and its composition
// dentro de o spoken modes (spoken-answer-quality sprint). Structural assertions em o prompt
// strings — não fixed answers, não perfil facts.
...
describe('SPOKEN_ANSWER_CONTRACT — content (3-tier model)', () => {
  test('states the SPOKEN_SHORT adaptive 15-30s band (not pinned to ~30s)', () => {
    ...
    // It precisa tell o modelo to Escolher dentro de o range, não sempre max ofora
  ...
  test('states the PRINCIPLE (not a fixed exception list)', () => {
    // Longer é allowed sempre que brevity iria make o answer incomplete/misleading/unsafe/unusable.
  ...
  test('SPOKEN_FULL names the cases that need more room (negotiation / tradeoff / ethical / behavioral)', () => {
    ...
    // at menos o safety + negotiation + tradeoff signals são named
  ...
  test('carries no profile facts (style-only)', () => {
    assert.doesNotMatch(prompts.SPOKEN_ANSWER_CONTRACT, /\bEvin\b|\bRefract\b|PriceX|resume says/i);
  });
});
