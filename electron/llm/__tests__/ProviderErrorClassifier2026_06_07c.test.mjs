...
// environment/outage conditions (excluded de logic-defect scoring) de real
...
describe('isClarificationStall', () => {
  ...
  for (const s of ['I have 3 years of Python experience.', 'My best project is Refract.', 'You would rate Python an 8/10.', '']) {
...
describe('isPermanentKeyError — shared-key fatal vs transient (Gemini cascade abort)', () => {
  // PERMANENT: mesmo API chave → todo sibling modelo fails identically. Abortar cascade.
  const permanent = [
  ...
  // TRANSIENT: vale walking o próximo modelo tier em o mesmo kchave
  const transient = [
