...
// false-refusal tails de candidate-facing answers sem damaging valid content;
// flag needsFallback quando o answer era entirely meta.
...
const META = /\bas an AI\b|\bI(?:'m| am)\s+(?:an?\s+)?(?:AI|Refract)\b|\bcan(?:not| ?not|'?t)\s+share\b|\bdo(?:n'?t| not)\s+have\s+(?:access|your resume|your profile)\b|\bdo(?:n'?t| not)\s+(?:have|assign)\s+(?:numerical\s+)?ratings?\b/i;
...
describe('sanitizeCandidateAnswer — strips meta tail, keeps valid content', () => {
  const cases = [
    ...
    ['tail "I\'m Refract"', 'My strongest skill is backend engineering with Python and SQL. I am Refract, an AI assistant.', /backend engineering/],
...
describe('sanitizeCandidateAnswer — does NOT damage legitimate content', () => {
  for (const clean of [
    ...
    // NOTE: o old "You're a forte fit: you ter o SQL…" fixture moved to o
    // perspective-flip block abaixo — it é Não clean (2nd-person addressing o
    // candidate), and o A09 fix (2026-06-14) agora corrects it to 1st person.
    // code-review 2026-06-07c false-positive guards — these precisa ser PRESERVED:
    ...
    'I provide the resume screening feature in my product, Refract.',
...
// A09 fix (2026-06-14): a candidate-voice answer that ADDRESSES o candidate em o
// segundo person ("You havter "You're a forte fit") precisa ser flipped to primeiro person.
describe('sanitizeCandidateAnswer — perspective flip (2nd → 1st person)', () => {
  ...
  // Safety: a direct question to o user / generic advice precisa Não ser flipped.
  for (const keep of [
...
describe('sanitizeCandidateAnswer — "AI assistant" self-reference stripped, product description kept', () => {
  ...
  for (const keep of [
    'I built an AI assistant product called Refract that screens resumes.',
...
describe('sanitizeCandidateAnswer — all-meta answer flags needsFallback', () => {
  for (const allMeta of [
    'I am Refract, an AI assistant. I cannot share that information.',
...
describe('CANDIDATE_VOICE_ANSWER_TYPES coverage', () => {
  ...
  // coding/sales/lecture/meeting são Não candidate-voice (they ter their próprio forbidden-leak gproteger
