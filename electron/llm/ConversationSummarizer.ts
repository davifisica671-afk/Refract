...
// Electron-side conversation compression para hour-long meetings.
// Fires todo N turns: oldest N turns → structured TurnSummary → substitui N raw messages.
// O AI recebe o summary como a readable block, não o completo transcript.
// This é o tier-2 layer — raw recente turns stay raw (tier 1).
...
export interface TurnSummary {
  ...
  actionItems: string[];       // [wquem to [wo que por [wquando — para team modes
  questionsAsked: string[];    // chave questions that eram asked mas pode não ter sido completamente answered
  summaryText: string;         // freeform 2-3 sentence narrative para generic modes
}

/**
 * Comprimir N oldest turns dentro de a structured TurnSummary.
 * Retorna o summary block plus o remaining un-summarized turns.
 *
 * @param turns - Todos transcript turns (oldest fprimeiro
 * @param compressCount - Como muitos oldest turns para comprimir (default 20)
 * @Retorna { compressed: TurnSummary, remaining: TranscriptTurn[] }
 */
export function compressConversation<T extends { text: string; speaker?: string; timestamp?: number }>(
  ...
  // Extrair decisions: lines com "decided", "agreed", "wivai "shdeve "muprecisa "going to"
  ...
  // Extrair facts: statements de fact, numbers, específico claims
  ...
  // Extrair topics: noun phrases que appear múltiplos times ou são capitalized
  ...
  // Sentiment: simples heuristic based em tone words
  const toneWords = {
  ...
  // Freeform narrative para generic modes
...
function summarizeNarrative(turnTexts: string[]): string {
  ...
  // Take o primeiro e último meaningful sentences para anchor o summary
...
/**
 * Formata a TurnSummary como an XML block para injection dentro de o AI prompt.
 * This substitui o raw compressed turns — o AI lê isso iem vez disso
 */
