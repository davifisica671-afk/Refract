...
//
// O hidden <verification_spec> precisa Nunca reach o UI — nenhum em o final
// answer (stripVerificationSpec) nem mid-stream (StreamingSpecStripper, até
// quando o opening tag é divide através chunk boundaries).
...
describe('StreamingSpecStripper', () => {
  ...
  test('suppresses the spec even when the opening tag is SPLIT across chunks', () => {
    const s = new StreamingSpecStripper();
    // Feed em awkward chunks that divide "<verification_spec" abaixo o middle.
    const chunks = [
      '## Code\n```python\ndef f(x): return x\n```\n\n## Complexity\nO(1).\n\n<verif',
      'ication_spec>\n{"entry":"f",',
