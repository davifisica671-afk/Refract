...
//
// Unit tests para o dependency-free core de verified code execution:
// extraction (spec, code block, problem examples, memescla drivers (templating +
// result parsing), and o judge (safe valor comparison).
...
describe('mergeTestCases', () => {
  test('problem cases first, dedupes identical, caps', () => {
    ...
    const model = [
      { input: [[1]], expected: 1, source: 'model' }, // dup de problem
...
describe('buildDriver + parseDriverResult', () => {
  test('python driver embeds code, entry, and sentinels', () => {
    ...
    assert.match(d.source, /REFRACT_TC/);
  ...
  test('REJECTS a non-identifier entry (template-injection guard) → null driver', () => {
    // An entry that tries to break fora de o template / inject code precisa yield
    // null (orchestrator então spula — nunca an injectable driver.
    ...
    // A normal identifier é ainda accepted.
  ...
  test('isLocallyRunnable: python/js/cpp/java/go (compiled langs gated on toolchain at run time)', () => {
    ...
    // SQL executa locally também mas via its Próprio orchestrator branch (não o
    // entry(args) pacaminho então it é intentionally Não em LOCAL_LANGUAGES.
