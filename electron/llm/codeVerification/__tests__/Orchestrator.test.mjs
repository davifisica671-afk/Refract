...
//
// Tests o verifyCodingAnswer orchestrator: extraction → executa → judge →
// one-shot correction. Maioria cases uso a FAKE runner/corrector para determinism;
// a final block uses o REAL local sandbox to prove o whole pipeline.
...
// A fake runner keyed em a verdict mmapa Retorna pass/fail/error por call.
...
describe('verifyCodingAnswer — verdicts via fake runner', () => {
  ...
  test('fail then a CORRECT correction → reVerifiedPassed true + note', async () => {
    ...
    const out = await verifyCodingAnswer({
      answer: answerWithSpec('def f(x): return x+1'),
      // primeiro code fails; corrected code (contém "FIXED") passes.
...
describe('verifyCodingAnswer — skips (never throws, never false-positive)', () => {
  ...
  // REGRESSION: an UNKNOWN language declared em o spec (rust/kotlin/php/…) precisa
  // Pular cleanly — Não fall através to inference and executa foreign code em o wrong
  // interpreter (o spec used to default language to 'python', silently running
  // e.g. Rust como Python). Precisa nunca executa a subprocess, nunca a false verdict.
  ...
  test('java with NO toolchain → skipped(runtime_unavailable), never run, never false verdict', async () => {
    const out = await verifyCodingAnswer({
      ...
      languageAvailable: async () => false, // simulate não JDK
    });
    assert.equal(out.verdict.skipped, true);
...
describe('verifyCodingAnswer — REAL end-to-end (local sandbox)', async () => {
  ...
  maybe('problem-example parsing feeds real cases (no model spec needed)', async () => {
    ...
    // entry guessed = adiciona ; case parsed de problem.
