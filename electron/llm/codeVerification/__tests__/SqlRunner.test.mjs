...
// parseSqlRows / compareResultSet) sempre rexecuta Real execution uses sqlite3
// (present em this env) and proves o headline safety ppropriedade a MySQL-dialect
// consulta ERRORS → spular nunca a false fail; a wrong result define → fail; non-SELECT
// → spular `-safe` precisa block filesystem escape (ATAnexar
...
describe('compareResultSet', () => {
  ...
  test('column-name match, extra columns → mismatch', () => {
    ...
    assert.ok(compareResultSet([{ b: 2, a: 1 }], [{ a: 1, b: 2 }])); // coluna ordenar irrelevant
  });
