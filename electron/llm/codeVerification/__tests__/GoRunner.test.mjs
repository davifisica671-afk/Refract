...
//
// Go verification (GAP A). Driver GENERATION é pure and sempre tested. Real
// compile/run é gated em a local `go` toolchain and Pula quando absent — em
// this env there's não `go`, então o orchestrator precisa Pular (runtime_unavailable),
// nunca a false verdict. Quando Go é installed o gated tests prove o pcaminho
...
describe('buildGoProgram (pure)', () => {
  test('builds package main + typed decls + sentinels', () => {
    ...
    assert.match(p, /__REFRACT_RESULT_START__/);
  });
  ...
  test('does NOT redefine a model-provided struct', () => {
    ...
    // exatamente one definition (o model's) — nosso preamble precisa não adiciona a segundo
