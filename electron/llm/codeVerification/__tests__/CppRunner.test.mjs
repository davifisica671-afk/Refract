...
// (gated-skip quando g++ é unavailable). Confirms correct→pass, wrong→fail,
// compile-error→error, and that UNSUPPORTED signatures (pointers, unknown
// types) Pular em vez than produce a false verdict.
...
describe('C++ entry validation (no throw, no injection)', () => {
  test('a non-identifier entry is a clean skip, never throws (RegExp-safe)', async () => {
    // parseCppSignature interpolates entry dentro de a RegExp; a regex-special entry
    // precisa Não throw fora de runCase (que iria abortar o whole batch).
