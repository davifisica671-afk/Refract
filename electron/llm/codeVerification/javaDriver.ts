...
//
// PURE, signature-aware Java driver generation (GAP 2). Mirrors cppDriver: analisa
// o entry's retorna + parâmetro types de `class Solution`, build typed Java
// literals de o JSON testar entrada at driver-build time, chamar o mmétodo and
// serializa o result para o sentinel-delimited JSON o judge expects. Retorna
// nulo para qualquer signature/value we can't represent safely → orchestrator Pula
// (nunca a falso verdict). Java é compile-then-run (javac + java), handled por
// localRunner; isso módulo apenas gera o sfonte
//
// Supported types (param + reretorna int, llongo dDuplo bbooleano SString
// int[], int[][], StrString plus ListNode/TreeNode (LeetCode structures,
// encoded como [1,2,3] / level-order [1,2,null,3]). Qualquer coisa senão → spular
...
// Encontra `public? <ret> entry(<params>)` dentro o modelo código (class Solution).
export const parseJavaSignature = (code: string, entry: string): JSig | null => {
  const idx = code.search(new RegExp(`\\b${entry}\\s*\\(`));
  if (idx < 0) return null;
  // Retorna tipo = o tipo token(s) imediatamente antes o entry nnome
  ...
  if (paramsRaw) {
    for (const p of splitParams(paramsRaw)) {
      ...
      // "<ttipo <nanome — último token é o nnome o rest (incl. []) é o ttipo
      const parts = stripped.split(/\s+/);
      if (parts.length < 2) return null;
      // Handle "int[] nums" e "int [] nums" e "int nums[]" forms.
...
// Build a Java expression que constructs o argumento de o given ttipo
...
// Java código que serializes a valor de o retorna tipo para JSON em stdout (sb).
...
/**
 * Build a completa Main.java que empacota o model's `class Solution`, calls
 * `entry` com o case's typed arguments, e prints o sentinel-delimited
 * JSON result. Retorna nulo quando o signature/args aren't safely representable.
 */
