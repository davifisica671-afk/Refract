// tests/intelligence/e2e/RefractIntelligence100Questions.test.mjs
//
// ============================================================================
//  REFRACT INTELLIGENCE OS — 100-QUESTION END-TO-END VERIFICATION SUITE
// ============================================================================
//
...
//      (real provider) against a SAFE COPY of the real refract.db. This is the EXACT
...
//
// Run: node --test tests/intelligence/e2e/RefractIntelligence100Questions.test.mjs
...
const ENABLED_FLAGS = {
  REFRACT_CONVERSATION_MEMORY_V2: '1',
  REFRACT_MEETING_MEMORY_V2: '1',
  REFRACT_GLOBAL_SEARCH_V2: '1',
  REFRACT_IN_MEETING_SEARCH_V2: '1',
  REFRACT_LECTURE_INTELLIGENCE_V2: '1',
  REFRACT_DIAGRAM_INTELLIGENCE: '1',
  REFRACT_ANSWER_DIVERSITY_GUARD: '1',
  REFRACT_HINDSIGHT_MEMORY: '1',
};
...
after(() => {
  ...
  const outPath = path.join(REPO_ROOT, 'refract-intelligence-e2e-results.json');
  const byCat = {};
...
for (const q of QUESTIONS.filter((x) => x.category === 'A' || x.category === 'B')) {
  test(`[${q.id}] ${q.category} manual — ${q.question}`, async () => {
    ...
    else if (scan.refractIdentityLeak) reason = 'WRONG IDENTITY — answer claims to be Refract/an AI assistant on a candidate question';
...
for (const q of QUESTIONS.filter((x) => x.category === 'C')) {
  test(`[${q.id}] C WTA — ${q.question}`, async () => {
    ...
    else if (scan.refractIdentityLeak) reason = 'WRONG IDENTITY — "I\'m Refract" in live copilot';
