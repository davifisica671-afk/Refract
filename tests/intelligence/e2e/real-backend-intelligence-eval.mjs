/**
 * real-backend-intelligence-eval.mjs — drives the REAL Refract answer path
...
const REFRACT_LEAK_RE = /\bI(?:'m| am)\s+Refract\b|\bI(?:'m| am)\s+an?\s+(?:AI\s+)?(?:assistant|language model|chatbot)\b|\bas\s+an\s+AI(?:\s+(?:language\s+)?model)?,?\s+I\b/i;
...
const BOT_MARKERS = [
  ...
  { re: /\bI(?:'m| am) Refract\b/i, tag: 'i_am_refract' },
...
function humanLikeness(text, c) {
  ...
  const hardMarkers = flags.filter((f) => ['as_an_ai', 'i_am_refract', 'the_candidate', 'according_to_resume', 'provided_context'].includes(f));
...
function score(c, answer, plan, tokens) {
  ...
  for (const s of c.mustNotContain || []) {
    ...
    const hit = /^nativel?y$/i.test(needle.trim()) ? PRODUCT_REFRACT_RE.test(text) : lower.includes(needle.toLowerCase());
  ...
  if (c.expectedVoice === 'first_person_candidate' && REFRACT_LEAK_RE.test(text)) f.push('refract_identity_leak');
  ...
  // coding/technical profile-forbidden answers must not leak Refract/profile —
  ...
  if (plan.profileContextPolicy === 'forbidden' && !c.profileShouldBeUsed) {
    ...
    if ((PRODUCT_REFRACT_RE.test(text) || CODING_PROFILE_RE_NONPRODUCT.test(text)) && !isInputRequest) f.push('coding_profile_leak');
    ...
    if (isWTA) {
      ...
      if (!usedFastPath) {
        ...
        if (plan.profileContextPolicy === 'forbidden' && h.stripProfileTokensFromCoding && h.validateProfileOutput) {
          const profileExplicitlyInvited = /\b(use|using|with|in|from)\s+(my|your|the)\s+(refract|project|portfolio)\b|\bin refract\b|\b(my|your) refract project\b/i.test(c.question);
...
function writeOutputs(raw, results, served, rotating, wallMs) {
  ...
  const meta = {
    ...
    leaks: { identity: cnt('refract_identity'), falseRefusal: cnt('false_refusal'), stealth: cnt('stealth_evasion'), codingProfile: cnt('coding_profile'), contextLeak: cnt('context_leak'), invented: cnt('invented_link'), hallucinated: cnt('hallucinated_source'), safetyNotRouted: cnt('safety_not_routed') },
