 ...
 * `llmHelper.groqClient` seam so the REAL Refract backend (routing → context →
 ...
 * NO MiniMax transport — MiniMax lives only in the refract-api gateway. This adapter
...
// ── status-code → reason (mirrors refract-api/lib/minimaxProvider.js) ───────────
