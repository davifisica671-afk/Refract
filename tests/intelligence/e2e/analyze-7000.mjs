...
function analyze() {
  ...
  for (const m of MODES) {
    ...
    byMode[m] = {
      ...
      identityLeaks: set.filter((r) => (r.failure_reason || '').includes('refract_identity') || (r.failure_reason || '').includes("forbidden_substring:I'm Refract") || (r.failure_reason || '').includes('forbidden_substring:I am Refract')).length,
  ...
  const critical = {
    identityLeak: clean.filter((r) => has(r, 'refract_identity') || has(r, "forbidden_substring:I'm Refract") || has(r, 'forbidden_substring:I am Refract')).length,
