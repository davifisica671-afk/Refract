 ...
 *   - identity        : Developer ID Application (auto-discovered, or REFRACT_SIGN_IDENTITY/CSC_NAME)
 ...
 *   Refract executable). So we build ONLY the `zip` target with electron-builder
 ...
 *
 *   Notarization credentials use the macOS keychain profile `refract-notary`
...
process.env.REFRACT_PRODUCTION_SIGN = '1';
...
process.env.APPLE_KEYCHAIN_PROFILE = process.env.APPLE_KEYCHAIN_PROFILE || 'refract-notary';
...
const signIdentity =
  process.env.REFRACT_SIGN_IDENTITY || process.env.CSC_NAME || undefined;
