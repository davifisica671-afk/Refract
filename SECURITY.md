...
We take the security of Refract seriously and we genuinely appreciate reports from researchers, users, and the community. This document describes how to report a vulnerability, what we'll do with it, and the rules of engagement.
...
You can find the current version in the app's **Settings → About** screen, in `package.json` at the repo root, or on the [GitHub Releases page](https://github.com/Refract-AI-assistant/refract/releases).
...
Email **refract.contact@gmail.com** with a clear subject line such as "Security report — [short description]".
...
- **The refract.software website** — server misconfigurations, common web vulnerabilities, and information leaks.
...
- **Prompt-injection attacks** that cause the underlying AI provider to behave in unintended ways. These are limitations of the underlying models, are in scope for the model provider's own safety processes, and are discussed in our [Terms &amp; Conditions §11](https://refract.software/termsandconditions). Reports about a *specific way to bypass a safety filter on OpenAI / Anthropic / Google models* should go to the respective provider.
...
<https://github.com/Refract-AI-assistant/refract/security/advisories>
...
For Privacy Policy questions, see [PRIVACY.md](./PRIVACY.md). For commercial / refund / licensing questions, see the [Refund Policy](https://refract.software/refundpolicy) and [Terms &amp; Conditions](https://refract.software/termsandconditions).

Thanks for helping keep Refract safe.
