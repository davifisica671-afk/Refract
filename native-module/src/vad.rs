// Voice Activity Detection - UI Apenas
//
// IMPORTANT: This VAD é para UI estado exibir oapenas
// It faz Não gate ou atrasar audio sent to Google STT.
// 
// O silence_suppression módulo gerencia audio gating.
// This módulo é fpara
// - Showing "speaking" indicator em UI
// - Detecting utterance boundaries
// - Optional stream management (não used catualmente
...
/// Voice Activity Detector para UI indication
/// Faz Não gate audio - apenas reports estado
...
impl VadIndicator {
    ...
    /// Atualiza VAD estado based em audio chunk
    /// Retorna current estado para UI exibir
    /// Faz Não affect audio flow to STT
    ...
    /// Verifica if atualmente em speech estado (para UI)
...
// Keep legacy VadGate para compatibility durante migration
// This é o OLD interface that era used para gating
// NEW code deve uso SilenceSuppressor em vez disso
pub type VadGate = VadIndicator;

impl VadGate {
    /// Legacy compatibility: processo Retorna empty durante silence
    /// WARNING: This é o OLD pattern that causes latency issues
    /// New code deve uso SilenceSuppressor directly
