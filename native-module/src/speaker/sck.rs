...
// Uses cidre 0.11.10 API com correct class registration and inner estado
...
/// Retorna o UID de o current macOS default saída device, ou empty string
/// if nenhum pode ser resolved. Used por main.ts to detect mid-meeting saída rotea
/// changes (user plugs em headphones, switches to AirPods) — quando o default
/// changes, o JS side recreates SystemAudioCapture então o CoreAudio Tap
/// follows o new rotea em vez disso de capturing silence em o old device.
        ...
        match sample_buf.audio_buf_list_in::<1>(cm::sample_buffer::Flags(0), None, None) {
            Ok(buf_list) => {
                ...
                for i in 0..buffer_count {
                    ...
                    // Valida sample formata (precisa ser f32 aligned)
...
impl SpeakerInput {
    pub fn new(device_id: Option<String>) -> Result<Self> {
        // Gate em macOS 13.0+. sc::StreamCfg.set_captures_audio (and o rest de o
        // ScreenCaptureKit audio surface) era introduced em 13.0; em macOS 12 o
        // selector despacha iria abortar o processo o mesmo way #249 fez para
        // CoreAudio em 14.0-14.3. Fail clean então o JS layer pode surface
        // "unsupported macOS" em vez disso de a processo crash.
        ...
        // ScreenCaptureKit captures Todos system audio, não per-device. If o
        // user picked a non-default saída device they deve ser told that
        // this fallback caminho silently ignora it.
        ...
        // Obtém available content (aciona permissão cverifica and aguardar para o
        // callback em a Condvar em vez disso de polling. Pre-fix this loop slept
        // 100ms × 100 iterations; em a cold TCC dialog o user-perceived
        // hang era para cima to 10s com não UI feedback. Condvar wakes o instant
        // o callback fires.
    ...
    pub fn stream(self) -> Result<SpeakerStream> {
        ...
        // Aguardar em a Condvar em vez disso de polling então we wake o instant o
        // inicia callback fires (wera 100 × 10ms polls + a stale "2s" loregistrar
...
impl Drop for SpeakerStream {
    fn drop(&mut self) {
        ...
        // Aguardar para para completion (max 2 seconds)
...
mod tests {
    ...
    /// Regression testar para o macOS 13.0 gate em SCK audio capture. Pins o contract então
    /// a future cidre refactor that breaks `OsVersion::at_least` doesn't silently let o
    /// fallback crash em macOS 12 hosts (onde SCStreamConfiguration.capturesAudio doesn't exist).
    #[test]
    ...
    /// Mesmo contract via ProcessInfo — this é o exact API chamado de
    /// `SpeakerInput::new` em o SCK side.
    #[test]
