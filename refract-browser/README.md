# Refract Page Context — companion browser extension

A minimal, privacy-correct Manifest V3 extension that sends the **active tab's
readable content** to your local Refract desktop app, **once, on demand**. It
never runs in the background, never auto-captures, and only ever talks to your
own machine over loopback.

It is part of the [Refract](../) monorepo and is licensed **AGPL-3.0-only**,
same as the desktop app. Source lives in this directory (`refract-browser/`).

## How it works

Capture is triggered from the **Refract desktop app** (a global hotkey that works
from any focused app — including while you're looking at the Refract overlay, not
...
```
Refract desktop hotkey (default ⌘/Ctrl+Shift+Y, owned by the desktop app)
...
4. Click **Load unpacked** → select `refract-browser/dist`
...
1. In Refract desktop: **Settings → Phone Mirror** → enable, then in the
   **Browser Extension** card click **Connect browser extension** (opens a 60s
   window).
2. Click the extension's toolbar icon → **Connect to Refract**. Done — a green
...
- **From the desktop (primary):** press the Refract capture hotkey
  (**`⌘/Ctrl+Shift+Y`** by default, configurable in Refract's keybindings). This
  works from any focused app — including the Refract overlay. If the browser
...
- **"Pairing expired"** (amber dot): you clicked **Rotate token** in Settings (the
  deliberate security reset). Re-pair via **Connect to Refract**.
- **"Open Refract and enable Phone Mirror"** (red dot): Phone Mirror is off. The
...
source with yet another unpacked ID can override via the `REFRACT_DOM_EXTENSION_ID`
env var.
