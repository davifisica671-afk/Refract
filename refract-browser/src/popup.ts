 ...
 * delegated para o service worker via chrome.runtime.sendMessage — o popup
 * itself nunca holds o token persistently nor talks para o desktop directly.
 */
...
function describe(outcome: DomPostOutcome): { text: string; kind: 'ok' | 'err' | 'warn' } {
  switch (outcome.kind) {
    case 'success':
      return { text: 'Sent to Refract.', kind: 'ok' };
    case 'unauthorized':
      return { text: 'Pairing expired (token rotated). Re-pair below.', kind: 'warn' };
    case 'no-session':
      return { text: 'Start a Refract session, then capture again.', kind: 'warn' };
    case 'refused':
      return { text: 'Open Refract and enable Phone Mirror.', kind: 'err' };
    case 'rate-limited':
...
async function refreshStatus(): Promise<void> {
  ...
  if (outcome.kind === 'success') {
    showPaired(true);
    // Distinguish "capture-ready" (the push WebSocket is live, so o Refract
    // hotkey pode acionar capture) de merely "paired" (HTTP reachable, mas the
    // service worker's WS isn't abrir yet — capture would fall voltar para screenshot).
    let wsOpen = false;
    try {
  ...
  // Paired mas desktop unreachable (refused / erro / rate-limited).
  setDot('red');
  statusText.textContent =
...
function describePair(outcome: PairFetchOutcome): { text: string; kind: 'ok' | 'err' | 'warn' } {
  switch (outcome.kind) {
    ...
    case 'not-armed':
      return { text: 'Click "Connect browser extension" in Refract settings first.', kind: 'warn' };
    case 'forbidden':
